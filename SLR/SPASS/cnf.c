...
***************************************************************/
{
  ...
  term_AddFatherLinks(Term);
  Term = ren_Rename(Term, Precedence, Symblist, FALSE, FALSE, flag_GetFlagValue(Flags, flag_CNFRENAMING));
...
***************************************************************/
{
  ...
  for (scan=Usables; !list_Empty(scan); scan = list_Cdr(scan)) {
    ...
    if (flag_GetFlagValue(Flags, flag_DOCPROOF)  || flag_GetFlagValue(Flags, flag_FLOTTER)) {
#ifdef CHECK
