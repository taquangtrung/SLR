 ...
 **********************************************************/
{
	...
	if (Term1 == Term2) /* pointers are equal */
		...
		LIST Scan1, Scan2;
		for (Scan1 = term_ArgumentList(Term1), Scan2 = term_ArgumentList(Term2); list_Exist(Scan1)
				&& list_Exist(Scan2); Scan1 = list_Cdr(Scan1), Scan2 = list_Cdr(Scan2))
 ...
 *******************************************************/
{
	...
	do {
		if (term_EqualTopSymbols(Term1, Term2) && term_IsComplex(Term1) && term_IsComplex(Term2)) {
...
// TTQ_COMMENT . func - term_IsGround : check ground or non ground Term
 ...
 *********************************************************/
{
	...
	if (term_IsComplex(Term)) {
		...
		for (Scan = ArgumentList = list_Copy(term_ArgumentList(Term)); list_Exist(Scan); Scan
				= list_Cdr(Scan))
 ...
 *********************************************************/
{
	...
	if (term_IsComplex(Term)) {
		...
		TopStack = list_Push((POINTER) term_TopSymbol(Term), list_StackBottom());
		ArgumentStack = list_Push(list_Copy(term_ArgumentList(Term)), list_StackBottom());
		ActStack = list_Push(list_Top(ArgumentStack), list_StackBottom());

		while (!list_StackEmpty(TopStack)) {
			if (list_Empty(list_Top(ActStack))) {
				Term = term_Create((SYMBOL) list_Top(TopStack), (LIST) list_Top(ArgumentStack));
				...
				if (term_IsComplex(Term)) {
					...
					ArgumentStack = list_Push(list_Copy(term_ArgumentList(Term)), ArgumentStack);
					ActStack = list_Push(list_Top(ArgumentStack), ActStack);
				} else {
					list_Rplaca(list_Top(ActStack), term_Create(term_TopSymbol(Term), list_Nil()));
					list_RplacTop(ActStack, list_Cdr(list_Top(ActStack)));
				}
			}
		}
...
TERM term_CopyWithEmptyArgListNode(TERM Term, LIST ArgListNode, LIST* ListNodeCopyPt)
/**********************************************************
 ...
 *********************************************************/
{
	...
	if (term_IsComplex(Term)) {
		...
		for (Scan = ArgumentList; list_Exist(Scan); Scan = list_Cdr(Scan), HelpScan = list_Cdr(
				HelpScan))
			if (HelpScan != ArgListNode)
				list_Rplaca(Scan, term_CopyWithEmptyArgListNode(list_Car(Scan), ArgListNode,
						ListNodeCopyPt));
			else {
 ...
 *************************************************************/
{
	if (Term == (TERM) NULL)
		...
		putchar(' ');
		list_Apply((void(*)(POINTER)) term_PrintWithEmptyArgListNode, term_ArgumentList(Term));
		putchar(')');
 ...
 *******************************************************/
{
	...
	if (symbol_Equal(term_TopSymbol(Term), Symbol)) {
		...
		Result = FALSE;
		for (List = term_ArgumentList(Term); list_Exist(List); List = list_Cdr(List))
 ...
 ***************************************************************/
{
	...
	if (symbol_IsVariable(lsymb)) {
		if (symbol_IsVariable(rsymb))
			...
			return 1;
	} else if (symbol_IsVariable(rsymb))
		return -1;
 ...
 ***************************************************************/
{
	...
	if (symbol_IsFunction(lsymb)) {
		if (symbol_IsFunction(rsymb)) {
			...
			while (!list_Empty(largs)) {
				result = term_CompareBySymbolOccurences(list_Car(largs), list_Car(rargs));
				if (result != 0)
					break;
 ...
 ***************************************************************/
{
	...
	static const TERM_COMPARE_FUNCTION term_compare_functions[] = { term_CompareByArity,
			term_CompareBySymbolOccurences, term_CompareByConstants, term_CompareByVariables,
 ...
 ***************************************************************/
{
	...
	while (!stack_Empty(stack)) {
		...
		if (term_IsComplex(Term)) {
			for (Scan = term_ArgumentList(ActTerm); !list_Empty(Scan); Scan = list_Cdr(Scan))
 ...
 ***************************************************************/
{
	if (term_IsComplex(Term)) {
		...
		if (term_HasJustification(Term)) {
			...
			putc(':', File);
			putc(' ', File);
			term_TermListFPrint(File, term_JustificationList(Term));
		}
#endif
 ...
 ***************************************************************/
{
	if (term_IsComplex(Term)) {
		...
		if (term_HasJustification(Term)) {
			...
			putc(':', File);
			putc(' ', File);
			term_TermListFPrintPrefix(File, term_JustificationList(Term));
		}
#endif
 ...
 ***************************************************************/
{
	if (term_IsComplex(Term)) {
		...
		if (term_HasJustification(Term)) {
			...
			putc(':', File);
			putc(' ', File);
			term_TermListFPrintOtterPrefix(File, term_JustificationList(Term));
		}
#endif
	} else
		symbol_FPrintOtter(File, term_TopSymbol(Term));
 ...
 ***************************************************************/
{
	...
	for (Scan = term_ArgumentList(TopTerm), pos = 1; !list_Empty(Scan); Scan = list_Cdr(Scan), pos++) {
 ...
 ***************************************************************/
{
	...
	do {
		if (term_IsComplex(Term))
			stack_Push(term_ArgumentList(Term));
		else if (symbol_Equal(term_TopSymbol(Term), Var)) {
			stack_SetBottom(Stack);
			return TRUE;
		}
 ...
 ***************************************************************/
{
	...
	if (term_IsComplex(Term))
		for (Scan = term_ArgumentList(Term); !list_Empty(Scan); Scan = list_Cdr(Scan))
			Result = list_Nconc(term_MakePseudoLinear(list_Car(Scan), Depth + 1, Mark), Result);
	else {
		Top = term_TopSymbol(Term);
		if (symbol_IsVariable(Top)) {
			if (term_VarIsMarked(Top, Mark)) {
				...
				Result = list_Cons(list_PairCreate((POINTER) Top, (POINTER) term_TopSymbol(Term)),
						Result);
			} else {
 ...
 ***************************************************************/
{
	...
	list_Apply((void(*)(POINTER)) term_SetTermSubtermStamp, term_ArgumentList(Term));
}
 ...
 ***************************************************************************/
{
	...
	Top = term_TopSymbol(Term);
	if (symbol_IsSignature(Top) && symbol_Arity(Top) != -1 && symbol_Arity(Top)
			!= (int) list_Length(term_ArgumentList(Term)))
		return FALSE;
	...
	if (Links) {
		for (Scan = term_ArgumentList(Term); !list_Empty(Scan); Scan = list_Cdr(Scan)) {
			if (term_Superterm(list_Car(Scan)) != Term || !term_CheckTermIntern(list_Car(Scan),
					Links))
				return FALSE;
		}
	} else {
		for (Scan = term_ArgumentList(Term); !list_Empty(Scan); Scan = list_Cdr(Scan)) {
			if (term_Superterm(list_Car(Scan)) != term_Null() || !term_CheckTermIntern(list_Car(
					Scan), Links))
				return FALSE;
		}
	}
 ...
 *********************************************************************************/
{
	if (term_IsComplex(Term) && term_Superterm(term_FirstArgument(Term)) != term_Null())
