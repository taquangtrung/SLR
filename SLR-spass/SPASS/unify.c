 ...
 ***********************************************************/
{
	...
	for (;;) {
		...
		while (term_IsVariable(TermL) && cont_VarIsBound(CtL, term_TopSymbol(
				TermL))) {
		...
		while (term_IsVariable(TermR) && cont_VarIsBound(CtR, term_TopSymbol(
				TermR))) {
		...
		if (term_IsVariable(TermL)) {
			if (term_IsVariable(TermR)) {
				if (!(CtL == CtR && term_EqualTopSymbols(TermL, TermR)))
					Bound = cont_CreateBinding(CtR, term_TopSymbol(TermR), CtL,
							TermL);
			} else if (Bound && unify_OccurCheck(CtL, term_TopSymbol(TermL),
					CtR, TermR)) {
				stack_SetBottom(Bottom);
				return FALSE;
			} else
				Bound = cont_CreateBinding(CtL, term_TopSymbol(TermL), CtR,
						TermR);

		} else if (term_IsVariable(TermR)) {
			if (Bound && unify_OccurCheck(CtR, term_TopSymbol(TermR), CtL,
					TermL)) {
				stack_SetBottom(Bottom);
				return FALSE;
			} else
 ...
 ***********************************************************/
{
	...
	for (;;) {
		...
		while (term_IsVariable(TermL) && cont_VarIsBound(Context,
				term_TopSymbol(TermL)))
			TermL = cont_ContextBindingTerm(Context, term_TopSymbol(TermL));

		while (term_IsVariable(TermR) && cont_VarIsBound(Context,
				term_TopSymbol(TermR)))
		...
		if (term_EqualTopSymbols(TermL, TermR)) {
			...
			if (term_IsVariable(TermR))
				cont_CreateBinding(Context, term_TopSymbol(TermL), Context,
						TermR);
			...
			if (unify_OccurCheckCom(term_TopSymbol(TermR), Context, TermL)) {
				...
				cont_CreateBinding(Context, term_TopSymbol(TermR), Context,
						TermL);
...
BOOL unify_UnifyAllOC(CONTEXT IndexContext, CONTEXT CtL, TERM TermL,
	...
	if (term_IsVariable(TermL)) {
		...
		if (term_IsVariable(TermR)) {
			if ((CtL != CtR || !term_EqualTopSymbols(TermL, TermR))) {
				if (term_IsIndexVariable(TermL))
					cont_CreateBinding(CtL, term_TopSymbol(TermL), CtR, TermR);
				else if (term_IsIndexVariable(TermR) || (CtR == IndexContext))
					cont_CreateBinding(CtR, term_TopSymbol(TermR), CtL, TermL);
				else
					cont_CreateBinding(CtL, term_TopSymbol(TermL), CtR, TermR);
			}
			return TRUE;
		} else if (unify_OccurCheck(CtL, term_TopSymbol(TermL), CtR, TermR))
			return FALSE;
		else {
			cont_CreateBinding(CtL, term_TopSymbol(TermL), CtR, TermR);
			return TRUE;
		...
		if (term_IsComplex(TermL)) {
			LIST ArgL, ArgR;
			for (ArgL = term_ArgumentList(TermL), ArgR = term_ArgumentList(
					TermR); !list_Empty(ArgL); ArgL = list_Cdr(ArgL), ArgR
					= list_Cdr(ArgR))
				if (!unify_UnifyAllOC(IndexContext, CtL, list_Car(ArgL), CtR,
						list_Car(ArgR)))
					return FALSE;
		}
		return TRUE;
...
BOOL unify_Match(CONTEXT Context, TERM TermL, TERM TermR) {
	if (term_IsVariable(TermL)) {
		if (cont_VarIsBound(Context, term_TopSymbol(TermL)))
			return term_Equal(cont_ContextBindingTerm(Context, term_TopSymbol(
					TermL)), TermR);
		else {
			cont_CreateBinding(Context, term_TopSymbol(TermL),
					cont_InstanceContext(), TermR);
			return TRUE;
		}
	} else if (term_EqualTopSymbols(TermL, TermR)) {
		if (term_IsComplex(TermL)) {
 ...
 ***************************************************************/
{
	...
	} else if (term_EqualTopSymbols(TermL, TermR) && list_Length(
			term_ArgumentList(TermL)) == list_Length(term_ArgumentList(TermR))) {
		if (term_IsComplex(TermL)) {
			LIST ArgL;
			LIST ArgR;
			for (ArgL = term_ArgumentList(TermL), ArgR = term_ArgumentList(
					TermR); !list_Empty(ArgL); ArgL = list_Cdr(ArgL), ArgR
					= list_Cdr(ArgR))
				if (!unify_MatchFlexible(Context, list_Car(ArgL),
						list_Car(ArgR)))
					return FALSE;
		}
		return TRUE;
...
void unify_EstablishMatcher(CONTEXT Context, SUBST Subst) {
	while (subst_Exist(Subst)) {
		/* Index to query */
		cont_CreateBinding(Context, subst_Dom(Subst), cont_InstanceContext(),
				subst_Cod(Subst));
		Subst = subst_Next(Subst);
	}
}

BOOL unify_MatchBindingsHelp(const CONTEXT IndexContext, TERM TermL,
		CONTEXT CtR, TERM TermR) {
	while (term_IsVariable(TermR)) {
		SYMBOL TermTop;
	...
	if (term_IsVariable(TermL)) {
		...
		if (cont_VarIsBound(IndexContext, term_TopSymbol(TermL)))
			return cont_TermEqualModuloBindings(IndexContext,
					cont_ContextBindingContext(IndexContext, term_TopSymbol(
							TermL)), cont_ContextBindingTerm(IndexContext,
							term_TopSymbol(TermL)), CtR, TermR);
		else {
...
BOOL unify_MatchBindings(const CONTEXT IndexContext, TERM TermL, TERM TermR) {
	return unify_MatchBindingsHelp(IndexContext, TermL, cont_InstanceContext(),
			TermR);
}
 ...
 **********************************************************/
{
	...
	if (term_IsVariable(TermL)) {
		if ((CtR == cont_InstanceContext()) && term_EqualTopSymbols(TermL,
				TermR))
		...
		} else if (term_IsVariable(TermR) && (term_IsIndexVariable(TermR)
				|| (CtR == IndexContext))) {
			cont_CreateBinding(IndexContext, term_TopSymbol(TermR),
					cont_InstanceContext(), TermL);
			return TRUE;
		...
		if (term_IsComplex(TermL)) {
			...
			for (ArgL = term_ArgumentList(TermL), ArgR = term_ArgumentList(
					TermR); !list_Empty(ArgL); ArgL = list_Cdr(ArgL), ArgR
					= list_Cdr(ArgR))
				if (!unify_MatchReverse(IndexContext, list_Car(ArgL), CtR,
						list_Car(ArgR)))
					return FALSE;
		}
		return TRUE;
...
BOOL unify_Variation(const CONTEXT Context, TERM TermL, TERM TermR) {
	if (term_IsVariable(TermL)) {
		if (term_EqualTopSymbols(TermL, TermR))
			...
			if (cont_VarIsBound(Context, term_TopSymbol(TermL)))
				return term_Equal(cont_ContextBindingTerm(Context,
						term_TopSymbol(TermL)), TermR);
			else {
				/* Index to query */
				cont_CreateBinding(Context, term_TopSymbol(TermL), Context,
						TermR);
...
TERM unify_ComGenLinear(const CONTEXT IndexContext, SUBST* SubstL, TERM TermL,
		SUBST* SubstR, TERM TermR) {
	if (term_IsIndexVariable(TermR)) {
		*SubstL = subst_Add(term_TopSymbol(TermR), term_Copy(TermL), *SubstL);
		...
		ArgList = list_Nil();
		for (ArgL = term_ArgumentList(TermL), ArgR = term_ArgumentList(TermR); !list_Empty(
				ArgL); ArgL = list_Cdr(ArgL), ArgR = list_Cdr(ArgR))
			ArgList = list_Nconc(ArgList, list_List(unify_ComGenLinear(
					IndexContext, SubstL, list_Car(ArgL), SubstR,
					list_Car(ArgR))));
