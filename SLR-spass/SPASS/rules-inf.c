...
static BOOL inf_LitMax(CLAUSE Clause, int i, int j, SUBST Subst, BOOL Strict, FLAGSTORE Flags,
 ...
 ***************************************************************/
{
	...
	if (!clause_LiteralIsMaximal(Lit) || (Strict && !clause_LiteralGetFlag(Lit, STRICTMAXIMAL)))
		return FALSE;
	...
	/* is sufficient, too.                                                */
	if ((clause_NumOfAnteLits(Clause) + clause_NumOfSuccLits(Clause) == 1) || subst_Empty(Subst))
		return TRUE;
	...
	for (k = clause_FirstAntecedentLitIndex(Clause); k <= l; k++)
		if (k != i && k != j && clause_LiteralIsMaximal(clause_GetLiteral(Clause, k))) {
			...
			Compare = ord_LiteralCompare(Max, clause_LiteralIsOrientedEquality(clause_GetLiteral(
					Clause, i)), LitTerm, clause_LiteralIsOrientedEquality(clause_GetLiteral(
					Clause, k)), TRUE, Flags, Precedence);
...
static BOOL inf_LiteralsMax(CLAUSE Clause, int i, SUBST Subst, CLAUSE PartnerClause, int j,
		SUBST PartnerSubst, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (!clause_GetFlag(Clause, CLAUSESELECT) && !inf_LitMax(Clause, i, -1, Subst, i
			> clause_LastAntecedentLitIndex(Clause), Flags, Precedence))
		return FALSE;
	if (!clause_GetFlag(PartnerClause, CLAUSESELECT) && !inf_LitMax(PartnerClause, j, -1,
			PartnerSubst, j > clause_LastAntecedentLitIndex(PartnerClause), Flags, Precedence))
		return FALSE;

	return TRUE;
}

static BOOL inf_LitMaxWith2Subst(CLAUSE Clause, int i, int j, SUBST Subst2, SUBST Subst1,
		BOOL Strict, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause, the index of a maximal literal, another
 ...
 ***************************************************************/
{
	...
	/* is sufficient, too.                                                   */
	if ((clause_NumOfAnteLits(Clause) + clause_NumOfSuccLits(Clause) == 1) || (subst_Empty(Subst1)
			&& subst_Empty(Subst2)))
		return TRUE;
...
static BOOL inf_LiteralsMaxWith2Subst(CLAUSE Clause, int i, CLAUSE PartnerClause, int j,
		SUBST Subst2, SUBST Subst1, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (!clause_GetFlag(Clause, CLAUSESELECT) && !inf_LitMaxWith2Subst(Clause, i, -1, Subst2,
			Subst1, i > clause_LastAntecedentLitIndex(Clause), Flags, Precedence))
		return FALSE;
	if (!clause_GetFlag(PartnerClause, CLAUSESELECT) && !inf_LitMaxWith2Subst(PartnerClause, j, -1,
			Subst2, Subst1, j > clause_LastAntecedentLitIndex(PartnerClause), Flags, Precedence))
		return FALSE;
 ...
 ***************************************************************/
{
	...
	if (clause_HasEmptyAntecedent(GivenClause) || !clause_HasSolvedConstraint(GivenClause)) {
		return list_Nil();
	}
	...
	for (i = clause_FirstAntecedentLitIndex(GivenClause); i <= last; i++) {
		...
		if (clause_LiteralIsEquality(ActLit) && (clause_LiteralGetFlag(ActLit, LITSELECT)
				|| (!clause_GetFlag(GivenClause, CLAUSESELECT) && (!Ordered
						|| clause_LiteralIsMaximal(ActLit))))) {
			TERM Atom;

			Atom = clause_GetLiteralAtom(GivenClause, i);

			cont_Check();
			if (unify_UnifyCom(cont_LeftContext(), term_FirstArgument(Atom), term_SecondArgument(
					Atom))) {
				SUBST mgu;
				CLAUSE NewClause;
				int j, k, bound;
				...
				/* Check postcondition */
				if (clause_LiteralGetFlag(ActLit, LITSELECT) || !Ordered || inf_LitMax(GivenClause,
						i, -1, mgu, FALSE, Flags, Precedence)) {

#ifdef _TRUNGTQ_CODE_

					NewClause = clause_CreateBody(clause_Length(GivenClause) - 1, 0);
					...
					clause_SetNumOfAnteLits(NewClause, (clause_NumOfAnteLits(GivenClause) - 1));
					...
					for (j = k = clause_FirstLitIndex(); j <= bound; j++) {
						if (j != i) {
							clause_SetLiteral(NewClause, k, clause_LiteralCreate(subst_Apply(mgu,
 ...
 ***************************************************************/
{
	...
	NewClause = clause_CreateBody(clause_Length(Clause), 0);
	...
	for (k = clause_FirstLitIndex(); k <= c; k++) {
		clause_SetLiteral(NewClause, k, clause_LiteralCreate(subst_Apply(Subst, term_Copy(
				clause_GetLiteralTerm(Clause, k))), NewClause));
	}
	...
	Atom = term_Create(fol_Equality(), list_Cons(term_Copy(Left), list_List(term_Copy(Right))));

	clause_SetLiteral(NewClause, k, clause_LiteralCreate(term_Create(fol_Not(), list_List(
			subst_Apply(Subst, Atom))), NewClause));
	...
	for (; k <= s; k++) {
		if (k == i)
			...
			clause_SetLiteral(NewClause, (k + a), clause_LiteralCreate(subst_Apply(Subst,
...
static BOOL inf_EqualityFactoringApplicable(CLAUSE Clause, int i, TERM Left, TERM Right,
		SUBST Subst, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (!clause_LiteralIsOrientedEquality(clause_GetLiteral(Clause, i))) {
		...
		if ((Help = ord_Compare(NLeft, NRight, Flags, Precedence)) == ord_SmallerThan() || Help
				== ord_Equal()) {
			term_Delete(NLeft);
			term_Delete(NRight);
			return FALSE;
...
LIST inf_EqualityFactoring(CLAUSE GivenClause, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (clause_HasEmptySuccedent(GivenClause) || clause_GetFlag(GivenClause, CLAUSESELECT)
	...
	for (i = clause_FirstSuccedentLitIndex(GivenClause); i <= last; i++) {
		...
		if (clause_LiteralIsMaximal(ActLit) && clause_LiteralIsEquality(ActLit)) {
			...
			for (j = clause_FirstSuccedentLitIndex(GivenClause); j <= last; j++) {
				...
				if (i != j && clause_LiteralIsEquality(PartnerLit)) {
					...
					if (unify_UnifyCom(cont_LeftContext(), Left, PartnerLeft)) {
						subst_ExtractUnifierCom(cont_LeftContext(), &mgu);
						if (inf_EqualityFactoringApplicable(GivenClause, i, Left, Right, mgu,
								Flags, Precedence))
							Result = list_Cons(inf_ApplyEqualityFactoring(GivenClause, Right,
									PartnerRight, i, j, mgu, Flags, Precedence), Result);
						subst_Delete(mgu);
					}
					cont_Reset();
					...
					if (unify_UnifyCom(cont_LeftContext(), Left, PartnerRight)) {
						...
						if (inf_EqualityFactoringApplicable(GivenClause, i, Left, Right, mgu,
								Flags, Precedence))
							Result = list_Cons(inf_ApplyEqualityFactoring(GivenClause, Right,
									PartnerLeft, i, j, mgu, Flags, Precedence), Result);
						subst_Delete(mgu);
					}
					cont_Reset();
					...
					if (!clause_LiteralIsOrientedEquality(ActLit)) {
						...
						if (unify_UnifyCom(cont_LeftContext(), Right, PartnerLeft)) {
							...
							if (inf_EqualityFactoringApplicable(GivenClause, i, Right, Left, mgu,
									Flags, Precedence))
...
static BOOL inf_NAllTermsRplac(TERM Term, TERM TestTerm, TERM RplacTerm, SUBST Subst)
/**************************************************************
...
static TERM inf_AllTermsRplac(TERM Term, TERM TestTerm, TERM RplacTerm, SUBST Subst)
/**************************************************************
...
static TERM inf_AllTermsSideRplacs(TERM Term, TERM TestTerm, TERM RplacTerm, SUBST Subst,
		BOOL Right)
/**************************************************************
...
static TERM inf_AllTermsRightRplac(TERM Term, TERM TestTerm, TERM RplacTerm, SUBST Subst)
/**************************************************************
 INPUT:   Three terms, a substitution.
 <Term> is typically an equality term.
...
static TERM inf_AllTermsLeftRplac(TERM Term, TERM TestTerm, TERM RplacTerm, SUBST Subst)
/**************************************************************
 INPUT:   Three terms, a substitution.
 <Term> is typically an equality term.
 RETURNS: See inf_AllTermSideRplac with argument
 ...
 ***************************************************************/
{
	...
	NewClause = clause_CreateBody(clause_Length(Clause) - 1 + clause_Length(PartnerClause), 0);

#else

	NewClause = clause_CreateBody(clause_Length(Clause) - 1 + clause_Length(PartnerClause));

#endif

	clause_SetNumOfConsLits(NewClause, (clause_NumOfConsLits(Clause) + clause_NumOfConsLits(
			PartnerClause)));
	clause_SetNumOfAnteLits(NewClause, (clause_NumOfAnteLits(Clause) + clause_NumOfAnteLits(
			PartnerClause)));
	clause_SetNumOfSuccLits(NewClause, ((clause_NumOfSuccLits(Clause) - 1) + clause_NumOfSuccLits(
			PartnerClause)));

	/* First set the literals from the Clause : */

	for (j = clause_FirstLitIndex(); j <= lc; j++) {
		clause_SetLiteral(NewClause, j, clause_LiteralCreate(subst_Apply(Subst, term_Copy(
	...
	for (; j <= la; j++) {
		clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(subst_Apply(Subst, term_Copy(
				clause_GetLiteralTerm(Clause, j))), NewClause));
	}
	...
	for (; j <= ls; j++) {
		if (j != ci) {
			...
			clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(subst_Apply(Subst,
	...
	for (j = clause_FirstLitIndex(); j <= plc; j++) {
		clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(subst_Apply(PartnerSubst,
	...
	for (; j <= pla; j++) {
		if (j != pci) {
			...
			clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(term_Create(fol_Not(),
					list_List(SupAtom)), NewClause));
		}
	}
	...
	for (; j <= pls; j++) {
		if (j != pci) {
			...
			/* The PartnerLit is modified appropriately!   */
			clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(SupAtom, NewClause));
		}
	}
	...
	clause_SetDataFromParents(NewClause, PartnerClause, pci, Clause, ci, Flags, Precedence);
...
static LIST inf_GenLitSPRight(CLAUSE Clause, TERM Left, TERM Right, int i, SHARED_INDEX ShIndex,
		BOOL OrdPara, BOOL MaxPara, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	Result = list_Nil();
	Terms = st_GetUnifier(cont_LeftContext(), sharing_Index(ShIndex), cont_RightContext(), Left);
	...
	for (; !list_Empty(Terms); Terms = list_Pop(Terms)) {
		...
		if (!term_IsVariable(Term) && !symbol_IsPredicate(term_TopSymbol(Term))) {
			...
			for (; !list_Empty(Lits); Lits = list_Pop(Lits)) {
				...
				if (!clause_GetFlag(PartnerClause, CLAUSESELECT) && (!MaxPara
						|| clause_LiteralGetFlag(PartnerLit, STRICTMAXIMAL)) && !clause_GetFlag(
						PartnerClause, NOPARAINTO) && clause_LiteralIsPositive(PartnerLit)
					...
					subst_ExtractUnifier(cont_LeftContext(), &Subst, cont_RightContext(),
							&PartnerSubst);
					cont_Reset();
					if (!MaxPara || inf_LiteralsMax(Clause, i, Subst, PartnerClause, pli,
							PartnerSubst, Flags, Precedence)) {
						NewRight = subst_Apply(Subst, term_Copy(Right));
						if (OrdPara && !clause_LiteralIsOrientedEquality(clause_GetLiteral(Clause,
								i)))
							NewLeft = subst_Apply(Subst, term_Copy(Left));
						if (!OrdPara || NewLeft == Left || /* TRUE, if oriented */
						ord_Compare(NewLeft, NewRight, Flags, Precedence) != ord_SmallerThan()) {
							if (!MaxPara || clause_LiteralIsPredicate(PartnerLit)) {
								...
								if (clause_LiteralIsOrientedEquality(PartnerLit))
									...
									NewPartnerLeft = subst_Apply(PartnerSubst, term_Copy(
											term_FirstArgument(PartnerAtom)));
									NewPartnerRight = subst_Apply(PartnerSubst, term_Copy(
											term_SecondArgument(PartnerAtom)));
									switch (ord_Compare(NewPartnerLeft, NewPartnerRight, Flags,
											Precedence)) {
							...
							if (SupAtom != NULL)
								Result = list_Cons(inf_ApplyGenSuperposition(Clause, i, Subst,
										PartnerClause, pli, PartnerSubst, SupAtom, TRUE, OrdPara,
										MaxPara, Flags, Precedence), Result);
						}
						if (NewLeft != Left)
							term_Delete(NewLeft);
...
static LIST inf_GenSPRightEqToGiven(CLAUSE Clause, int i, BOOL Left, SHARED_INDEX ShIndex,
		BOOL OrdPara, BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
	...
	while (!stack_Empty(Bottom)) {
		...
		if (!term_IsVariable(Term)) {
			...
			for (; !list_Empty(TermList); TermList = list_Pop(TermList)) {
				...
				for (Parents = term_SupertermList(PartnerTerm); !list_Empty(Parents); Parents
						= list_Cdr(Parents)) {
					...
					if (fol_IsEquality(PartnerEq)) {
						...
						LIST Scl;
						int j;
						for (Scl = sharing_NAtomDataList(PartnerEq); !list_Empty(Scl); Scl
								= list_Cdr(Scl)) {
							...
							if (!clause_GetFlag(PartnerClause, CLAUSESELECT) && (!MaxPara
									...
									&& clause_LiteralIsPositive(PartnerLit) && clause_Number(
									PartnerClause) != clause_Number(Clause) && (!Unit
								...
								cont_Reset();
								if (!MaxPara || inf_LiteralsMax(Clause, i, Subst, PartnerClause, j,
										PartnerSubst, Flags, Precedence)) {
									TERM PartnerLeft, PartnerRight;
									BOOL Check, PartnerCheck;
									PartnerLeft = PartnerRight = NULL;
									PartnerCheck = Check = TRUE;
									if (OrdPara && !clause_LiteralIsOrientedEquality(PartnerLit)) {
										...
										PartnerLeft = subst_Apply(PartnerSubst, term_Copy(
												PartnerTerm));
										PartnerRight = subst_Apply(PartnerSubst, term_Copy(
												PartnerRight));
										PartnerCheck = (ord_Compare(PartnerLeft, PartnerRight,
												Flags, Precedence) != ord_SmallerThan());
									}
									if (PartnerCheck && MaxPara
...
#endif
										Result = list_Cons(inf_ApplyGenSuperposition(PartnerClause,
												j, PartnerSubst, Clause, i, Subst, SupAtom, TRUE,
												OrdPara, MaxPara, Flags, Precedence), Result);
									}
...
static LIST inf_GenSPRightLitToGiven(CLAUSE Clause, int i, TERM Atom, SHARED_INDEX ShIndex,
		BOOL OrdPara, BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   An unshared clause, the index of a succedent literal
 ...
 ***************************************************************/
{
	...
	while (!stack_Empty(Bottom)) {
		...
		if (!term_IsVariable(Term)) {
			...
			for (; !list_Empty(TermList); TermList = list_Pop(TermList)) {
				PartnerTerm = (TERM) list_Car(TermList);
				for (ParentList = term_SupertermList(PartnerTerm); !list_Empty(ParentList); ParentList
						= list_Cdr(ParentList)) {
					...
					if (fol_IsEquality(PartnerEq)) {
						...
						for (Scl = sharing_NAtomDataList(PartnerEq); !list_Empty(Scl); Scl
							...
							if (!clause_GetFlag(PartnerClause, CLAUSESELECT) && (!MaxPara
								...
								if (!MaxPara || inf_LiteralsMax(Clause, i, Subst, PartnerClause, j,
										PartnerSubst, Flags, Precedence)) {
									PartnerLeft = subst_Apply(PartnerSubst, term_Copy(PartnerTerm));
									if (PartnerTerm == term_FirstArgument(PartnerEq))
										PartnerRight = subst_Apply(PartnerSubst, term_Copy(
									...
									if (!OrdPara || clause_LiteralIsOrientedEquality(PartnerLit)
											|| ord_Compare(PartnerLeft, PartnerRight, Flags,
													Precedence) != ord_SmallerThan()) {
										SupAtom
												= inf_AllTermsRplac(Atom, Term, PartnerRight, Subst);
#ifdef CHECK
										if (SupAtom == NULL) {
											misc_StartErrorReport();
...
static LIST inf_GenSPRightToGiven(CLAUSE Clause, int i, SHARED_INDEX ShIndex, BOOL OrdPara,
		BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   An unshared clause, the index of a succedent literal
 ...
 ***************************************************************/
{
	...
	if (fol_IsEquality(Atom)) {
		Result = list_Nconc(inf_GenSPRightEqToGiven(Clause, i, TRUE, ShIndex, OrdPara, MaxPara,
				Unit, Flags, Precedence), Result);

		if (!MaxPara || !clause_LiteralIsOrientedEquality(clause_GetLiteral(Clause, i)))
			/* For SPm and OPm always try other direction, for SpR try it */
			/* only if the literal is not oriented.                       */
			Result = list_Nconc(inf_GenSPRightEqToGiven(Clause, i, FALSE, ShIndex, OrdPara,
					MaxPara, Unit, Flags, Precedence), Result);
	} else
		Result = list_Nconc(inf_GenSPRightLitToGiven(Clause, i, Atom, ShIndex, OrdPara, MaxPara,
				Unit, Flags, Precedence), Result);

	return Result;
}

LIST inf_GenSuperpositionRight(CLAUSE GivenClause, SHARED_INDEX ShIndex, BOOL OrdPara,
		BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (clause_GetFlag(GivenClause, CLAUSESELECT) || clause_HasEmptySuccedent(GivenClause)
			|| !clause_HasSolvedConstraint(GivenClause))
		return list_Nil();
	...
	for (i = clause_FirstSuccedentLitIndex(Copy); i <= n; i++) {
		...
		if (!MaxPara || clause_LiteralGetFlag(ActLit, STRICTMAXIMAL)) {
			if (fol_IsEquality(Atom) && (!Unit || clause_Length(GivenClause) == 1)) {

				Result = list_Nconc(
						inf_GenLitSPRight(Copy, term_FirstArgument(Atom),
								term_SecondArgument(Atom), i, ShIndex, OrdPara, MaxPara, Flags,
								Precedence), Result);

				if (!OrdPara || !clause_LiteralIsOrientedEquality(ActLit))
					Result = list_Nconc(inf_GenLitSPRight(Copy, term_SecondArgument(Atom),
							term_FirstArgument(Atom), i, ShIndex, OrdPara, MaxPara, Flags,
							Precedence), Result);
			}
			if (!clause_GetFlag(Copy, NOPARAINTO))
				Result = list_Nconc(inf_GenSPRightToGiven(Copy, i, ShIndex, OrdPara, MaxPara, Unit,
 ...
 ***************************************************************/
{
	...
	newClause = clause_CreateBody(clause_Length(C1) + clause_Length(C2) - 1, 0);
	...
	clause_SetNumOfConsLits(newClause, (clause_NumOfConsLits(C1) + clause_NumOfConsLits(C2)));
	clause_SetNumOfAnteLits(newClause, (clause_NumOfAnteLits(C1) + clause_NumOfAnteLits(C2)));
	clause_SetNumOfSuccLits(newClause, (clause_NumOfSuccLits(C1) - 1 + clause_NumOfSuccLits(C2)));

	for (m = clause_FirstLitIndex(); m <= lc; m++)
		clause_SetLiteral(newClause, m, clause_LiteralCreate(subst_Apply(rho, subst_Apply(tau,
	...
	for (; m <= la; m++)
		clause_SetLiteral(newClause, (m + help), clause_LiteralCreate(subst_Apply(rho, subst_Apply(
				tau, term_Copy(clause_GetLiteralTerm(C1, m)))), newClause));
	...
	for (; m <= ls; m++) {
		if (m != i)
			/* The literal used in the inference isn't copied */
			clause_SetLiteral(newClause, (m + help), clause_LiteralCreate(subst_Apply(rho,
					subst_Apply(tau, term_Copy(clause_GetLiteralTerm(C1, m)))), newClause));
		else
			/* the index has to be decreased to avoid an empty literal! */
			help--;
	}
	...
	for (m = clause_FirstLitIndex(); m <= plc; m++)
		clause_SetLiteral(newClause, (m + help), clause_LiteralCreate(subst_Apply(rho, subst_Apply(
				tau, term_Copy(clause_GetLiteralTerm(C2, m)))), newClause));
	...
	for (; m <= pls; m++) {
		...
		if (m == j) {
			...
			newAtom = term_Create(fol_Equality(), list_Cons(term_Copy(u_sigma), list_List(
					subst_Apply(rho, subst_Apply(tau, right)))));
		} else if (m == k) {
			...
			newAtom = subst_Apply(rho, subst_Apply(tau, term_Copy(clause_GetLiteralTerm(C2, m))));
		}

		clause_SetLiteral(newClause, (m + help), clause_LiteralCreate(newAtom, newClause));
	}
...
static LIST inf_Lit2MParamod(CLAUSE C1, CLAUSE C2, int i, int j, TERM s, TERM t, TERM s2, TERM v,
		TERM u_tau, SUBST tau, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
	...
	for (k = clause_FirstSuccedentLitIndex(C2); k <= last; k++) {
		...
		if (k != j && fol_IsEquality(partnerAtom2)) {
			...
			if (unify_UnifyCom(cont_LeftContext(), u_tau, pLeft2)) {
				...
				if (checkPassed && inf_LiteralsMaxWith2Subst(C1, i, C2, j, rho, tau, Flags,
						Precedence)) {
					v2_sigma = subst_Apply(rho, term_Copy(pRight2));
					result = list_Nconc(inf_ApplyMParamod(C1, C2, i, j, k, u_tau, v, s2, t,
							v2_sigma, tau, rho, Flags, Precedence), result);
...
static LIST inf_LitMParamod(CLAUSE Clause, int i, BOOL Turn, SHARED_INDEX ShIndex, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	unifiers = st_GetUnifier(cont_LeftContext(), sharing_Index(ShIndex), cont_RightContext(), s);
	...
	for (; !list_Empty(unifiers); unifiers = list_Pop(unifiers)) {
		...
		if (!term_IsVariable(s2) && !term_IsAtom(s2)) {
			for (literals = sharing_GetDataList(s2, ShIndex); !list_Empty(literals); literals
					= list_Pop(literals)) {
				...
				if (!clause_GetFlag(partnerClause, CLAUSESELECT) && clause_LiteralGetFlag(
						partnerLit, STRICTMAXIMAL) && clause_LiteralIsPositive(partnerLit) && /* succedent literal */
				clause_LiteralIsEquality(partnerLit) && clause_NumOfSuccLits(partnerClause) > 1 && /* > 1 pos. literals */
				clause_HasSolvedConstraint(partnerClause)) {
					...
					if (!clause_LiteralIsOrientedEquality(partnerLit) || inPartnerRight) {
						...
						if (inPartnerRight && ord_IsGreaterThan(ordResult)) {
							...
							result = list_Nconc(inf_Lit2MParamod(Clause, partnerClause, i, pli, s,
									t, s2, partnerRight, newPLeft, tau, Flags, Precedence), result);
						}
						if (ord_IsSmallerThan(ordResult) && (!inPartnerRight
								|| term_HasPointerSubterm(partnerLeft, s2))) {
							/* If s2 is not in partnerRight, it MUST be in partnerLeft,
							 else really do the test */
							/* Take a look at left side */
							result = list_Nconc(inf_Lit2MParamod(Clause, partnerClause, i, pli, s,
									t, s2, partnerLeft, newPRight, tau, Flags, Precedence), result);
						}
...
static LIST inf_MParamodLitToGiven(CLAUSE Clause, int j, BOOL Turn, SHARED_INDEX ShIndex,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause with a strict maximal equality literal at
 ...
 ***************************************************************/
{
	...
	while (!stack_Empty(bottom)) {
		...
		for (unifiers = st_GetUnifier(cont_LeftContext(), sharing_Index(ShIndex),
				cont_RightContext(), s2); !list_Empty(unifiers); unifiers = list_Pop(unifiers)) {
			TERM s = (TERM) list_Car(unifiers);

			for (superterms = term_SupertermList(s); !list_Empty(superterms); superterms
					= list_Cdr(superterms)) {
				TERM partnerAtom = (TERM) list_Car(superterms);

				if (fol_IsEquality(partnerAtom)) {
					for (literals = sharing_NAtomDataList(partnerAtom); !list_Empty(literals); literals
							= list_Cdr(literals)) {
						...
						if (!clause_GetFlag(partnerClause, CLAUSESELECT) && clause_LiteralGetFlag(
								partnerLit, STRICTMAXIMAL) && clause_LiteralIsPositive(partnerLit)
								&& (s == term_FirstArgument(partnerAtom)
										|| !clause_LiteralIsOrientedEquality(partnerLit))
								&& clause_HasSolvedConstraint(partnerClause) && clause_Number(
								partnerClause) != clause_Number(Clause)) {
							...
							if (!clause_LiteralIsOrientedEquality(actLit)) {
								...
								if (ord_Compare(u_tau, v_tau, Flags, Precedence)
										!= ord_GreaterThan())
									checkPassed = FALSE;
							}
							...
							if (checkPassed) {
								...
								result = list_Nconc(inf_Lit2MParamod(partnerClause, Clause, i, j,
										s, t, s2, v, u_tau, tau, Flags, Precedence), result);
							}
...
LIST inf_MergingParamodulation(CLAUSE GivenClause, SHARED_INDEX ShIndex, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(copy); i <= last; i++) {
		...
		if (clause_LiteralGetFlag(actLit, STRICTMAXIMAL) && fol_IsEquality(atom)) {

			result
					= list_Nconc(inf_LitMParamod(copy, i, FALSE, ShIndex, Flags, Precedence),
							result);
			/* Assume GivenClause is the second clause of the rule */
			result = list_Nconc(inf_MParamodLitToGiven(copy, i, FALSE, ShIndex, Flags, Precedence),
					result);

			if (!clause_LiteralIsOrientedEquality(actLit)) {
				/* First check rule with left and right side exchanged */
				result = list_Nconc(inf_LitMParamod(copy, i, TRUE, ShIndex, Flags, Precedence),
						result);
				/* Now assume GivenClause is the second clause of the rule */
				/* Check with sides exchanged */
				result = list_Nconc(inf_MParamodLitToGiven(copy, i, TRUE, ShIndex, Flags,
						Precedence), result);
			}
		}
	} /* for */
...
// FIXME - sua luat Justified Resolution tai day
// Literal trong model gui sang nam o PartnerClause
 ...
 ***************************************************************/
{
	...
	LITERAL new_literal = clause_LiteralCopy(NegLit);
	TERM new_term = subst_Apply(SubstPartnerTermS, clause_LiteralSignedAtom(new_literal));
	printf("       Term after subst: "); term_Print(new_term); printf("\n");
	...
	NewClause = clause_CreateBody((clause_Length(GivenClause) - 1) + clause_Length(PartnerClause) - 1);

#endif


	clause_SetNumOfConsLits(NewClause, (clause_NumOfConsLits(GivenClause) + (clause_NumOfConsLits(
			PartnerClause) - ConNeg)));

	clause_SetNumOfAnteLits(NewClause, (clause_NumOfAnteLits(GivenClause) + (clause_NumOfAnteLits(
			PartnerClause) - AntNeg)));

	clause_SetNumOfSuccLits(NewClause, ((clause_NumOfSuccLits(GivenClause) - 1)
			+ clause_NumOfSuccLits(PartnerClause)));

	/* First set the literals from the GivenClause : */
	for (j = clause_FirstLitIndex(); j <= lc; j++) {
		clause_SetLiteral(NewClause, j, clause_LiteralCreate(subst_Apply(SubstTermS, term_Copy(
				clause_GetLiteralTerm(GivenClause, j))), NewClause));
	}
	...
	for (; j <= la; j++) {
		clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(subst_Apply(SubstTermS,
				term_Copy(clause_GetLiteralTerm(GivenClause, j))), NewClause));
	}
	...
	for (j = clause_FirstLitIndex(); j <= plc; j++) {
		if (j != pi) {
			clause_SetLiteral(NewClause, (j + help), clause_LiteralCreate(subst_Apply(
					SubstPartnerTermS, term_Copy(clause_GetLiteralTerm(PartnerClause, j))),
					NewClause));
		} else {
			help--;
		}
	...
	if (clause_LiteralIsFromDpllModel(new_literal)) {
		...
		for (LIST new_scan = NewJustification; !list_Empty(new_scan); new_scan = list_Cdr(new_scan)) {
			LITERAL justi_lit = list_Car(new_scan);
			LITERAL neg_justi_lit;
			if (clause_LiteralIsNegative(justi_lit)) {
				TERM neg_justi_term = term_Copy(term_FirstArgument(clause_LiteralSignedAtom(justi_lit)));
				neg_justi_lit = clause_LiteralCreate(neg_justi_term, NewClause);
			} else {
				TERM justi_term = term_Copy(clause_LiteralSignedAtom(justi_lit));
				neg_justi_lit = clause_LiteralCreateNegative(justi_term, NewClause);
			}
			clause_LiteralDelete(justi_lit);
			list_Rplaca(new_scan, neg_justi_lit);
		}
	...
	// neu literal lay ra de thuc hien resolution ko phai tu DPLL model
	// thi thuc hien general resolution
	// TTQ_NOTE - RULE GENERAL RESOLUTION tai day
	else {
		// set justification for clause
		clause_SetJustifiedLiterals(NewClause, NewJustification);
	}
...
LIST inf_UnitResolution(CLAUSE GivenClause, SHARED_INDEX ShIndex, BOOL Equations, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause and an Index, usually the WorkedOffIndex,
							...
							(clause_LiteralGetFlag(PartnerLit, LITSELECT) || !clause_GetFlag(
								...
								if (clause_LiteralIsNegative(PartnerLit))
									Result = list_Cons(inf_ApplyGenRes(ActLit, PartnerLit, Subst,
											PartnerSubst, Flags, Precedence), Result);
								else
									Result = list_Cons(inf_ApplyGenRes(PartnerLit, ActLit,
											PartnerSubst, Subst, Flags, Precedence), Result);
								subst_Delete(Subst);
...
LIST inf_BoundedDepthUnitResolution(CLAUSE GivenClause, SHARED_INDEX ShIndex, BOOL ConClause,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause and an Index, usually the WorkedOffIndex,
			...
			for (; !list_Empty(TermList); TermList = list_Pop(TermList)) {
				...
				if (!term_IsVariable(PartnerAtom)) {
					...
					for (LitList = sharing_NAtomDataList(PartnerAtom); !list_Empty(LitList); LitList
						...
						if (clause_LiteralsAreComplementary(PartnerLit, ActLit) && (clause_Length(
								GivenCopy) == 1 || clause_Length(PartnerClause) == 1)
								&& (clause_GetFlag(GivenCopy, CONCLAUSE) || clause_GetFlag(
										PartnerClause, CONCLAUSE)) && (!ConClause
...
static CLAUSE inf_ApplyGeneralFactoring(CLAUSE Clause, NAT i, NAT j, SUBST Subst, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
...
LIST inf_GeneralFactoring(CLAUSE GivenClause, BOOL Ordered, BOOL Left, BOOL Equations,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (!clause_GetFlag(GivenClause, CLAUSESELECT)) {
		for (i = clause_FirstSuccedentLitIndex(GivenClause); i <= last; i++) {
			ActLit = clause_GetLiteral(GivenClause, i);
			if ((!Ordered || clause_LiteralIsMaximal(ActLit)) && (Equations
					...
					if (i != j) {
						...
						if ((j > i || (Ordered && !clause_LiteralIsMaximal(PartnerLit)))
							...
							if (unify_UnifyCom(cont_LeftContext(), Atom, PartnerAtom)) {
								SUBST mgu;
								subst_ExtractUnifierCom(cont_LeftContext(), &mgu);
								if (!Ordered || inf_LitMax(GivenClause, i, j, mgu, FALSE, Flags,
										Precedence))
									Result = list_Cons(inf_ApplyGeneralFactoring(GivenClause, i, j,
											mgu, Flags, Precedence), Result);
								subst_Delete(mgu);
							}
							cont_Reset();
							if (fol_IsEquality(Atom) && /* PartnerAtom is equality, too */
							unify_UnifyCom(cont_LeftContext(), term_SecondArgument(Atom),
									term_FirstArgument(PartnerAtom)) && unify_UnifyCom(
	...
	if (Left) {
		...
		for (i = clause_FirstAntecedentLitIndex(GivenClause); i <= last; i++) {
			ActLit = clause_GetLiteral(GivenClause, i);
			if ((Equations || !clause_LiteralIsEquality(ActLit)) && (clause_LiteralGetFlag(ActLit,
					LITSELECT) || (!clause_GetFlag(GivenClause, CLAUSESELECT) && (!Ordered
					|| clause_LiteralIsMaximal(ActLit))))) {
				TERM Atom, PartnerAtom;
				LITERAL PartnerLit;
				Atom = clause_LiteralAtom(ActLit);
				for (j = clause_FirstAntecedentLitIndex(GivenClause); j <= last; j++) {
					if (i != j) {
						...
						if ((j > i || clause_LiteralGetFlag(ActLit, LITSELECT) || (Ordered
								&& !clause_LiteralIsMaximal(PartnerLit))) && term_EqualTopSymbols(
								Atom, PartnerAtom)) {
							PartnerAtom = clause_LiteralAtom(PartnerLit);
							cont_Check();
							if (unify_UnifyCom(cont_LeftContext(), Atom, PartnerAtom)) {
								...
								if (!Ordered || clause_LiteralGetFlag(ActLit, LITSELECT)
										|| inf_LitMax(GivenClause, i, j, mgu, FALSE, Flags,
												Precedence))
									Result = list_Cons(inf_ApplyGeneralFactoring(GivenClause, i, j,
											mgu, Flags, Precedence), Result);
								subst_Delete(mgu);
...
static LIST inf_GenLitSPLeft(CLAUSE Clause, TERM Left, TERM Right, int i, SHARED_INDEX ShIndex,
		BOOL OrdPara, BOOL MaxPara, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause (unshared) with a positive equality literal
	...
	for (; !list_Empty(Terms); Terms = list_Pop(Terms)) {
		...
		if (!term_IsVariable(Term) && !symbol_IsPredicate(term_TopSymbol(Term))) {
			...
			for (; !list_Empty(Lits); Lits = list_Pop(Lits)) {
				...
				if ((clause_LiteralGetFlag(PartnerLit, LITSELECT) || (!clause_GetFlag(
						PartnerClause, CLAUSESELECT) && (!MaxPara || clause_LiteralIsMaximal(
						PartnerLit)))) && clause_LiteralIsNegative(PartnerLit) && !clause_GetFlag(
						PartnerClause, NOPARAINTO) && clause_HasSolvedConstraint(PartnerClause)) {
					...
					if (!MaxPara || inf_LiteralsMax(Clause, i, Subst, PartnerClause, pli,
						...
						if (OrdPara && !clause_LiteralIsOrientedEquality(clause_GetLiteral(Clause,
								i)))
							NewLeft = subst_Apply(Subst, term_Copy(Left));
						if (!OrdPara || NewLeft == Left || ord_Compare(NewLeft, NewRight, Flags,
								Precedence) != ord_SmallerThan()) {
							if (!MaxPara || clause_LiteralIsPredicate(PartnerLit)) {
								SupAtom = inf_AllTermsRplac(PartnerAtom, Term, NewRight,
										PartnerSubst);
							...
							if (SupAtom != NULL)
								Result = list_Cons(inf_ApplyGenSuperposition(Clause, i, Subst,
										PartnerClause, pli, PartnerSubst, SupAtom, FALSE, OrdPara,
										MaxPara, Flags, Precedence), Result);
						}
						if (NewLeft != Left)
							term_Delete(NewLeft);
...
static LIST inf_GenSPLeftEqToGiven(CLAUSE Clause, int i, BOOL Left, SHARED_INDEX ShIndex,
		BOOL OrdPara, BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
...
#endif
										Result = list_Cons(inf_ApplyGenSuperposition(PartnerClause,
												j, PartnerSubst, Clause, i, Subst, SupAtom, FALSE,
												OrdPara, MaxPara, Flags, Precedence), Result);
									}
									if (PartnerLeft != term_Null())
										term_Delete(PartnerLeft);
...
static LIST inf_GenSPLeftLitToGiven(CLAUSE Clause, int i, TERM Atom, SHARED_INDEX ShIndex,
		BOOL OrdPara, BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   An unshared clause, the index of an antecedent
				...
				for (ParentList = term_SupertermList(PartnerTerm); !list_Empty(ParentList); ParentList
					...
					if (fol_IsEquality(PartnerEq)) {
						...
						for (Scl = sharing_NAtomDataList(PartnerEq); !list_Empty(Scl); Scl
							...
							if (!clause_GetFlag(PartnerClause, CLAUSESELECT) && (!MaxPara
									|| clause_LiteralGetFlag(PartnerLit, STRICTMAXIMAL))
									&& (!OrdPara || PartnerTerm == term_FirstArgument(PartnerAtom)
											|| !clause_LiteralIsOrientedEquality(PartnerLit))
									&& clause_LiteralIsPositive(PartnerLit) && clause_Number(
									PartnerClause) != clause_Number(Clause) && (!Unit
									|| clause_Length(PartnerClause) == 1)
...
static LIST inf_GenSPLeftToGiven(CLAUSE Clause, int i, SHARED_INDEX ShIndex, BOOL OrdPara,
		BOOL MaxPara, BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   An unshared clause, the index of an antecedent
 ...
 ***************************************************************/
{
	...
	if (fol_IsEquality(Atom)) {
		Result = list_Nconc(inf_GenSPLeftEqToGiven(Clause, i, TRUE, ShIndex, OrdPara, MaxPara,
				Unit, Flags, Precedence), Result);
		if (!MaxPara || !clause_LiteralIsOrientedEquality(clause_GetLiteral(Clause, i)))
			/* For SPm and OPm always try other direction, for SpR try it */
			/* only if the literal is not oriented.                       */
			Result = list_Nconc(inf_GenSPLeftEqToGiven(Clause, i, FALSE, ShIndex, OrdPara, MaxPara,
					Unit, Flags, Precedence), Result);
	} else
		Result = list_Nconc(inf_GenSPLeftLitToGiven(Clause, i, Atom, ShIndex, OrdPara, MaxPara,
				Unit, Flags, Precedence), Result);

	return Result;
}

LIST inf_GenSuperpositionLeft(CLAUSE GivenClause, SHARED_INDEX ShIndex, BOOL OrdPara, BOOL MaxPara,
		BOOL Unit, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause and an Index, usually the WorkedOffIndex,
 two boolean flags for controlling inference
 preconditions, a flag store and a precedence.
 ...
 ***************************************************************/
{
	...
	if (!clause_GetFlag(Copy, CLAUSESELECT) && (!Unit || clause_Length(Copy) == 1)) {
		for (i = clause_FirstSuccedentLitIndex(Copy); i <= n; i++) {
			ActLit = clause_GetLiteral(Copy, i);
			Atom = clause_LiteralSignedAtom(ActLit);

			if (fol_IsEquality(Atom) && (!MaxPara || clause_LiteralGetFlag(ActLit, STRICTMAXIMAL))) {

				Result = list_Nconc(
						inf_GenLitSPLeft(Copy, term_FirstArgument(Atom), term_SecondArgument(Atom),
								i, ShIndex, OrdPara, MaxPara, Flags, Precedence), Result);
				if (!OrdPara || !clause_LiteralIsOrientedEquality(ActLit))
					/* For SPm always try the other direction, for OPm and SpL */
					/* only try it if the literal is not oriented.             */
					Result = list_Nconc(inf_GenLitSPLeft(Copy, term_SecondArgument(Atom),
							term_FirstArgument(Atom), i, ShIndex, OrdPara, MaxPara, Flags,
							Precedence), Result);
			}
		}
	...
	if (!clause_GetFlag(Copy, NOPARAINTO)) {
		for (i = clause_FirstAntecedentLitIndex(Copy); i <= n; i++) {
			...
			if (clause_LiteralGetFlag(ActLit, LITSELECT) || (!clause_GetFlag(Copy, CLAUSESELECT)
					&& (!MaxPara || clause_LiteralIsMaximal(ActLit))))
				Result = list_Nconc(inf_GenSPLeftToGiven(Copy, i, ShIndex, OrdPara, MaxPara, Unit,
						Flags, Precedence), Result);
		}
	}
	clause_Delete(Copy);
...
LIST inf_ApplyDefinition(PROOFSEARCH Search, CLAUSE Clause, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (Defs = prfs_Definitions(Search); !list_Empty(Defs); Defs = list_Cdr(Defs)) {
		...
		Result = list_Nconc(def_ApplyDefToClauseOnce(Def, Clause, Flags, Precedence), Result);
	}
	return Result;
}
...
static void inf_CopyHyperElectron(CLAUSE Clause, SUBST Subst2, SUBST Subst1, int PLitInd,
		LIST* Constraint, LIST* Succedent)
/**************************************************************
		...
		if (j != PLitInd) {
			...
			if (j <= lc)
				*Constraint = list_Cons(Atom, *Constraint);
				...
				/* Literal must be from succedent */
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstConstraintLitIndex(Nucleus); i <= bound; i++)
		Constraint = list_Cons(subst_Apply(Subst, term_Copy(clause_GetLiteralAtom(Nucleus, i))),
				Constraint);
	...
	for (i = clause_FirstSuccedentLitIndex(Nucleus); i <= bound; i++)
		Succedent = list_Cons(subst_Apply(Subst, term_Copy(clause_GetLiteralAtom(Nucleus, i))),
				Succedent);
	...
	for (i = clause_FirstAntecedentLitIndex(Nucleus); i <= bound; i++) {
		...
		for (Scan = FoundMap, MapItem = NULL; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		inf_CopyHyperElectron(NewClause, Subst, ESubst, clause_LiteralGetIndex(Lit), &Constraint,
				&Succedent);
	}
...
static LIST inf_GetHyperResolutionPartnerLits(TERM Atom, SHARED_INDEX Index, BOOL StrictlyMaximal)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	Result = list_Nil();
	TermList = st_GetUnifier(cont_LeftContext(), sharing_Index(Index), cont_RightContext(), Atom);

	for (; !list_Empty(TermList); TermList = list_Pop(TermList)) {
		if (!term_IsVariable(list_Car(TermList))) {
			for (LitScan = sharing_NAtomDataList(list_Car(TermList)); !list_Empty(LitScan); LitScan
					= list_Cdr(LitScan)) {
				NextLit = list_Car(LitScan);
				Clause = clause_LiteralOwningClause(NextLit);
				if (clause_LiteralIsFromSuccedent(NextLit) && (!StrictlyMaximal
						|| clause_LiteralGetFlag(NextLit, STRICTMAXIMAL))
						&& clause_HasSolvedConstraint(Clause) && clause_HasEmptyAntecedent(Clause))
...
static LIST inf_HyperResolvents(CLAUSE Clause, SUBST Subst, LIST Restlits, int GlobalMaxVar,
		LIST FoundMap, BOOL StrictlyMaximal, SHARED_INDEX Index, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
	...
	if (list_Empty(Restlits)) {
		...
		if (StrictlyMaximal) { /* only for ordered hyper resolution */
			for (Scan = FoundMap; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
				MapItem = list_Car(Scan);
				Lit = MapItem->ElectronLit;
				if (!inf_LitMaxWith2Subst(clause_LiteralOwningClause(Lit), clause_LiteralGetIndex(
						Lit), -1, Subst, MapItem->ElectronSubst, TRUE, Flags, Precedence))
					return list_Nil();
			}
		}
		/* Build the resolvent */
		return list_List(inf_BuildHyperResolvent(Clause, Subst, FoundMap, StrictlyMaximal, Flags,
				Precedence));
	} else {
		CLAUSE PartnerCopy;
		...
		Result = list_Nil();
		Restlits = clause_MoveBestLiteralToFront(list_Copy(Restlits), Subst, GlobalMaxVar,
		...
		/* The 'endless' loop may run twice for equations, once for other atoms */
		while (TRUE) {
			NextLits = inf_GetHyperResolutionPartnerLits(AtomCopy, Index, StrictlyMaximal);
			...
			for (; !list_Empty(NextLits); NextLits = list_Pop(NextLits)) {
				...
				NewMaxVar = symbol_GreaterVariable(GlobalMaxVar, NewMaxVar) ? GlobalMaxVar
						: NewMaxVar;
				...
				subst_ExtractUnifier(cont_LeftContext(), &NewSubst, cont_RightContext(),
						&RightSubst);
				cont_Reset();
				...
				Result = list_Nconc(inf_HyperResolvents(Clause, NewSubst, Restlits, NewMaxVar,
						FoundMap, StrictlyMaximal, Index, Flags, Precedence), Result);
 ...
 ***************************************************************/
{
	...
	Result = list_Nconc(inf_HyperResolvents(GivenClause, subst_Nil(), RestLits, clause_MaxVar(
			GivenClause), list_Nil(), StrictlyMaximal, Index, Flags, Precedence), Result);
	list_Delete(RestLits);

	return Result;
}

static LIST inf_BackwardHyperResolution(CLAUSE Electron, SHARED_INDEX Index, BOOL StrictlyMaximal,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (!clause_HasEmptyAntecedent(Electron) || clause_HasEmptySuccedent(Electron))
		return list_Nil();
					...
					if (!term_IsVariable(list_Car(CandAtoms))) {
						...
						for (; !list_Empty(CandLits); CandLits = list_Cdr(CandLits)) {
							...
							if (clause_LiteralIsFromAntecedent(NucLit)
								...
								GlobalMaxVar
										= symbol_GreaterVariable(GlobalMaxVar, MaxVar) ? GlobalMaxVar
												: MaxVar;
								...
								Result = list_Nconc(inf_HyperResolvents(Nucleus, LeftSubst,
										RestLits, GlobalMaxVar, FoundMap, StrictlyMaximal, Index,
										Flags, Precedence), Result);
...
LIST inf_GeneralHyperResolution(CLAUSE GivenClause, SHARED_INDEX Index, BOOL Ordered,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	Result = list_Nil();
	if (clause_HasSolvedConstraint(GivenClause)) {
		Result = inf_ForwardHyperResolution(GivenClause, Index, Ordered, Flags, Precedence);
		Result = list_Nconc(inf_BackwardHyperResolution(GivenClause, Index, Ordered, Flags,
				Precedence), Result);
	}
	return Result;
}
	...
	if (Dynamic && !clause_HasSolvedConstraint(GivenClause)) {
		...
		if (clause_HasTermSortConstraintLits(GivenClause)) {
			if (flag_GetFlagValue(Flags, flag_ISOR))
				ListOfDerivedClauses = list_Nconc(inf_ForwardSortResolution(GivenClause,
						sort_TheoryIndex(Dynamic), Dynamic, FALSE, Flags, Precedence),
						ListOfDerivedClauses);
		} else if (flag_GetFlagValue(Flags, flag_IEMS))
			ListOfDerivedClauses = list_Nconc(inf_ForwardEmptySort(GivenClause, sort_TheoryIndex(
					Dynamic), Dynamic, FALSE, Flags, Precedence), ListOfDerivedClauses);
		...
		if (Dynamic && flag_GetFlagValue(Flags, flag_IEMS))
			ListOfDerivedClauses = list_Nconc(inf_BackwardEmptySort(GivenClause, sharing_Index(
					ShIndex), Dynamic, FALSE, Flags, Precedence), ListOfDerivedClauses);

		if (Dynamic && flag_GetFlagValue(Flags, flag_ISOR))
			ListOfDerivedClauses = list_Nconc(inf_BackwardSortResolution(GivenClause,
					sharing_Index(ShIndex), Dynamic, FALSE, Flags, Precedence),
					ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IEQR))
			ListOfDerivedClauses = list_Nconc(inf_EqualityResolution(GivenClause, TRUE, Flags,
					Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IERR))
			ListOfDerivedClauses = list_Nconc(inf_EqualityResolution(GivenClause, FALSE, Flags,
					Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IMPM))
			ListOfDerivedClauses = list_Nconc(inf_MergingParamodulation(GivenClause, ShIndex,
					Flags, Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IEQF))
			ListOfDerivedClauses = list_Nconc(
					inf_EqualityFactoring(GivenClause, Flags, Precedence), ListOfDerivedClauses);

		switch (flag_GetFlagValue(Flags, flag_IOFC)) {
		case flag_FACTORINGOFF:
			break; /* Do nothing */
		case flag_FACTORINGONLYRIGHT:
			ListOfDerivedClauses = list_Nconc(inf_GeneralFactoring(GivenClause, TRUE, FALSE, TRUE,
					Flags, Precedence), ListOfDerivedClauses);
			break;
		case flag_FACTORINGRIGHTANDLEFT:
			ListOfDerivedClauses = list_Nconc(inf_GeneralFactoring(GivenClause, TRUE, TRUE, TRUE,
					Flags, Precedence), ListOfDerivedClauses);
			break;
		default:
		...
		if (flag_GetFlagValue(Flags, flag_ISFC))
			ListOfDerivedClauses = list_Nconc(inf_GeneralFactoring(GivenClause, FALSE, TRUE, TRUE,
					Flags, Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_ISPR))
			ListOfDerivedClauses = list_Nconc(inf_SuperpositionRight(GivenClause, ShIndex, Flags,
					Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_ISPM))
			ListOfDerivedClauses = list_Nconc(inf_Paramodulation(GivenClause, ShIndex, Flags,
					Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IOPM))
			ListOfDerivedClauses = list_Nconc(inf_OrderedParamodulation(GivenClause, ShIndex,
					Flags, Precedence), ListOfDerivedClauses);
		...
		case flag_ORDEREDRESOLUTIONNOEQUATIONS: /* no equations */
			ListOfDerivedClauses = list_Nconc(inf_GeneralResolution(GivenClause, ShIndex, TRUE,
					FALSE, Flags, Precedence), ListOfDerivedClauses);
			break;

		case flag_ORDEREDRESOLUTIONWITHEQUATIONS: /* allow equations */
			ListOfDerivedClauses = list_Nconc(inf_GeneralResolution(GivenClause, ShIndex, TRUE,
					TRUE, Flags, Precedence), ListOfDerivedClauses);
			break;
		default:
			misc_StartUserErrorReport();
		...
		switch (flag_GetFlagValue(Flags, flag_ISRE)) { // Su dung Standard Resolution
		case flag_STANDARDRESOLUTIONOFF:
			break; /* Do nothing */
		case flag_STANDARDRESOLUTIONNOEQUATIONS: /* no equations */
			ListOfDerivedClauses = list_Nconc(inf_GeneralResolution(GivenClause, ShIndex, FALSE,
					FALSE, Flags, Precedence), ListOfDerivedClauses);
			break;
		...
		if (flag_GetFlagValue(Flags, flag_IUNR))
			ListOfDerivedClauses = list_Nconc(inf_UnitResolution(GivenClause, ShIndex, FALSE,
					Flags, Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IBUR))
			ListOfDerivedClauses = list_Nconc(inf_BoundedDepthUnitResolution(GivenClause, ShIndex,
					FALSE, Flags, Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_ISHY))
			ListOfDerivedClauses = list_Nconc(inf_GeneralHyperResolution(GivenClause, ShIndex,
					FALSE, Flags, Precedence), ListOfDerivedClauses);
		...
		if (flag_GetFlagValue(Flags, flag_IURR))
			ListOfDerivedClauses = list_Nconc(inf_URResolution(GivenClause, ShIndex, Flags,
					Precedence), ListOfDerivedClauses);

		if (flag_GetFlagValue(Flags, flag_IDEF))
			ListOfDerivedClauses = list_Nconc(inf_ApplyDefinition(Search, GivenClause, Flags,
					Precedence), ListOfDerivedClauses);
	}
