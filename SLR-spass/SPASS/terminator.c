 ...
 ***************************************************************/
{
	...
	for (; !list_Empty(IndexList); IndexList = list_Cdr(IndexList)) {
		Unifiers = st_GetUnifier(cont_LeftContext(), sharing_Index(list_Car(IndexList)),
		...
		for (; !list_Empty(Unifiers); Unifiers = list_Pop(Unifiers)) {
			if (!term_IsVariable(list_Car(Unifiers))) {
				for (LitScan = sharing_NAtomDataList(list_Car(Unifiers)); !list_Empty(LitScan); LitScan
					...
					if (clause_LiteralsAreComplementary(Lit, NextLit) && (!UnitsOnly
							|| clause_Length(clause_LiteralOwningClause(NextLit)) == 1))
 ...
 ***************************************************************/
{
	...
	Result = clause_Create(list_Nil(), list_Nil(), list_Nil(), list_Nil(), Flags, Precedence);
 ...
 ***************************************************************/
{
	...
	(clause_LiteralIsPositive(L1) && S1 > S2) || (clause_LiteralIsNegative(L2) && S1 > S2))
		return TRUE;
	else
		return FALSE;
}

static CLAUSE red_SearchTerminator(NAT n, LIST RestLits, LIST FoundMap, SUBST Subst,
		SYMBOL GlobalMaxVar, LIST IndexList, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	if (list_Empty(RestLits)) {
		...
		Swapped = FALSE;
		Result = clause_Null();
		clause_MoveBestLiteralToFront(RestLits, Subst, GlobalMaxVar, red_TerminatorLitIsBetter);
		...
		while (TRUE) {
			...
			for (; !list_Empty(ClashList) && Result == NULL; ClashList = list_Pop(ClashList)) {
				...
				Result = red_SearchTerminator(Limit, ToDoList, FoundMap, NewSubst, NewMaxVar,
						IndexList, Flags, Precedence);
...
CLAUSE red_Terminator(CLAUSE RedClause, NAT n, SHARED_INDEX WoIndex, SHARED_INDEX UsIndex,
 ...
 ***************************************************************/
{
	...
	Result = red_SearchTerminator(n, Rest, list_Nil(), subst_Nil(), clause_MaxVar(RedClause),
			IndexList, Flags, Precedence);
