...
/* State definitions for flag_QUANTEXCH */
typedef enum { flag_QUANTEXCHMIN = -1,
	       flag_QUANTEXCHOFF = flag_OFF,
	       flag_QUANTEXCHON  = flag_ON,
	       flag_QUANTEXCHMAX 
} FLAG_QUANTEXCHTYPE;
...
typedef enum { flag_AUTO, flag_STDIN, flag_INTERACTIVE, flag_FLOTTER,
	       flag_SOS,
               ...
               flag_SPLITS,     flag_SPLITHEURISTIC,
	       ...
	       flag_EML,	flag_EMLAUTO,    flag_EML2REL,
	       flag_EMLTR,      flag_EMLTHEORY,  flag_EMLFUNCNDEQ,
...
void        flag_FPrintFlagWithValue(FILE*, FLAGSTORE, FLAG_ID, int);
...
int         flag_GetFlagValue(FLAGSTORE, FLAG_ID);
void        flag_SetFlagValue(FLAGSTORE, FLAG_ID, int);
...
void        flag_PrintEmlTranslationMethod(FLAG);
void        flag_CheckEmlTranslationFlags(FLAGSTORE, FLAG);
...
***************************************************************/
{
  ...
  for (i = (FLAG_ID) 0; i < flag_MAXFLAG; i++) {
    if (flag_IsTransforming(i) && flag_GetFlagValue(Store, i))
      printf("%s=%d ",flag_Name(i), flag_GetFlagValue(Store,i));
  }
} 
...
***************************************************************/
{
  ...
  for (i = (FLAG_ID) 0; i < flag_MAXFLAG; i++) {
    if (flag_IsEmlSpecial(i) && flag_GetFlagValue(Store, i))
      printf("%s=%d ",flag_Name(i), flag_GetFlagValue(Store,i));
  }
} 
