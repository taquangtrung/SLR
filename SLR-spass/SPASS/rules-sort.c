...
static BOOL inf_SubsortPrecheck(CLAUSE Clause, LIST TLits, LITERAL Special, st_INDEX Index,
		SORTTHEORY SortTheory)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	unifiers = inf_GetForwardPartnerLits(clause_GetLiteral(Clause, (int) list_Car(TLits)), Index);
	...
	for (; !list_Empty(TLits); TLits = list_Cdr(TLits)) {
		...
		tSort = sort_Intersect(sort_TheorySortOfSymbol(SortTheory, term_TopSymbol(actAtom)), tSort);
	}
	...
	for (; !list_Empty(TermList); TermList = list_Pop(TermList)) {
		...
		if (term_IsAtom(list_Car(TermList))) {
			...
			for (LitScan = sharing_NAtomDataList(list_Car(TermList)); !list_Empty(LitScan); LitScan
				...
				if (clause_LiteralIsPositive(NextLit) && clause_LiteralGetFlag(NextLit,
						STRICTMAXIMAL) && clause_GetFlag(Clause, WORKEDOFF)
						&& clause_HasSolvedConstraint(Clause) && !list_PointerMember(Result,
						NextLit))
 ...
 ***************************************************************/
{
	...
	for (Scan = Foundlits; !list_Empty(Scan); Scan = list_Cdr(Scan))
		depth = misc_Max(depth, clause_Depth(clause_LiteralOwningClause(list_Car(Scan))));

	ClauseCopy = clause_Copy(Clause);
	Partners = list_Cons(ClauseCopy, Partners);
	clause_SubstApply(Subst, ClauseCopy);

	IsFromEmptySort = term_IsVariable(term_FirstArgument(clause_GetLiteralAtom(Clause,
			(int) list_Car(Lits))));
	...
	for (i = clause_FirstLitIndex(); i <= bound; i++) {
		...
		if (list_PointerMember(Lits, (POINTER) i)) {
			...
			for (Scan = Foundlits; term_TopSymbol(Atom) != term_TopSymbol(clause_LiteralAtom(
					list_Car(Scan))); Scan = list_Cdr(Scan))
				;
			PLit = list_Car(Scan);
			...
			for (j = clause_FirstLitIndex(); j < n; j++) {
				if (j <= lc)
					Constraint = list_Cons(subst_Apply(RightSubst, term_Copy(clause_GetLiteralAtom(
							PartnerCopy, j))), Constraint);
				else if (j <= la)
					Antecedent = list_Cons(subst_Apply(RightSubst, term_Copy(clause_GetLiteralAtom(
							PartnerCopy, j))), Antecedent);
				else if (j != PLitInd)
					Succedent = list_Cons(subst_Apply(RightSubst, term_Copy(clause_GetLiteralAtom(
							PartnerCopy, j))), Succedent);
			}
			...
			for (j = clause_FirstLitIndex(); j < n; j++) {
				if (j <= lc)
					...
					Succedent
							= list_Cons(term_Copy(clause_GetLiteralAtom(NewClause, j)), Succedent);
			}
...
static LIST inf_ConstraintHyperResolvents(CLAUSE Clause, LIST Lits, SUBST Subst, LIST Restlits,
		LIST Foundlits, st_INDEX Index, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A <Clause> where the sort constraint is resolved,
 a list <Lits> of constraint indices in <Clause> where
 all corresponding constraints have the same term,
 ...
 ***************************************************************/
{
	if (list_Empty(Restlits))
		return list_List(inf_BuildConstraintHyperResolvent(Clause, Lits, Subst, Foundlits, Flags,
				Precedence));
	else {
		CLAUSE PartnerCopy;
		LITERAL Lit, PLit;
		...
		for (; !list_Empty(NextLits); NextLits = list_Pop(NextLits)) {
			...
			cont_Check();
			unify_UnifyNoOC(cont_LeftContext(), AtomCopy, cont_RightContext(), clause_LiteralAtom(
					PLit));
			...
			Result = list_Nconc(inf_ConstraintHyperResolvents(Clause, Lits, NewSubst, list_Cdr(
					Restlits), Foundlits, Index, Flags, Precedence), Result);
			subst_Delete(NewSubst);
...
LIST inf_BackwardSortResolution(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		BOOL Precheck, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(GivenClause); i <= ls; i++) {
		...
		if (clause_LiteralGetFlag(pLit, STRICTMAXIMAL) && clause_LiteralIsSort(pLit)) {
			LIST termList;
			termList = st_GetUnifier(cont_LeftContext(), Index, cont_RightContext(), pAtom);

			for (; !list_Empty(termList); termList = list_Pop(termList)) {
				if (term_IsAtom(list_Car(termList)) && !term_IsVariable(term_FirstArgument(
						list_Car(termList)))) {
					...
					for (; !list_Empty(litScan); litScan = list_Cdr(litScan)) {
						...
						if (clause_LiteralGetIndex(gLit) < clause_FirstAntecedentLitIndex(gClause)
							...
							for (j = clause_FirstLitIndex(); j <= lc; j++) {
								...
								if (j != gi && term_FirstArgument(clause_LiteralAtom(tCand))
							...
							if (!Precheck || inf_SubsortPrecheck(gClause, tLits, pLit, Index,
									SortTheory)) {
								CLAUSE pClauseCopy;
								SYMBOL minVar;
								LIST foundLits;
								...
								cont_Check();
								unify_UnifyNoOC(cont_LeftContext(), gAtom, cont_RightContext(),
										pAtom);
								...
								result = list_Nconc(inf_ConstraintHyperResolvents(gClause, tLits,
										leftSubst, restLits, foundLits, Index, Flags, Precedence),
										result);
...
LIST inf_ForwardSortResolution(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		BOOL Precheck, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (j = i + 1; j <= lc; j++) {
		if (term_FirstArgument(clause_GetLiteralAtom(GivenClause, j)) == term_FirstArgument(TAtom))
			TLits = list_Cons((POINTER) j, TLits);
	}
	RestLits = list_Copy(TLits);

	if (!Precheck || inf_SubsortPrecheck(GivenClause, TLits, NULL, Index, SortTheory)) {

		Result = inf_ConstraintHyperResolvents(GivenClause, TLits, subst_Nil(), RestLits,
				list_Nil(), Index, Flags, Precedence);
...
LIST inf_BackwardEmptySort(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		BOOL Precheck, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause with a solved sort constraint, an 'Index' of clauses,
 a sort theory, a boolean flag indicating whether the subsort
	...
	for (i = clause_FirstSuccedentLitIndex(GivenClause); i <= ls; i++) {
		...
		if (clause_LiteralGetFlag(pLit, STRICTMAXIMAL) && clause_LiteralIsSort(pLit)) {
			...
			for (; !list_Empty(unifiers); unifiers = list_Pop(unifiers)) {
				if (term_IsAtom(list_Car(unifiers)) && term_IsVariable(term_FirstArgument(list_Car(
						unifiers)))) {
					...
					for (; !list_Empty(litScan); litScan = list_Cdr(litScan)) {
						...
						if (clause_LiteralGetIndex(gLit) < clause_FirstAntecedentLitIndex(gClause)
							...
							for (j = clause_FirstAntecedentLitIndex(gClause); (j <= bound)
									&& varOccursNoMore; j++) {
							...
							if (varOccursNoMore || clause_HasOnlySpecDomArgs(gClause)) {
								...
								if (!Precheck || inf_SubsortPrecheck(gClause, tLits, pLit, Index,
									...
									result = list_Nconc(inf_ConstraintHyperResolvents(gClause,
											tLits, leftSubst, restLits, foundLits, Index, Flags,
											Precedence), result);
...
LIST inf_ForwardEmptySort(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory, BOOL Precheck,
 ...
 ***************************************************************/
{
	...
	while (i <= lc && !Hit) {
		...
		if (term_IsVariable(term_FirstArgument(TAtom))) {
			...
			for (j = clause_FirstAntecedentLitIndex(GivenClause); j <= ls && Hit; j++) {
 ...
 ***************************************************************/
{
	...
	unifiers = st_GetUnifier(cont_LeftContext(), Index, cont_RightContext(), term_FirstArgument(
			clause_LiteralAtom(Literal)));

	for (; !list_Empty(unifiers); unifiers = list_Pop(unifiers)) {

		if (!term_IsAtom(list_Car(unifiers))) { /* Can happen if arg is variable */

			for (atomScan = term_SupertermList(list_Car(unifiers)); !list_Empty(atomScan); atomScan
					= list_Cdr(atomScan)) {
				TERM atomCand;
				...
				if (term_IsDeclaration(atomCand)) {
					...
					for (litScan = sharing_NAtomDataList(atomCand); !list_Empty(litScan); litScan
							= list_Cdr(litScan)) {
						LITERAL nextLit;
						CLAUSE nextClause;
						nextLit = list_Car(litScan);
						...
						if (clause_LiteralIsPositive(nextLit) && clause_LiteralGetFlag(nextLit,
								STRICTMAXIMAL) && clause_GetFlag(nextClause, WORKEDOFF)
 ...
 ***************************************************************/
{
	return (term_FirstArgument(clause_LiteralAtom(L1))
			== term_FirstArgument(clause_LiteralAtom(L2)) && clause_LiteralOwningClause(L1)
			== clause_LiteralOwningClause(L2));
}

static void inf_GetBackwardPartnerLits(LITERAL Literal, st_INDEX Index, LIST* ConstraintLits,
		LIST* Unifiers, BOOL IsFromEmptySort, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	candidates = st_GetUnifier(cont_LeftContext(), Index, cont_RightContext(), term_FirstArgument(
			clause_LiteralAtom(Literal)));
	...
	for (; !list_Empty(candidates); candidates = list_Pop(candidates)) {
		if (!term_IsAtom(list_Car(candidates))) { /* May happen if arg is variable */
			...
			for (atomScan = term_SupertermList(list_Car(candidates)); !list_Empty(atomScan); atomScan
					= list_Cdr(atomScan)) {
				TERM atomCand;
				atomCand = (TERM) list_Car(atomScan);
				if (term_IsDeclaration(atomCand)) {
					...
					for (litScan = sharing_NAtomDataList(atomCand); !list_Empty(litScan); litScan
						...
						if (clause_GetFlag(nextClause, WORKEDOFF)) {
							if (clause_LiteralIsPositive(nextLit)) {
								if (clause_LiteralGetFlag(nextLit, STRICTMAXIMAL)
										&& (!term_IsVariable(list_Car(candidates))
												|| clause_HasEmptyConstraint(nextClause))
										&& clause_HasSolvedConstraint(nextClause) && !symbol_Equal(
										clause_LiteralPredicate(Literal), clause_LiteralPredicate(
												nextLit))) {
							...
							} else if (clause_LiteralGetIndex(nextLit)
									< clause_FirstAntecedentLitIndex(nextClause)
									&& ((!term_IsVariable(list_Car(candidates)) && !IsFromEmptySort)
											|| (term_IsVariable(list_Car(candidates))
													&& IsFromEmptySort
													&& clause_HasOnlyVarsInConstraint(nextClause,
															Flags, Precedence)))) {
	...
	*ConstraintLits = list_DeleteDuplicates(*ConstraintLits,
 ...
 ***************************************************************/
{
	...
	for (; !list_Empty(Literals); Literals = list_Cdr(Literals)) {
		SORT newSort = sort_TheorySortOfSymbol(SortTheory, clause_LiteralPredicate(list_Car(
				Literals)));
 ...
 ***************************************************************/
{
	...
	for (scan = sort_ConditionConstraint(Condition); !list_Empty(scan); scan = list_Cdr(scan)) {
		...
		constraint = list_Cons(cont_CopyAndApplyBindings(cont_LeftContext(), termCopy), constraint);
	...
	for (scan = sort_ConditionAntecedent(Condition); !list_Empty(scan); scan = list_Cdr(scan)) {
		...
		termCopy = term_Copy(list_Car(scan));
		term_ReplaceVariable(termCopy, sort_ConditionVar(Condition), tSubterm);
		antecedent = list_Cons(cont_CopyAndApplyBindings(cont_LeftContext(), termCopy), antecedent);
	...
	for (scan = sort_ConditionSuccedent(Condition); !list_Empty(scan); scan = list_Cdr(scan)) {
		...
		termCopy = term_Copy(list_Car(scan));
		term_ReplaceVariable(termCopy, sort_ConditionVar(Condition), tSubterm);
		succedent = list_Cons(cont_CopyAndApplyBindings(cont_LeftContext(), termCopy), succedent);
	...
	for (scan = Partners; !list_Empty(scan); scan = list_Cdr(scan)) {
		...
		for (i = clause_FirstLitIndex(); i <= bound; i++) {
			constraint = list_Cons(cont_CopyAndApplyBindings(cont_RightContext(),
					clause_GetLiteralAtom(pClause, i)), constraint);
		}
		...
		for (i = clause_FirstAntecedentLitIndex(pClause); i <= bound; i++) {
			antecedent = list_Cons(cont_CopyAndApplyBindings(cont_RightContext(),
					clause_GetLiteralAtom(pClause, i)), antecedent);
		}
		...
		for (i = clause_FirstSuccedentLitIndex(pClause); i <= bound; i++) {
			if (i != pi)
				succedent = list_Cons(cont_CopyAndApplyBindings(cont_RightContext(),
						clause_GetLiteralAtom(pClause, i)), succedent);
		}
	...
	for (i = clause_FirstLitIndex(); i <= bound; i++) {
		if (!list_PointerMember(TLits, (POINTER) i))
			constraint = list_Cons(cont_CopyAndApplyBindings(cont_LeftContext(),
					clause_GetLiteralAtom(Clause, i)), constraint);
		else {
	...
	for (i = clause_FirstAntecedentLitIndex(Clause); i <= bound; i++) {
		antecedent = list_Cons(cont_CopyAndApplyBindings(cont_LeftContext(), clause_GetLiteralAtom(
				Clause, i)), antecedent);
	}
	bound = clause_LastSuccedentLitIndex(Clause);
	for (i = clause_FirstSuccedentLitIndex(Clause); i <= bound; i++) {
		succedent = list_Cons(cont_CopyAndApplyBindings(cont_LeftContext(), clause_GetLiteralAtom(
				Clause, i)), succedent);
	}
...
static LIST inf_InternWeakening(CLAUSE Clause, LIST TLits, LIST Unifiers, LITERAL Special,
		LIST SojuList, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	searchTerm = term_FirstArgument(clause_GetLiteralAtom(Clause, (int) list_Car(TLits)));
	...
	for (; !list_Empty(SojuList); SojuList = list_Pop(SojuList)) {
		...
		fputs("\nSOJU: ", stdout);
		sort_PairPrint(actSoju);
		fflush(stdout);

		if (Special == NULL || sort_ContainsSymbol(sort_PairSort(actSoju), clause_LiteralPredicate(
				Special))) {
			...
			do {
				...
				if (list_Empty(symbolScan)) {
					...
					result = list_Nconc(inf_ApplyWeakening(Clause, TLits, subset,
							sort_PairCondition(actSoju), Flags, Precedence), result);
				}
...
LIST inf_ForwardWeakening(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
			...
			if (!list_Empty(unifiers)) {
				...
				for (j = lc; j > i; j--) {
					...
					if (term_FirstArgument(actAtom) == term_FirstArgument(tAtom)) {
						tLits = list_Cons((POINTER) j, tLits);
						tSort = sort_Intersect(sort_TheorySortOfSymbol(SortTheory, term_TopSymbol(
								actAtom)), tSort);
					}
				}
				...
				result = list_Nconc(inf_InternWeakening(GivenClause, tLits, unifiers, NULL,
						sojuList, Flags, Precedence), result);
...
LIST inf_BackwardWeakening(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(GivenClause); i <= ls; i++) {
		...
		if (clause_LiteralGetFlag(sLit, STRICTMAXIMAL) && clause_LiteralIsSort(sLit)
				&& (!term_IsVariable(term_FirstArgument(sAtom)) || clause_HasEmptyConstraint(
						GivenClause))) {
			LIST unifiers, partners;
			SORT unifierSort;

			unifiers = partners = list_Nil();
			inf_GetBackwardPartnerLits(sLit, Index, &partners, &unifiers, FALSE, Flags, Precedence);
			...
			for (; !list_Empty(partners); partners = list_Pop(partners)) {
				...
				for (j = lc; j >= clause_FirstLitIndex(); j--) {
					...
					if (j != ti && term_FirstArgument(actAtom) == term_FirstArgument(tAtom)) {
						tLits = list_Cons((POINTER) j, tLits);
						tSort = sort_Intersect(sort_TheorySortOfSymbol(SortTheory, term_TopSymbol(
								actAtom)), tSort);
					}
				...
				cont_StartBinding();
				unify_UnifyNoOC(cont_LeftContext(), tAtom, cont_RightContext(), sAtom);

				result = list_Nconc(inf_InternWeakening(tClause, tLits, unifiers, sLit, sojuList,
...
LIST inf_ForwardEmptySortPlusPlus(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause,  an 'Index' of clauses, a sort theory, a flag store
 and a precedence.
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstLitIndex(); i <= lc && !hit; i++) {
		...
		if (term_IsVariable(term_FirstArgument(clause_GetLiteralAtom(GivenClause, i)))) {
			...
			varOccursNoMore = TRUE;
			/* Check if the variable occurs in antecedent or succedent literals */
			for (j = clause_FirstAntecedentLitIndex(GivenClause); (j <= ls) && varOccursNoMore; j++) {
				if (term_ContainsSymbol(clause_GetLiteralAtom(GivenClause, j), term_TopSymbol(var)))
					varOccursNoMore = FALSE;
			}
					...
					for (j = lc; j > i; j--) {
						...
						if (term_FirstArgument(actAtom) == var) { /* tClause is shared */
							...
							tSort = sort_Intersect(sort_TheorySortOfSymbol(SortTheory,
									term_TopSymbol(actAtom)), tSort);
						}
					}
					tLits = list_Cons((POINTER) i, tLits);
					tSort = sort_Intersect(sort_TheorySortOfSymbol(SortTheory,
							term_TopSymbol(tAtom)), tSort);
					list_PointerDeleteDuplicates(tSort);
...
LIST inf_BackwardEmptySortPlusPlus(CLAUSE GivenClause, st_INDEX Index, SORTTHEORY SortTheory,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause with solved sort constraint, an index of clauses,
 a sort theory, a flag store and a precedence.
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(GivenClause); i <= ls; i++) {
		...
		if (clause_LiteralGetFlag(sLit, STRICTMAXIMAL) && clause_LiteralIsSort(sLit)
			...
			unifiers = partners = list_Nil();
			inf_GetBackwardPartnerLits(sLit, Index, &partners, &unifiers, TRUE, Flags, Precedence);
			unifiers = list_Cons(sLit, unifiers);
			/* <partners> holds monadic constraint literals */
			/* <unifiers> holds monadic, maximal succedent literals */
			...
			for (; !list_Empty(partners); partners = list_Pop(partners)) {
				...
				varOccursNoMore = TRUE;
				for (j = clause_FirstAntecedentLitIndex(tClause); j <= li && varOccursNoMore; j++) {
					if (term_ContainsSymbol(clause_GetLiteralAtom(tClause, j), term_TopSymbol(var)))
						varOccursNoMore = FALSE;
				}
				...
				if (varOccursNoMore) {
					...
					for (j = lc; j >= clause_FirstLitIndex(); j--) {
						...
						actAtom = clause_GetLiteralAtom(tClause, j);
						if (j != ti && term_TopSymbol(term_FirstArgument(actAtom))
								== term_TopSymbol(var)) {
							tLits = list_Cons((POINTER) j, tLits);
							tSort = sort_Intersect(sort_TheorySortOfSymbol(SortTheory,
									term_TopSymbol(actAtom)), tSort);
					...
					result = list_Nconc(inf_InternWeakening(tClause, tLits, unifiers, sLit,
							sojuList, Flags, Precedence), result);
