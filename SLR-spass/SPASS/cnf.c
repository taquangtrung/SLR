 ...
 ***************************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_CNFSTRSKOLEM))
		cnf_VARIABLEDEPTHARRAY = (int*) memory_Malloc(
				sizeof(int[symbol__MAXSTANDARDVAR + 1]));
	else
 ...
 ***************************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_CNFSTRSKOLEM)) {
		memory_Free(cnf_VARIABLEDEPTHARRAY, sizeof(int[symbol__MAXSTANDARDVAR
				+ 1]));
		cnf_VARIABLEDEPTHARRAY = NULL;
	}
 ...
 ********************************************************/
{
	...
	do {
		...
		if (term1 == subterm) {
			vec_SetMax(bottom);
			return polterm1;
		} else if (symbol_Equal(term_TopSymbol(term1), fol_Not())) {
			vec_Push((POINTER) (-polterm1));
			vec_Push(list_Car(term_ArgumentList(term1)));
		}
		if (symbol_Equal(term_TopSymbol(term1), fol_Exist()) || symbol_Equal(
				term_TopSymbol(term1), fol_All())) {
			vec_Push((POINTER) polterm1);
			vec_Push(list_Second(term_ArgumentList(term1)));
		} else if (symbol_Equal(term_TopSymbol(term1), fol_Implies())) {
			vec_Push((POINTER) (-polterm1));
			vec_Push(list_Car(term_ArgumentList(term1)));
			vec_Push((POINTER) polterm1);
			vec_Push(list_Second(term_ArgumentList(term1)));
		} else if (symbol_Equal(term_TopSymbol(term1), fol_Equiv())) {
			vec_Push(0);
			vec_Push(list_Car(term_ArgumentList(term1)));
			vec_Push(0);
			vec_Push(list_Second(term_ArgumentList(term1)));
		} else if (symbol_Equal(term_TopSymbol(term1), fol_And())
				|| symbol_Equal(term_TopSymbol(term1), fol_Or())) {
			for (scan = term_ArgumentList(term1); !list_Empty(scan); scan
					= list_Cdr(scan)) {
				vec_Push((POINTER) polterm1);
 ...
 ********************************************************/
{
	...
	if (symbol_Equal(term_TopSymbol(Def), fol_And()) || symbol_Equal(
			term_TopSymbol(Def), fol_Or())) {
		/* Polarity is ok */
		LIST l;
		for (l = term_ArgumentList(Def); !list_Empty(l); l = list_Cdr(l))
			if (cnf_ContainsDefinitionIntern(TopDef, list_Car(l), Polarity,
					FoundPred))
				return TRUE;
		return FALSE;
	}
	...
	if (fol_IsQuantifier(term_TopSymbol(Def)))
		return cnf_ContainsDefinitionIntern(TopDef, term_SecondArgument(Def),
				Polarity, FoundPred);

	/* Negation */
	if (symbol_Equal(term_TopSymbol(Def), fol_Not()))
		return cnf_ContainsDefinitionIntern(TopDef, term_FirstArgument(Def),
				-Polarity, FoundPred);

	/* Implication */
	if (symbol_Equal(term_TopSymbol(Def), fol_Implies())) {
		if (Polarity == 1) {
			if (cnf_ContainsDefinitionIntern(TopDef, term_FirstArgument(Def),
					-Polarity, FoundPred))
				return TRUE;
			return cnf_ContainsDefinitionIntern(TopDef,
					term_SecondArgument(Def), Polarity, FoundPred);
		}
		return FALSE;
	}
		...
		if (defpredicate != (TERM) NULL) {
			...
			for (l = defpath; !list_Empty(l); l = list_Cdr(l)) {
				...
				if (symbol_Equal(term_TopSymbol((TERM) list_Car(l)), fol_Not()))
					...
					if (symbol_Equal(term_TopSymbol((TERM) list_Car(l)),
							fol_Implies())
							&& (term_FirstArgument((TERM) list_Car(l))
									== (TERM) list_Car(list_Cdr(l))))
						pol = -pol;
					else if (symbol_Equal(term_TopSymbol((TERM) list_Car(l)),
							fol_Equiv()))
						pol = 0;
				}
			}
			/* <defpath> is now a list of pairs (term, polarity) */
			for (l = defpath; !list_Empty(l) && !list_Empty(predicate_vars); l
					= list_Cdr(l)) {
				LIST pair;
				TERM t;
				int p;
				...
				if (fol_IsQuantifier(term_TopSymbol(t))) {
					...
					if ((symbol_Equal(term_TopSymbol(t), fol_All()) && (p == 1))
							|| (symbol_Equal(term_TopSymbol(t), fol_Exist())
									&& (p == -1))) {
						LIST scan;
						allquantifierfound = TRUE;
						for (scan = fol_QuantifierVariables(t); !list_Empty(
								scan); scan = list_Cdr(scan))
							predicate_vars = list_DeleteElement(predicate_vars,
									(TERM) list_Car(scan), (BOOL(*)(POINTER,
											POINTER)) term_Equal);
					} else {
						...
						if (allquantifierfound) {
							...
							for (scan = fol_QuantifierVariables(t); !list_Empty(
									scan); scan = list_Cdr(scan)) {
								if (term_ListContainsTerm(predicate_vars,
										list_Car(scan))) {
									list_Delete(predicate_vars);
									list_DeletePairList(defpath);
									return FALSE;
 ...
 **********************************************************/
{
	...
	for (l = predicatevars; !list_Empty(l); l = list_Cdr(l))
		freevars = list_DeleteElement(freevars, list_Car(l), (BOOL(*)(POINTER,
				POINTER)) term_Equal);
...
static BOOL cnf_ContainsPredicateIntern(TERM Target, SYMBOL Predicate,
		int Polarity, TERM* TargetPredicate, TERM* ToTopLevel,
 ...
 ********************************************************/
{
	...
	if ((symbol_Equal(term_TopSymbol(Target), fol_And()) && Polarity == 1)
			|| (symbol_Equal(term_TopSymbol(Target), fol_Or()) && Polarity
					== -1) || (symbol_Equal(term_TopSymbol(Target),
			fol_Implies()) && Polarity != 1) || symbol_Equal(term_TopSymbol(
			Target), fol_Equiv())) {
		TERM s;
		LIST l;
		...
		for (l = term_ArgumentList(s); !list_Empty(l); l = list_Cdr(l))
			*TargetVars = list_Nconc(fol_FreeVariables((TERM) list_Car(l)),
					*TargetVars);
	...
	/* AND / OR continued */
	if (symbol_Equal(term_TopSymbol(Target), fol_And()) || symbol_Equal(
			term_TopSymbol(Target), fol_Or())) {
		/* The polarity is ok here */
		LIST l;
		for (l = term_ArgumentList(Target); !list_Empty(l); l = list_Cdr(l))
			if (cnf_ContainsPredicateIntern((TERM) list_Car(l), Predicate,
					Polarity, TargetPredicate, ToTopLevel, TargetVars,
					VarsForTopLevel))
				return TRUE;
		return FALSE;
	}
	...
	if (fol_IsQuantifier(term_TopSymbol(Target))) {
		if (cnf_ContainsPredicateIntern(term_SecondArgument(Target), Predicate,
			...
			if ((symbol_Equal(term_TopSymbol(Target), fol_All()) && Polarity
					== 1) || (symbol_Equal(term_TopSymbol(Target), fol_Exist())
					&& Polarity == -1)) {
				LIST l;
				/* Check for all variables found in the predicates arguments */
				for (l = *TargetVars; !list_Empty(l); l = list_Cdr(l)) {
					if (term_ListContainsTerm(fol_QuantifierVariables(Target),
							list_Car(l)))
						*VarsForTopLevel = list_Cons(list_Car(l),
								*VarsForTopLevel);
				}
			}
			return TRUE;
	...
	if (symbol_Equal(term_TopSymbol(Target), fol_Not()))
		return cnf_ContainsPredicateIntern(term_FirstArgument(Target),
				Predicate, -Polarity, TargetPredicate, ToTopLevel, TargetVars,
				VarsForTopLevel);
	/* Implication */
	if (symbol_Equal(term_TopSymbol(Target), fol_Implies())) {
		...
		if (cnf_ContainsPredicateIntern(term_FirstArgument(Target), Predicate,
				...
				VarsForTopLevel))
			return TRUE;
		return cnf_ContainsPredicateIntern(term_SecondArgument(Target),
				Predicate, Polarity, TargetPredicate, ToTopLevel, TargetVars,
				VarsForTopLevel);
	}
...
BOOL cnf_ContainsPredicate(TERM Target, SYMBOL Predicate,
		TERM* TargetPredicate, TERM* ToTopLevel, LIST* TargetVars,
 ...
 **************************************************/
{
	...
	/* Junctors and NOT */
	if (fol_IsJunctor(term_TopSymbol(Term)) || symbol_Equal(
			term_TopSymbol(Term), fol_Not())) {
		LIST scan;
		int count;
		count = 0;
	...
	while (term1 != NULL) {
		...
		if (symbol_Equal(term_TopSymbol(term1), fol_Not())) {
			...
			if (symbol_Equal(term_TopSymbol(subterm), fol_Not())) {
				LIST l;
				term_RplacTop(term1,
						term_TopSymbol(term_FirstArgument(subterm)));
				list_Delete(term_ArgumentList(term1));
				term_RplacArgumentList(term1, term_ArgumentList(
						term_FirstArgument(subterm)));
				term_Free(term_FirstArgument(subterm));
				list_Delete(term_ArgumentList(subterm));
				term_Free(subterm);
				/* Set superterm member to new superterm */
				for (l = term_ArgumentList(term1); !list_Empty(l); l
						= list_Cdr(l))
				...
				if (fol_IsQuantifier(term_TopSymbol(subterm))) {
					LIST l;
					symbol
							= (SYMBOL) cnf_GetDualSymbol(
									term_TopSymbol(subterm));
					termL = term_CreateAddFather(fol_Not(), list_List(
							term_SecondArgument(subterm)));
					...
					if (symbol_Equal(term_TopSymbol(subterm), fol_Or())
						...
						LIST l;
						symbol = (SYMBOL) cnf_GetDualSymbol(term_TopSymbol(
								subterm));
						for (scan = term_ArgumentList(subterm); !list_Empty(
								scan); scan = list_Cdr(scan)) {
							TERM new;
							termL = list_Car(scan);
							new = term_CreateAddFather(fol_Not(), list_List(
									termL));
...
static TERM cnf_ApplyDefinitionInternOnce(TERM Predicate, TERM Formula,
		TERM TargetTerm, TERM TargetPredicate, BOOL* Success)
/**********************************************************
 ...
 ***********************************************************/
{
	...
	if (fol_IsQuantifier(term_TopSymbol(TargetTerm))) {
		term_RplacSecondArgument(TargetTerm, cnf_ApplyDefinitionInternOnce(
				Predicate, Formula, term_SecondArgument(TargetTerm),
				TargetPredicate, Success));
	...
	/* Junctors and NOT */
	if (fol_IsJunctor(term_TopSymbol(TargetTerm)) || symbol_Equal(
			term_TopSymbol(TargetTerm), fol_Not())) {
		LIST scan;
		for (scan = term_ArgumentList(TargetTerm); !list_Empty(scan); scan
				= list_Cdr(scan)) {
			list_Rplaca(scan, cnf_ApplyDefinitionInternOnce(Predicate, Formula,
					list_Car(scan), TargetPredicate, Success));
 ...
 ********************************************************/
{
	...
	while (bottom != vec_ActMax()) {
		...
		if (symbol_Equal(term_TopSymbol(term1), fol_Implies())) {
			...
			list_Rplaca(term_ArgumentList(term1), term_Create(fol_Not(),
					list_List(list_Car(term_ArgumentList(term1)))));
		} else if (symbol_Equal(term_TopSymbol(term1), fol_Equiv())) {
			...
			if (pol == 1 || pol == 0) {
				...
				list_Rplaca(term_ArgumentList(term1), term_Create(fol_Or(),
						list_Cons(termLneg, list_List(termR))));
				list_RplacSecond(term_ArgumentList(term1), term_Create(
						fol_Or(), list_Cons(termRneg, list_List(termL))));
			} else if (pol == -1) {
				term_RplacTop(term1, fol_Or());
				list_Rplaca(term_ArgumentList(term1), term_Create(fol_And(),
						list_Cons(termLneg, list_List(termRneg))));
				list_RplacSecond(term_ArgumentList(term1), term_Create(
						fol_And(), list_Cons(termL, list_List(termR))));
			}
		}
		if (!list_Empty(term_ArgumentList(term1)) && !symbol_IsPredicate(
				term_TopSymbol(term1))) {
			for (scan = term_ArgumentList(term1); !list_Empty(scan); scan
					= list_Cdr(scan))
				vec_Push(list_Car(scan));
		}
 ...
 ********************************************************/
{
			...
			for (scan = term_ArgumentList(term1); !list_Empty(scan); scan
				...
				if (term_HasProperSuperterm(TargetPredicateTerm, arg)) {
					if (symbol_Equal(term_TopSymbol(arg), fol_All())) {
						...
						for (predicatevarscan = VarsForTopLevel; !list_Empty(
								predicatevarscan); predicatevarscan = list_Cdr(
								predicatevarscan))
							quantifiervars = list_DeleteElementFree(
									quantifiervars, (TERM) list_Car(
											predicatevarscan), (BOOL(*)(
											POINTER, POINTER)) term_Equal,
									(void(*)(POINTER)) term_Delete);
						if (!list_Empty(quantifiervars))
							term_RplacArgumentList(term_FirstArgument(arg),
									quantifiervars);
						else {
							TERM subterm;
	...
	if (symbol_Equal(term_TopSymbol(Term), fol_All())) {
		...
		TERM newtop;
		newtop = fol_CreateQuantifier(fol_All(), list_Copy(VarsForTopLevel),
				list_List(Term));
	...
	while (bottom != vec_ActMax()) {
		...
		if (term_HasProperSuperterm(PredicateTerm, term1)) {
			if (symbol_Equal(term_TopSymbol(term1), fol_Implies())) {
				TERM newterm;
				term_RplacTop(term1, fol_Or());
				newterm = term_CreateAddFather(fol_Not(), list_List(list_Car(
						term_ArgumentList(term1))));
	...
	while (bottom != vec_ActMax()) {
		...
		if (symbol_Equal(term_TopSymbol(term1), fol_Not())) {
			...
			if (symbol_Equal(term_TopSymbol(subterm), fol_Not())) {
				...
				term_Free(subterm);
				vec_Push(term1);
			} else if (fol_IsQuantifier(term_TopSymbol(subterm))) {
				symbol = (SYMBOL) cnf_GetDualSymbol(term_TopSymbol(subterm));
				termL = term_Create(fol_Not(), list_List(term_SecondArgument(
						subterm)));
				list_RplacSecond(term_ArgumentList(subterm), termL);
				term_RplacTop(term1, symbol);
				list_Delete(term_ArgumentList(term1));
				term_RplacArgumentList(term1, term_CopyTermList(
						term_ArgumentList(subterm)));
				term_Delete(subterm);
			} else if (symbol_Equal(term_TopSymbol(subterm), fol_Or())
					|| symbol_Equal(term_TopSymbol(subterm), fol_And())) {
				symbol = (SYMBOL) cnf_GetDualSymbol(term_TopSymbol(subterm));
				for (scan = term_ArgumentList(subterm); !list_Empty(scan); scan
						= list_Cdr(scan)) {
	...
	while (bottom != vec_ActMax()) {
		...
		if (fol_IsQuantifier(term_TopSymbol(term1))) {
			...
			if (list_Length(term_ArgumentList(term_FirstArgument(term1))) > 1) {
				varlist = list_Copy(list_Cdr(term_ArgumentList(
						term_FirstArgument(term1))));
				for (scan = varlist; !list_Empty(scan); scan = list_Cdr(scan)) {
					termL = term_SecondArgument(term1);
					term_RplacSecondArgument(term1, fol_CreateQuantifier(
							quantor, list_List(list_Car(scan)),
							list_List(termL)));
				}
				for (scan = varlist; !list_Empty(scan); scan = list_Cdr(scan)) {
					term_RplacArgumentList(term_FirstArgument(term1),
							list_PointerDeleteElement(term_ArgumentList(
									term_FirstArgument(term1)), list_Car(scan)));
				}
				list_Delete(varlist);
			}
		}
 ...
 ***************************************************************/
{
	...
	result = list_Nil();
	for (scan = varlist; !list_Empty(scan); scan = list_Cdr(scan))
		result = list_Cons((POINTER) term_TopSymbol((TERM) list_Car(scan)),
				result);
	...
	if (symbol_Equal(term_TopSymbol(term), fol_Or())) {
		...
		while (!list_Empty(scan)) {
			...
			if (symbol_Equal(term_TopSymbol(argterm), fol_Or())) {
				scan = list_Cdr(scan);
				term_RplacArgumentList(term, list_Nconc(term_ArgumentList(
						argterm), list_PointerDeleteElement(term_ArgumentList(
						term), argterm)));
				term_Free(argterm);
			} else
				scan = list_Cdr(scan);
 ...
 ***************************************************************/
{
	...
	if (cnf_TopIsAnd(term_ArgumentList(term))) {
		...
		for (scan1 = term_ArgumentList(term); !(list_Empty(scan1)
				|| symbol_Equal(term_TopSymbol(list_Car(scan1)), fol_And())); scan1
				= list_Cdr(scan1))
			;
		...
		if (!list_Empty(scan1)) {
			...
			for (scan = predlist; !list_Empty(scan); scan = list_Cdr(scan))
				term_RplacArgumentList(term, list_PointerDeleteElement(
						term_ArgumentList(term), list_Car(scan)));

			if (!list_Empty(predlist))
				term_RplacArgumentList(term, list_Nconc(predlist,
 ...
 ***************************************************************/
{
	...
	for (scan1 = term_ArgumentList(term); !(list_Empty(scan1) || symbol_Equal(
			term_TopSymbol(list_Car(scan1)), fol_Or())); scan1
			= list_Cdr(scan1))
		;
 ...
 ***********************************************************/
{
	...
	if (symbol_Equal(Symbol, fol_Or())) {
		...
		for (Scan = list_Cdr(term_ArgumentList(Term)); !list_Empty(Scan); Scan
				= list_Cdr(Scan)) {
			Help = cnf_ComputeLiteralLists(list_Car(Scan));
			NewResult = list_Nil();
			for (Scan1 = Help; !list_Empty(Scan1); Scan1 = list_Cdr(Scan1))
				for (Scan2 = Result; !list_Empty(Scan2); Scan2
						= list_Cdr(Scan2)) {
					...
					NewResult = list_Cons(term_DestroyDuplicatesInList(
							list_Nconc(List1, List2)), NewResult);
				}
			list_Delete(Help);
			list_Delete(Result);
			Result = NewResult;
	...
	if (symbol_Equal(Symbol, fol_And())) {
		...
		for (Scan = list_Cdr(term_ArgumentList(Term)); !list_Empty(Scan); Scan
				= list_Cdr(Scan)) {
			Result
					= list_Nconc(cnf_ComputeLiteralLists(list_Car(Scan)),
							Result);
		}
		return Result;
	}
	...
	while (bottom != vec_ActMax()) {
		...
		if (symbol_Equal(term_TopSymbol(term1), fol_And())) {
			for (scan = term_ArgumentList(term1); !list_Empty(scan); scan
					= list_Cdr(scan))
				vec_Push(list_Car(scan));
		} else if (symbol_Equal(term_TopSymbol(term1), fol_Or())
				|| symbol_IsPredicate(term_TopSymbol(term1)) || symbol_Equal(
				term_TopSymbol(term1), fol_Not()))
 ...
 ***************************************************************/
{
	...
	for (scan = term_ArgumentList(term); !list_Empty(scan); scan = list_Cdr(
			scan)) {
		termlist = list_Nil();
		if (!(symbol_IsPredicate(term_TopSymbol(list_Car(scan)))
				|| fol_IsNegativeLiteral(list_Car(scan))))
 ...
 ***************************************************************/
{
	...
	for (scan = delclauselist; !list_Empty(scan); scan = list_Cdr(scan))
		newclauselist
				= list_PointerDeleteElement(newclauselist, list_Car(scan));
		...
		while (!list_Empty(scan1)) {
			...
			if (term_HasProperSuperterm(PredicateTerm, Argterm)) {
				if (symbol_Equal(term_TopSymbol(Argterm), fol_Or())) {
					...
					for (l = term_ArgumentList(Argterm); !list_Empty(l); l
							= list_Cdr(l))
			...
			if (list_Empty(list_Cdr(Rest))) { /* One subformula */
				if (symbol_Equal(Top, term_TopSymbol(list_Car(Rest)))) { /* Optimization: quantifier still exist */
					...
					term_RplacArgumentList(term_FirstArgument(NewForm),
							list_Cons((POINTER) Var, fol_QuantifierVariables(
									NewForm)));
					list_Delete(Rest);
				} else
					NewForm = fol_CreateQuantifier(Top,
							list_List((POINTER) Var), Rest);
			} else
				NewForm = fol_CreateQuantifier(Top, list_List((POINTER) Var),
						list_List(term_Create(Subtop, Rest)));
			term_RplacArgumentList(Subterm, list_Cons(NewForm, Subformulas));
			term_RplacArgumentList(term_FirstArgument(Term),
					list_PointerDeleteElement(fol_QuantifierVariables(Term),
							(POINTER) Var));
		}
	}
 ...
 ***************************************************************/
{
	...
	if (fol_IsQuantifier(Top)) {
		...
		if (!symbol_IsPredicate(Subtop) && !symbol_Equal(Subtop, fol_Not())) { /* Formula in NNF: No Literals or Atoms */
			...
			if (symbol_Equal(Subtop, DistrSymb)) {
				...
				for (Scan = term_ArgumentList(Subterm); !list_Empty(Scan); Scan
					...
					NewVars = list_NIntersect(fol_FreeVariables(Actterm),
							Variables, (BOOL(*)(POINTER, POINTER)) term_Equal);
					if (!list_Empty(NewVars)) {
						if (symbol_Equal(Top, term_TopSymbol(Actterm))) { /* Quantor already there */
							...
							term_RplacArgumentList(term_FirstArgument(Actterm),
									list_Nconc(
											fol_QuantifierVariables(Actterm),
											NewVars));
						} else {
							term_CopyTermsInList(NewVars);
							list_Rplaca(Scan, fol_CreateQuantifier(Top,
									NewVars, list_List(Actterm)));
						}
					}
				}
			...
			} else if (!fol_IsQuantifier(Subtop)) {
	...
	} else if (!symbol_Equal(Top, fol_Not()) && !symbol_IsPredicate(Top))
		for (Scan = term_ArgumentList(Term); !list_Empty(Scan); Scan
				= list_Cdr(Scan))
			cnf_AntiPrenex(list_Car(Scan));
		...
		if (!list_Empty(Subformulas)) {
			...
			if (list_Empty(list_Cdr(Rest))) { /* One subformula */
				if (symbol_Equal(Top, term_TopSymbol(list_Car(Rest)))) { /* Optimization: quantifier still exist */
					...
					NewForm = fol_CreateQuantifierAddFather(Top, list_List(
							(POINTER) Var), Rest);
				}
			} else {
				TERM t;
		...
		if (!symbol_Equal(Subtop, fol_Not()) && !symbol_IsPredicate(Subtop)) { /* No Literals or Atoms */
			...
			if (symbol_Equal(Subtop, DistrSymb)) {
				...
				for (Scan = term_ArgumentList(Subterm); !list_Empty(Scan); Scan
					...
					if (!list_Empty(NewVars)) {
						if (symbol_Equal(Top, term_TopSymbol(Actterm))) { /* Quantor already there */
							...
							term_CopyTermsInList(NewVars);
							list_Rplaca(Scan, fol_CreateQuantifierAddFather(
									Top, NewVars, list_List(Actterm)));
						}
					}
				}
				term_Delete(term_FirstArgument(Term)); /* Delete old variable list */
				...
				for (Scan = term_ArgumentList(Term); !list_Empty(Scan); Scan
					...
					if (term_HasPointerSubterm((TERM) list_Car(Scan),
							PredicateTerm)) {
						puts("\ncheck1");
						list_Rplaca(Scan, cnf_AntiPrenexPath(list_Car(Scan),
								PredicateTerm));
						term_RplacSuperterm((TERM) list_Car(Scan), Term);
					}
				}
 ...
 ***************************************************************/
{
	...
	if (fol_IsQuantifier(Top)) {
		...
		while (symbol_Equal(term_TopSymbol(Subterm), Top)) {
			term_RplacArgumentList(term_FirstArgument(Term), list_Nconc(
					fol_QuantifierVariables(Term), fol_QuantifierVariables(
							Subterm)));
			term_Free(term_FirstArgument(Subterm));
		...
		for (Scan = fol_QuantifierVariables(Term); !list_Empty(Scan); Scan
				= list_Cdr(Scan)) {
		...
		if (!list_Empty(Vars)) {
			...
			term_RplacArgumentList(Subterm, list_NPointerDifference(
					term_ArgumentList(Subterm), Vars));
 ...
 ***************************************************************/
{
	...
	if (symbol_Equal(Top, fol_And())) {
		...
		if (Update) {
			term_RplacArgumentList(Term, fol_DeleteTrueTermFromList(
					term_ArgumentList(Term)));
		...
		if (Update) {
			term_RplacArgumentList(Term, fol_DeleteFalseTermFromList(
					term_ArgumentList(Term)));
			if (list_Empty(term_ArgumentList(Term))) {
				term_RplacTop(Term, fol_False());
				return Term;
		...
		if ((symbol_Equal(Subtop, fol_False()) && symbol_Equal(Top, fol_Not()))
			...
			term_RplacArgumentList(Term, list_Nil());
			return Term;
		} else if ((symbol_Equal(Subtop, fol_True()) && symbol_Equal(Top,
				fol_Not())) || (symbol_Equal(Subtop, fol_False())
 ...
 ***************************************************************/
{
	...
	if (fol_IsQuantifier(term_TopSymbol(Term))) {
		if (symbol_Equal(Top, fol_All())) {
			...
			if (flag_GetFlagValue(Flags, flag_QUANTEXCH)) {
				Varlist = list_Nil();
				Arity = 0;
			} else {
			...
			for (Scan = fol_QuantifierVariables(Term); !list_Empty(Scan); Scan
				...
				*Symblist = list_Cons((POINTER) SkolemSymbol, *Symblist);
				...
				fol_ReplaceVariable(term_SecondArgument(Term), term_TopSymbol(
						list_Car(Scan)), SkolemTerm);
...
static TERM cnf_Cnf(TERM Term, FLAGSTORE Flags, PRECEDENCE Precedence,
		LIST* Symblist)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	Term = ren_Rename(Term, Precedence, Symblist, FALSE, FALSE,
			flag_GetFlagValue(Flags, flag_CNFRENAMING));
 ...
 ***************************************************************/
{
	...
	for (Scan = clause_ParentClauses(C); !list_Empty(Scan); Scan = list_Cdr(
			Scan)) {
		CLAUSE P;
		int ClauseNumber;
		...
		if (P == NULL) {
			...
			if (P == NULL)
				P = clause_GetNumberedCl(ClauseNumber, prfs_DocProofClauses(
						Search));
		}
...
static BOOL cnf_HaveProofOptSkolem(PROOFSEARCH Search, TERM topterm,
		char* toplabel, TERM term2, LIST* UsedTerms, LIST* Symblist,
		HASH InputClauseToTermLabellist)
/**************************************************************
 INPUT:
 RETURNS: ??? EK
 ***************************************************************/
{
	...
	if (!list_Empty(EmptyClauses)) {
		...
		if (flag_GetFlagValue(Flags, flag_DOCPROOF) || flag_GetFlagValue(Flags,
				flag_FLOTTER))
			*UsedTerms = list_Nconc(*UsedTerms, cnf_GetUsedTerms(
					(CLAUSE) list_Car(EmptyClauses), Search,
 ...
 ***************************************************************/
{
	...
	for (scan = TermList; !list_Empty(scan); scan = list_Cdr(scan)) {
		...
		usables = list_Nconc(cnf_MakeClauseList(t, FALSE, FALSE, Flags,
				Precedence), usables);
		term_Delete(t);
	}
	...
	usables = list_Nconc(cnf_MakeClauseList(ToProve, FALSE, FALSE, Flags,
			Precedence), usables);
	term_Delete(ToProve);
 ...
 ***********************************************************/
{
	...
	while (bottom != vec_ActMax()) {
		...
		if (symbol_Equal(term_TopSymbol(term1), varsymb)) {
			...
			term_RplacArgumentList(term1, term_CopyTermList(term_ArgumentList(
					function)));
		} else if (!list_Empty(term_ArgumentList(term1)))
			for (scan = term_ArgumentList(term1); !list_Empty(scan); scan
					= list_Cdr(scan))
				vec_Push(list_Car(scan));
	}
 ...
 ***********************************************************/
{
	...
	while (bottom != vec_ActMax()) {
		...
		if (symbol_IsVariable(term_TopSymbol(term1))) {
			...
			for (scan = Varlist, scan2 = Termlist; !list_Empty(scan) && !done; scan
					= list_Cdr(scan), scan2 = list_Cdr(scan2)) {
				if (symbol_Equal(term_TopSymbol(term1), term_TopSymbol(
						list_Car(scan)))) {
					term_RplacTop(term1, term_TopSymbol((TERM) list_Car(scan2)));
					term_RplacArgumentList(term1, term_CopyTermList(
							term_ArgumentList(list_Car(scan2))));
					done = TRUE;
				}
			}
		} else if (!list_Empty(term_ArgumentList(term1)))
 ...
 ***************************************************************/
{
	...
	Result
			= fol_CreateQuantifier(fol_Exist(), VarListCopy,
					list_List(TermCopy));
	...
	if (!list_Empty(FreeList)) {
		...
		Result = fol_CreateQuantifier(fol_All(), FreeListCopy,
				list_List(Result));
	}
 ...
 *******************************************************************/
{
	...
	term_RplacArgumentList(termR, list_PointerDeleteElement(term_ArgumentList(
			termR), Proved));
	...
	if (!list_Empty(FreeList)) {
		Proved = fol_CreateQuantifier(fol_All(), term_CopyTermList(FreeList),
				list_List(Proved));
...
static void cnf_StrongSkolemization(PROOFSEARCH Search, TERM Topterm,
		char* Toplabel, BOOL TopAnd, TERM Term, LIST* UsedTerms,
		LIST* Symblist, BOOL Result1, HASH InputClauseToTermLabellist,
		int Depth)
/******************************************************************
 ...
 *******************************************************************/
{
	...
	for (scan = term_ArgumentList(Term); !list_Empty(scan); scan = list_Cdr(
			scan)) {
		pair = list_PairCreate((TERM) list_Car(scan), cnf_FreeVariablesBut(
				(TERM) list_Car(scan), exlist));
		if (list_Empty(pairlist))
			...
			while (!list_Empty(pairscan)) {
				currentlength = list_Length((LIST) list_PairSecond(
						(LIST) list_Car(pairscan)));
				if (currentlength < pairlength) {
					...
					if (cnf_HasDeeperVariable((LIST) list_PairSecond(
							(LIST) list_Car(pairscan)), /* in list */
	...
	for (scan = exlist; !list_Empty(scan); scan = list_Cdr(scan)) {
		SYMBOL skolem;
		skolem = symbol_CreateSkolemFunction(numberofallfreevariables,
				Precedence);
	...
	for (pairscan = pairlist; !list_Empty(pairscan); pairscan = list_Cdr(
			pairscan)) {
		LIST allist;
		...
		accumulatedvariables = term_DeleteDuplicatesFromList(
		...
		allist = list_Nconc(list_Copy(accumulatedvariables), list_Copy(
				newvariables));

		cnf_SkolemFunctionFormulaMapped((TERM) list_PairFirst((LIST) list_Car(
				pairscan)), allist, mapping);
#ifdef CHECK_STRSKOLEM
		if (flag_GetFlagValue(flag_PSTRSKOLEM)) {
		...
		cnf_OptimizedSkolemFormula(Search, Topterm, Toplabel, TopAnd,
				(TERM) list_PairFirst((LIST) list_Car(pairscan)), UsedTerms,
	...
	if (fol_IsQuantifier(top)) {
		if (symbol_Equal(top, fol_All())) {
...
#endif
					cnf_VARIABLEDEPTHARRAY[term_TopSymbol(list_Car(scan))]
							= Depth;
				}
				Depth++;
			}
			...
			cnf_OptimizedSkolemFormula(Search, topterm, toplabel, TopAnd, term,
					UsedTerms, Symblist, Result1, InputClauseToTermLabellist,
					Depth);
			return;
		}
		...
		if (symbol_Equal(term_TopSymbol(term_SecondArgument(term)), fol_And())) {
				...
				while (!list_Empty(scan) && !optimized) {
					if (!Result1) {
						if (TopAnd) {
							...
							termL2 = cnf_QuantifyAndNegate(
									(TERM) list_Car(scan), varlist,
									freevariables);
							result2 = cnf_HaveProofOptSkolem(Search, topterm,
									toplabel, termL2, UsedTerms, Symblist,
									InputClauseToTermLabellist);
					...
					if (Result1 || result2) {
						...
						provedterm = cnf_MoveProvedTermToTopLevel(topterm,
								term, list_Car(scan), varlist, freevariables,
								Precedence);
						if (flag_GetFlagValue(Flags, flag_POPTSKOLEM)) {
							fputs("Result : ", stdout);
							term_Print(topterm);
						...
						cnf_OptimizedSkolemFormula(Search, topterm, toplabel,
								TRUE, provedterm, UsedTerms, Symblist, Result1,
			...
			if (!optimized) {
				...
				if (flag_GetFlagValue(Flags, flag_CNFSTRSKOLEM)) {
					...
					cnf_StrongSkolemization(Search, topterm, toplabel, TopAnd,
							term, UsedTerms, Symblist, Result1,
							InputClauseToTermLabellist, Depth);
				}
			}
		} else
		...
		cnf_OptimizedSkolemFormula(Search, topterm, toplabel, TopAnd, term,
				UsedTerms, Symblist, Result1, InputClauseToTermLabellist, Depth);
		return;
	} else {
		if (symbol_Equal(top, fol_And()) || symbol_Equal(top, fol_Or())) {
			...
			for (scan = term_ArgumentList(term); !list_Empty(scan); scan
					= list_Cdr(scan))
				cnf_OptimizedSkolemFormula(Search, topterm, toplabel, TopAnd,
						(TERM) list_Car(scan), UsedTerms, Symblist, Result1,
						InputClauseToTermLabellist, Depth);
		}
	}
	return;
...
static LIST cnf_OptimizedSkolemization(PROOFSEARCH Search, TERM Term,
		char* Label, LIST* UsedTerms, LIST* Symblist, BOOL result,
		BOOL Conjecture, HASH InputClauseToTermLabellist)
/**************************************************************
 ...
 **************************************************************/
{
	...
	if (symbol_IsFunction(Top) && symbol_HasProperty(Top, SKOLEM)) {
		...
		if (list_Length(*SkolToExVar) > Arity) {
			...
			for (SkolScan = (LIST) list_Car(SkolScan); (SkolScan != list_Nil())
					&& !found; SkolScan = list_Cdr(SkolScan)) {
				SYMBOL Skol;
		...
		for (Scan = term_ArgumentList(Term); Scan != list_Nil(); Scan
				= list_Cdr(Scan))
 ...
 **************************************************************/
{
	...
	for (Scan = ArgList; Scan != list_Nil(); Scan = list_Cdr(Scan)) {
		for (TermScan = TermList; TermScan != list_Nil(); TermScan = list_Cdr(
				TermScan)) {
			Term = (TERM) list_Car(TermScan);
			cnf_ReplaceVariable(Term, term_TopSymbol((TERM) list_Car(Scan)),
 ...
 **************************************************************/
{
	...
	for (Scan = Clauses; Scan != list_Nil(); Scan = list_Cdr(Scan)) {
		...
		if (!list_Empty(FreeVars)) {
			...
			list_Delete(FreeVars);
			Term = fol_CreateQuantifier(fol_All(), FreeVarsCopy,
					list_List(Term));
		}
		term_RplacArgumentList(TopTerm, list_Cons(Term, term_ArgumentList(
				TopTerm)));
	}
	...
	for (Scan = SkolToExVar; Scan != list_Nil(); Scan = list_Cdr(Scan)) {
		if (list_Empty(list_Car(Scan))) {
			if (term_TopSymbol(TopTerm) == fol_All())
				term_RplacArgumentList(TopTerm, list_Cons(term_Create(
						(SYMBOL) list_Car(VarScan), NULL), term_ArgumentList(
						TopTerm)));
			if (!First)
				TopTerm = fol_CreateQuantifier(fol_All(), list_List(
						term_Create((SYMBOL) list_Car(VarScan), NULL)),
						list_List(TopTerm));
		} else {
			LIST ExVarScan;
			LIST ExVars = list_Nil();
			for (ExVarScan = list_Car(Scan); ExVarScan != list_Nil(); ExVarScan
					= list_Cdr(ExVarScan)) {
				if (ExVars == list_Nil())
					ExVars = list_List(term_Create((SYMBOL) list_PairSecond(
							(LIST) list_Car(ExVarScan)), NULL));
				else
					ExVars = list_Cons(term_Create((SYMBOL) list_PairSecond(
							(LIST) list_Car(ExVarScan)), NULL), ExVars);
			...
			if (term_TopSymbol(TopTerm) == fol_Exist()) {
				...
				term_RplacArgumentList(VarListTerm, list_Nconc(
						term_ArgumentList(VarListTerm), ExVars));
			} else
				TopTerm = fol_CreateQuantifier(fol_Exist(), ExVars, list_List(
						TopTerm));
			ExVars = list_Nil();
 ...
 ***************************************************************/
{
	...
	for (Scan = AxiomList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (flag_GetFlagValue(Flags, flag_CNFRENAMING)) {
			...
			AxTerm = ren_Rename(AxTerm, Precedence, &SkolemPredicates,
					flag_GetFlagValue(Flags, flag_CNFPRENAMING),
					flag_GetFlagValue(Flags, flag_CNFRENMATCH),
					flag_GetFlagValue(Flags, flag_CNFRENAMING));
		}
	...
	for (Scan = ConjectureList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (list_PairFirst(list_Car(Scan)) == NULL) {
			...
			if ((flag_GetFlagValue(Flags, flag_DOCPROOF) || flag_GetFlagValue(
					Flags, flag_FLOTTER)) && flag_GetFlagValue(Flags,
					flag_PLABELS)) {
	...
	for (Scan = AllFormulae; !list_Empty(Scan); Scan = list_Cdr(Scan), Count++) {
		...
		if (flag_GetFlagValue(Flags, flag_DOCPROOF) || flag_GetFlagValue(Flags,
			...
			for (Scan2 = FormulaClausesTemp; !list_Empty(Scan2); Scan2
					= list_Cdr(Scan2)) {
				hsh_Put(InputClauseToTermLabellist, list_Car(Scan2),
						list_PairFirst(list_Car(Scan)));
			}
		}
	...
	if (!list_Empty(EmptyClauses)) {
		...
		/* may cuase performance problems */
		Result = FALSE;
	...
	*Symblist = list_Nil();
	for (Scan = AllFormulae; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (flag_GetFlagValue(Flags, flag_DOCPROOF) || flag_GetFlagValue(Flags,
				flag_FLOTTER)) {
			hsh_PutListWithCompareFunc(TermLabelToClauselist, list_PairFirst(
					Pair), list_Copy(Ax),
			...
			for (Scan2 = Ax; !list_Empty(Scan2); Scan2 = list_Cdr(Scan2)) {
				hsh_PutList(ClauseToTermLabellist, list_Car(Scan2), list_Copy(
						UsedTerms));
				hsh_PutList(InputClauseToTermLabellist, list_Car(Scan2),
						list_Copy(UsedTerms));
			}
		}
 ...
 ***************************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_CNFRENAMING)) {
		...
		Term = ren_Rename(Term, Precedence, &SkolemPredicates,
				flag_GetFlagValue(Flags, flag_CNFPRENAMING), flag_GetFlagValue(
						Flags, flag_CNFRENMATCH), flag_GetFlagValue(Flags,
						flag_CNFRENAMING));
	}
	Term = cnf_RemoveEquivImplFromFormula(Term);
	Term = cnf_NegationNormalFormula(Term);
	...
	IndexedClauses = cnf_MakeClauseList(TermCopy, FALSE, FALSE, Flags,
			Precedence);
	...
	while (!list_Empty(prfs_UsableClauses(cnf_SEARCHCOPY))) {
		prfs_MoveUsableWorkedOff(cnf_SEARCHCOPY, (CLAUSE) list_Car(
				prfs_UsableClauses(cnf_SEARCHCOPY)));
	}
	...
	ResultClauses = cnf_OptimizedSkolemization(cnf_SEARCHCOPY, term_Copy(Term),
			NULL, &Dummy, Symblist, Result, FALSE, NULL);
			...
			if (clause_IsEmptyClause(Given))
				...
				if (Derived != 0) {
					...
					Derivables = list_Nconc(Derivables,
							inf_BoundedDepthUnitResolution(Given,
									prfs_WorkedOffSharingIndex(Search), FALSE,
									Flags, Precedence));
				...
				Derivables = split_ExtractEmptyClauses(Derivables,
						&EmptyClauses);
				...
				for (Scan = Derivables; !list_Empty(Scan); Scan
						= list_Cdr(Scan))
					ClauseList = clause_InsertWeighed(list_Car(Scan),
							ClauseList, Flags, Precedence);
...
TERM cnf_DefTargetConvert(TERM Target, TERM ToTopLevel, TERM ToProveDef,
		LIST DefPredArgs, LIST TargetPredArgs, LIST TargetPredVars,
		LIST VarsForTopLevel, FLAGSTORE Flags, PRECEDENCE Precedence,
		BOOL* LocallyTrue)
/**********************************************************
...

{
	...
	*LocallyTrue = FALSE;
	...
	if (symbol_Equal(term_TopSymbol(targettoprove), fol_Or())) {
		...
		for (l1 = arglist, l2 = term_ArgumentList(orterm); !list_Empty(l1); l1
				= list_Cdr(l1), l2 = list_Cdr(l2)) {
	...
	for (l1 = vars; !list_Empty(l1); l1 = list_Cdr(l1))
		term_ExchangeVariable(ToProveDef, term_TopSymbol(list_Car(l1)),
				symbol_CreateStandardVariable());
	list_Delete(vars);

	/* Replace arguments of predicate in condition of definition by matching arguments
	 of predicate in target term */
	for (l1 = DefPredArgs, l2 = TargetPredArgs; !list_Empty(l1); l1 = list_Cdr(
			l1), l2 = list_Cdr(l2))
		term_ReplaceVariable(ToProveDef, term_TopSymbol((TERM) list_Car(l1)),
				(TERM) list_Car(l2));

	targettoprove = term_Create(fol_Not(), list_List(targettoprove));
	targettoprove = cnf_NegationNormalFormula(targettoprove);
	targettoprove = term_Create(fol_Implies(), list_Cons(targettoprove,
			list_List(ToProveDef)));
	...
	targettoprove = fol_CreateQuantifier(fol_All(), freevars, list_List(
			targettoprove));
 ...
 **********************************************************/
{
	...
	while (Term1 != TopTerm) {
		...
		while (symbol_Equal(fol_Or(), term_TopSymbol(Term1)) && (TopTerm
				!= Term1)) {
		...
		if (fol_IsQuantifier(term_TopSymbol(Term1))) {
			...
			while (!list_Empty(Scan1)) {
				...
				if (fol_VarOccursFreely(Variable, SubTerm)) {
					Scan2 = list_Cdr(Scan1);
					fol_DeleteQuantifierVariable(Term1, term_TopSymbol(
							list_Car(Scan1)));
					Scan1 = Scan2;
				} else {
			...
			if (fol_IsQuantifier(term_TopSymbol(Term1))) {
				...
				term_RplacArgumentList(Flat, list_PointerDeleteOneElement(
						term_ArgumentList(Flat), SubTerm));
				ArgList = term_ArgumentList(Term1);
				term_RplacArgumentList(Term1, list_Nil());
				Term2 = term_Create(term_TopSymbol(Term1), ArgList);
				term_RplacArgumentList(Term1, list_Cons(SubTerm, list_List(
						Term2)));
				term_RplacTop(Term1, fol_Or());
 ...
 ************************************************************/
{
	...
	Def
			= cnf_RemoveQuantFromPathAndFlatten(Def, term_Superterm(
					FoundPredicate));
	...
	if (symbol_Equal(term_TopSymbol(Def), fol_All())) {
		...
		if (symbol_Equal(term_TopSymbol(term_SecondArgument(Def)), fol_Or())
				&& (list_Length(term_ArgumentList(term_SecondArgument(Def)))
						== 1)) {
			TERM t;
			t = term_SecondArgument(Def);
			term_RplacSecondArgument(Def, term_FirstArgument(
					term_SecondArgument(Def)));
			term_Free(t);
			orterm = NULL;
		...
		/* Make sure there are several arguments */
		if (symbol_Equal(term_TopSymbol(Def), fol_Or()) && (list_Length(
				term_ArgumentList(Def)) == 1)) {
			TERM t;
			t = Def;
	...
	if (orterm != (TERM) NULL) {
		...
		if (symbol_Equal(term_TopSymbol(orterm), fol_Equiv())) {
			...
			*ToProve = cnf_NegationNormalFormula(*ToProve);
			...
			t = term_Create(fol_Not(), list_List(term_Create(fol_Or(),
	...
	if (cnf_ContainsDefinition(definition, &defpredicate)) {
		...
		while (!list_Empty(scan)) {
			...
			if ((pair != NULL) && (definition != (TERM) list_PairSecond(pair))) {
				...
				localfound = FALSE;
				if (!alwaysapplicable) {
					if (cnf_ContainsPredicate(target, term_TopSymbol(
							defpredicate), &targetpredicate, &totoplevel,
							&targettermvars, &varsfortoplevel)) {
						TERM toprovecopy;
						toprovecopy = term_Copy(toprove);
						target = cnf_DefTargetConvert(target, totoplevel,
								toprovecopy, term_ArgumentList(defpredicate),
								term_ArgumentList(targetpredicate),
								targettermvars, varsfortoplevel, Flags,
								Precedence, &localfound);
						...
						if (localfound)
							list_Rplacd(pair, (LIST) cnf_ApplyDefinitionOnce(
									defpredicate, equivterm, list_PairSecond(
											pair), targetpredicate, Flags));
					}
				} else {
					if (cnf_ContainsPredicate(target, term_TopSymbol(
							defpredicate), &targetpredicate, &totoplevel,
							&targettermvars, &varsfortoplevel))
						list_Rplacd(pair, (LIST) cnf_ApplyDefinitionOnce(
								defpredicate, equivterm, list_PairSecond(pair),
								targetpredicate, Flags));
					else
						scan = list_Cdr(scan);
					list_Delete(targettermvars);
					list_Delete(varsfortoplevel);
 ...
 ***************************************************************/
{
	...
	for (scan = term_ArgumentList(clauseterm); !list_Empty(scan); scan
			= list_Cdr(scan)) {
		BOOL isneg;
	...
	if (changed) {
		...
		result
				= cnf_MakeClauseList(clauseterm, FALSE, FALSE, Flags,
						Precedence);
		list_Delete(symblist);
