 ...
 ***************************************************************/
{
	...
	for (Scan1 = List, Scan2 = ReverseList; !list_Empty(Scan1); Scan1
 ...
 ***************************************************************/
{
	...
	if (!(list_Empty(L) || list_Empty(list_Cdr(L)))) {
		...
		do {
			...
			/* for equality first. */
			if (!Test(list_Car(Scan1), list_Car(Scan2)) && Test(
					list_Car(Scan2), list_Car(Scan1)))
 ...
 ***************************************************************/
{
	...
	for (Scan1 = List; !list_Empty(Scan1); Scan1 = list_Cdr(Scan1)) {
		Min = Scan1;
		for (Scan2 = list_Cdr(Scan1); !list_Empty(Scan2); Scan2 = list_Cdr(
				Scan2))
...
LIST list_DeleteElement(LIST List, POINTER Element, BOOL(*Test)(POINTER,
		POINTER))
/**************************************************************
...
LIST list_DeleteElementIfFree(LIST List, BOOL(*Test)(POINTER), void(*Delete)(
		POINTER))
/**************************************************************
...
LIST list_DeleteElementFree(LIST List, POINTER Element, BOOL(*Test)(POINTER,
		POINTER), void(*Free)(POINTER))
/**************************************************************
 INPUT:   A list, an element pointer, an equality test for
...
LIST list_DeleteOneElement(LIST List, POINTER Element, BOOL(*Test)(POINTER,
		POINTER))
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (scan2 = List, scan1 = list_Cdr(List); !list_Empty(scan1); scan2
...
LIST list_PointerDeleteElementFree(LIST List, POINTER Element, void(*Free)(
		POINTER))
/**************************************************************
 ...
 ***************************************************************/
{
	...
	while (!list_Empty(Scan)) {
		list_Rplacd(Scan, list_DeleteElement(list_Cdr(Scan), list_Car(Scan),
				Test));
		Scan = list_Cdr(Scan);
	}
	return List;
 ...
 ***************************************************************/
{
	...
	while (!list_Empty(Scan)) {
		list_Rplacd(Scan, list_DeleteElementFree(list_Cdr(Scan),
				list_Car(Scan), Test, Free));
		Scan = list_Cdr(Scan);
	}
	return List;
}
 ...
 ***************************************************************/
{
	...
	while (!list_Empty(Scan)) {
		list_Rplacd(Scan, list_PointerDeleteElement(list_Cdr(Scan), list_Car(
				Scan)));
		Scan = list_Cdr(Scan);
	}
	return List;
		...
		for (Scan1 = List1; !list_Empty(Scan1); Scan1 = list_Cdr(Scan1))
			for (Scan2 = List2; !list_Empty(Scan2); Scan2 = list_Cdr(Scan2))
				Result = list_Cons(list_Append(((LIST) list_Car(Scan1)),
						list_Copy((LIST) list_Car(Scan2))), Result);
	}
...
/**************************************************************
 * Ham ghep 2 xau lai voi nhau. Ghep xau 2 vao sau xau 1
 *
 INPUT:   Two lists.
...
LIST list_NMultisetDifference(LIST List1, LIST List2, BOOL(*Test)(POINTER,
		POINTER))
/**************************************************************
 ...
 ***************************************************************/
{
	if ((int) list_PairSecond(LeftPair) < (int) list_PairSecond(RightPair)) {
		return -1;
	} else if ((int) list_PairSecond(LeftPair) > (int) list_PairSecond(
			RightPair)) {
		return 1;
	}
 ...
 ***************************************************************/
{
	...
	while (!(list_Empty(scan) || list_Empty(scan2))) {
		result = list_CompareElementDistribution(list_Car(scan),
				list_Car(scan2));
		if (result != 0) {
			break;
		}
 ...
 ***************************************************************/
{
	...
	lmsd = list_Sort(lmsd,
			(BOOL(*)(POINTER, POINTER)) list_CompareElementDistributionLEQ);
	rmsd = list_Sort(rmsd,
			(BOOL(*)(POINTER, POINTER)) list_CompareElementDistributionLEQ);
