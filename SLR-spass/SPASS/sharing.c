	...
	while (!stack_Empty(B_Stack)) {
		...
		if (term_IsVariable(InsTerm)) {
			if (!sharing_IsSharedVar(InsTerm, SharedIndex)) {
				...
				sharing_SetVartableEntry(SharedIndex, sharing_VariableIndex(
						InsTerm), term_Create(term_TopSymbol(InsTerm),
						list_Nil()));
			...
			sharing_RememberSharedTermCopy(InsTerm, sharing_VartableEntry(
					SharedIndex, sharing_VariableIndex(InsTerm)));

		} else if (term_IsConstant(InsTerm)) {
			if (!sharing_IsSharedConst(InsTerm, SharedIndex)) {
				sharing_SetConsttableEntry(SharedIndex, sharing_ConstantIndex(
						InsTerm), term_Create(term_TopSymbol(InsTerm),
						list_Nil()));

				st_EntryCreate(sharing_Index(SharedIndex),
						sharing_ConsttableEntry(SharedIndex,
								sharing_ConstantIndex(InsTerm)),
						sharing_ConsttableEntry(SharedIndex,
								sharing_ConstantIndex(InsTerm)),
						cont_LeftContext());
			}
			...
			sharing_RememberSharedTermCopy(InsTerm, sharing_ConsttableEntry(
					SharedIndex, sharing_ConstantIndex(InsTerm)));
			...
			HelpList = term_SupertermList(sharing_SharedTermCopy(term_FirstArgument(InsTerm)));
			if (list_Empty(HelpList)) {
				...
				while (!DuplCandHasSameArgs && !list_Empty(HelpList)) {
					...
					if (term_TopSymbol(SharedDuplicate) == term_TopSymbol(
							InsTerm)) {
						...
						while (!list_Empty(OrigHelpArgList)
								&& (DuplCandHasSameArgs
										= (sharing_SharedTermCopy(list_First(
												OrigHelpArgList))
				...
				if (!DuplCandHasSameArgs)
					SharedDuplicate = term_Create(term_TopSymbol(InsTerm),
							list_Nil());
			...
			if (!DuplCandHasSameArgs) {
				...
				for (HelpList = term_ArgumentList(InsTerm); !list_Empty(
						HelpList); HelpList = list_Cdr(HelpList)) {
					TERM SharedArg;

					SharedArg = sharing_SharedTermCopy((TERM) list_First(
							HelpList));

					term_RplacArgumentList(SharedDuplicate, list_Cons(
							SharedArg, term_ArgumentList(SharedDuplicate)));

					term_RplacSupertermList(SharedArg, list_Cons(
							SharedDuplicate, term_SupertermList(SharedArg)));

				}

				term_RplacArgumentList(SharedDuplicate, list_NReverse(
						term_ArgumentList(SharedDuplicate)));

				/* Now a newly generated term can be inserted into the 'SharedIndex' !  */

				st_EntryCreate(sharing_Index(SharedIndex), SharedDuplicate,
						SharedDuplicate, cont_LeftContext());

			}
			sharing_RememberSharedTermCopy(InsTerm, SharedDuplicate);
		}
	}
 ...
 ***********************************************************/
{
	...
	term_RplacSupertermList(Atom, list_PointerDeleteElement(term_SupertermList(
			Atom), Data));
 ...
 ***********************************************************/
{
	...
	IsIndexed = st_EntryDelete(sharing_Index(SharedIndex), Term, Term,
			cont_LeftContext());
	...
	if (term_IsComplex(Term)) {
		...
		while (!list_Empty(Args)) {
			...
			term_RplacSupertermList(NextArg, list_PointerDeleteOneElement(
					term_SupertermList(NextArg), Term));
			if (sharing_IsNoMoreUsed(NextArg))
				sharing_DeleteFromSharing(NextArg, SharedIndex);
		}

	} else if (term_IsConstant(Term)) {
		sharing_SetConsttableEntry(SharedIndex, sharing_ConstantIndex(Term),
				NULL);
	} else {
 ...
 ********************************************************/
{
	if (term_IsAtom(Term))
		...
		for (SuperList = term_SupertermList(Term); !list_Empty(SuperList); SuperList
				= list_Cdr(SuperList)) {
			TERM superterm = (TERM) list_Car(SuperList);
			if (!term_AlreadyVisited(superterm)) {
				DataList = list_Nconc(sharing_InternGetDataList(superterm),
						DataList);
	...
	if (!list_Empty(sharing_DATALIST)) {
		...
		list_Empty(superlist)) {
			...
			do { /* Backtracking */
				...
				while (!list_Empty(superlist) && term_AlreadyVisited(list_Car(
						superlist)));
			} while ((sharing_STACKPOINTER > sharing_STACK) && list_Empty(
					superlist));
			while (!list_Empty(superlist) && !term_IsAtom(list_Car(superlist))) { /* Expansion */
 ...
 ***************************************************************/
{
	if (term_IsAtom(Term))
		...
		Result = 0;
		for (Scan = term_SupertermList(Term); !list_Empty(Scan); Scan
				= list_Cdr(Scan))
