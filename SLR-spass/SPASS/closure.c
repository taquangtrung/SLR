...
static __inline__ TERM cc_GetCar(int stack_index) {
...
static __inline__ int cc_GetCdr(int stack_index) {
	return (int) ras_Get(cc_GetCdrs(), stack_index);
}
...
static __inline__ void cc_SetCdr(int stack_index, int cdr) {
...
static __inline__ int cc_GetSize(int stack_index) {
	return (int) ras_Get(cc_GetSizes(), stack_index);
}
...
static __inline__ void cc_SetSize(int stack_index, int size) {
 ...
 ***************************************************************/
{
	...
	for (terms = term_ArgumentList(term); !list_Empty(terms); terms = list_Cdr(
			terms))
 ...
 ***************************************************************/
{
	...
	cc_SetTable(table_Init(cc_GetTable(), symbol_ActIndex() - 1, clause_MaxVar(
			clause), actno - 1));
 ...
 ***************************************************************/
{
	...
	last = clause_LastLitIndex(clause);
	for (i = clause_FirstSuccedentLitIndex(clause), result = FALSE; i <= last
			&& !result; i++) {
		atom = clause_GetLiteralAtom(clause, i);
		if (fol_IsEquality(atom))
			result = part_Equivalent(cc_GetPartition(), term_Size(
					term_FirstArgument(atom)), term_Size(term_SecondArgument(
					atom)));
		else
			result = part_Equivalent(cc_GetPartition(), term_Size(atom),
					cc_NOOFTRUE);
	}
	return result;
}
 ...
 ***************************************************************/
{
	...
	while (!ras_Empty(cc_GetPending())) {
		...
		while (!ras_Empty(cc_GetCombine()))
			cc_Union(part_Find(cc_GetPartition(), term_Size(ras_Pop(
					cc_GetCombine()))), part_Find(cc_GetPartition(), term_Size(
					ras_Pop(cc_GetCombine()))));
	}
