...
static APPROX_TRANSLATION approx_LookupTranslationSymbols(LIST Translation, SYMBOL Symbol)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (trans = NULL; trans == NULL && !list_Empty(Translation); Translation = list_Cdr(
			Translation)) {
...
static APPROX_TRANSLATION approx_CreateProjectionSymbols(SYMBOL Symbol, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	i = symbol__SYMBOLMAXLEN - (NAT) log10((double) Arity) - 2;/* maximum characters */
	pos = strlen(symbol_Name(Symbol));
	if (pos >= i)
		pos = i;
	strcpy(approx_BUFFER, symbol_Name(Symbol));
	/* If <Symbol> has arity <n> create <n> new symbols */
	for (i = 1; i <= Arity; ++i) {
		Suffix = i;
		/* Ensure that new symbol wasn't previously defined */
		do {
			sprintf(approx_BUFFER + pos, "%u", Suffix++);
	...
	if (LitIndex <= clause_LastAntecedentLitIndex(Clause)) {
		...
		for (i = clause_FirstAntecedentLitIndex(Clause); i <= clause_LastAntecedentLitIndex(Clause); i++) {
			if (i != LitIndex) {
				...
				for (Scan = Translation->newSymbols, ArgScan = term_ArgumentList(Atom); !list_Empty(
		...
		for (Scan = Translation->newSymbols, ArgScan = term_ArgumentList(Atom); !list_Empty(Scan); Scan
			...
			clause_SetLiteral(NewClause, LitIndex, clause_LiteralCreate(Atom, NewClause));
			clause_Normalize(NewClause);
			clause_UpdateWeight(NewClause, Flags);
			clause_UpdateMaxVar(NewClause);
...
static LIST approx_ApplyProjectionToList(LIST* Translation, LIST Clauses, FLAGSTORE Flags,
 ...
 ***************************************************************/
{
	...
	for (Hit = FALSE, i = clause_FirstLitIndex(); !Hit && i <= clause_LastLitIndex(Clause); i++) {
	...
	if (!Hit) {
		...
		for (; !list_Empty(Clauses); Clauses = list_Pop(Clauses)) {
			Result = list_Nconc(Result, approx_ApplyProjection(Trans, list_Car(Clauses), i, Flags,
					Precedence));
...
LIST approx_MonadicByProjection(LIST Clauses, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (; !list_Empty(Clauses); Clauses = list_Cdr(Clauses)) {
		Result = list_Nconc(Result, approx_ApplyProjectionToList(&Translation, list_List(
				clause_Copy(list_Car(Clauses))), Flags, Precedence));
	}
...
static APPROX_TRANSLATION approx_CreateFunctionForPredicate(SYMBOL Symbol, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	i = symbol__SYMBOLMAXLEN - 3; /* maximum characters */
	pos = strlen(symbol_Name(Symbol));
	if (pos >= i)
		pos = i;
	strcpy(approx_BUFFER, symbol_Name(Symbol));
	strcpy(approx_BUFFER + pos, "_f");
	/* If the predicate symbol name + "_f" is already defined, try additional numbers */
	i = 0;
	while (symbol_Lookup(approx_BUFFER) != 0) {
		sprintf(approx_BUFFER + pos + 2, "%u", ++i);
	}
	Predicate = symbol_CreateFunction(approx_BUFFER, symbol_Arity(Symbol), symbol_STATLEX,
			Precedence);
	Result->newSymbols = list_Cons((POINTER) Predicate, Result->newSymbols);
	return Result;
}

static void approx_EncodeClause(LIST* Translation, CLAUSE Clause, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
...
LIST approx_MonadicByTermEncoding(LIST Clauses, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A list of clauses, a flag store and a precedence.
 ...
 ***************************************************************/
{
	...
	if (symbol_Equal(approx_TRUESYMBOL, symbol_Null())) {
		i = 0;
		Length = strlen(approx_TRUENAME);
		strcpy(approx_BUFFER, approx_TRUENAME);
		while (symbol_Lookup(approx_BUFFER) != 0) {
			sprintf(approx_BUFFER + Length, "%u", ++i);
		}
		approx_TRUESYMBOL = symbol_CreatePredicate(approx_BUFFER, 1, symbol_STATLEX, Precedence);
	}
 ...
 ***************************************************************/
{
	...
	for (i = clause_LastSuccedentLitIndex(Clause); i >= clause_FirstSuccedentLitIndex(Clause); --i) {
	...
	for (i = clause_LastAntecedentLitIndex(Clause); i >= clause_FirstConstraintLitIndex(Clause); --i) {
...
LIST approx_MonadicMakeLinear(LIST Clauses, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A list of clauses, a flag store and a precedence.
 ...
 ***************************************************************/
{
	if (list_PointerMember(Duplicates, Term))
		...
		for (Scan = Arguments; !list_Empty(Scan); Scan = list_Cdr(Scan))
			list_Rplaca(Scan, approx_ReplaceSubterms(list_Car(Scan), Duplicates, NewVarTerm));
 ...
 ***************************************************************/
{
	...
	strcpy(approx_BUFFER, "Fl");
	/* Ensure that new symbol wasn't previously defined */
	do {
		sprintf(approx_BUFFER + 2, "%u", ++approx_PREDICATECOUNTER);
	} while (symbol_Lookup(approx_BUFFER) != 0);
...
static LIST approx_GetDuplicatesAndCheckVariables(TERM Atom, TERM ComplexTerm, BOOL CheckVariables)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	do {
		if (term_Equal(Atom, ComplexTerm))
			...
			if (CheckVariables && term_IsVariable(Atom) && term_VarIsMarked(term_TopSymbol(Atom),
					ActMark)) {
...
static LIST approx_OneFlatteningStep(CLAUSE Clause, BOOL CheckVarCondition,
		BOOL CheckNegativeLiterals, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
	...
	do {
		...
		if (!stack_Empty(Stack)) {
			...
			if (term_IsComplex(Term)) { /* replace only complex terms */
				LIST duplicates;
				duplicates = approx_GetDuplicatesAndCheckVariables(Atom, Term, CheckVarCondition);
				if (list_Empty(duplicates))
					/* Var condition had to be checked but was violated */
					stack_Push(term_ArgumentList(Term));
				else {
					/* Apply rule */
					Result = approx_ApplyFlattening(Clause, duplicates, CheckNegativeLiterals,
							Flags, Precedence);
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstConstraintLitIndex(Clause); i <= clause_LastAntecedentLitIndex(Clause); ++i) {
	...
	do {
		...
		NewClauses = approx_OneFlatteningStep(ActClause, CheckVarCondition, CheckNegativeLiterals,
				Flags, Precedence);
 ...
 ***************************************************************/
{
	...
	if (!clause_IsHornClause(Clause)) {
		...
		for (i = clause_LastConstraintLitIndex(Clause); i >= clause_FirstConstraintLitIndex(Clause); --i)
			Constraint = list_Cons(clause_GetLiteralAtom(Clause, i), Constraint);
		/* Collect antecedent atoms */
		for (i = clause_LastAntecedentLitIndex(Clause); i >= clause_FirstAntecedentLitIndex(Clause); --i)
