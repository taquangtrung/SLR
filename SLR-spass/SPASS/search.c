 ...
 ***************************************************************/
{
	...
	for (Scan = prfs_UsableClauses(Search); !list_Empty(Scan); Scan = list_Cdr(
		...
		if (!clause_IsClause(Clause, prfs_Store(Search),
				prfs_Precedence(Search)) || clause_GetFlag(Clause, WORKEDOFF)
				|| !prfs_IsClauseValid(Clause, prfs_ValidLevel(Search)))
			return FALSE;
	}

	for (Scan = prfs_WorkedOffClauses(Search); !list_Empty(Scan); Scan
			= list_Cdr(Scan)) {
		Clause = (CLAUSE) list_Car(Scan);
		if (!clause_IsClause(Clause, prfs_Store(Search),
				prfs_Precedence(Search)) || !clause_GetFlag(Clause, WORKEDOFF)
	...
	for (Scan = prfs_SplitStack(Search); !list_Empty(Scan); Scan = list_Cdr(
			Scan)) {
		Split = (SPLIT) list_Car(Scan);
		if (prfs_SplitIsUsed(Split)) {
			if (!list_Empty(prfs_SplitBlockedClauses(Split)) || !list_Empty(
					prfs_SplitDeletedClauses(Split))) {
				/*putchar('\n');prfs_PrintSplit(Split); putchar('\n');*/
				return FALSE;
			} else {
				for (Clauses = prfs_UsableClauses(Search); !list_Empty(Clauses); Clauses
						= list_Cdr(Clauses))
					if (clause_SplitLevel(list_Car(Clauses))
							== prfs_SplitSplitLevel(Split)) {
						/*puts("\n");prfs_PrintSplit(Split);
						 fputs("\n Clause must not exist: ",stdout);
						 clause_Print(list_Car(Clauses)); putchar('\n');*/
						return FALSE;
					}
				for (Clauses = prfs_WorkedOffClauses(Search); !list_Empty(
						Clauses); Clauses = list_Cdr(Clauses))
					if (clause_SplitLevel(list_Car(Clauses))
							== prfs_SplitSplitLevel(Split)) {
						/*puts("\n");prfs_PrintSplit(Split);
						 fputs("\n Clause must not exist: ",stdout);
	...
	if (prfs_ValidLevel(Search) == 0) {
		...
		if (prfs_ValidLevel(Search) != prfs_SplitSplitLevel(prfs_SplitStackTop(
				Search)))
			return FALSE;
	}
 ...
 ***************************************************************/
{
	if ((prfs_DynamicSortTheory(Search) != (SORTTHEORY) NULL
		...
		for (i = clause_FirstSuccedentLitIndex(Clause); i < l; i++) {
			...
			if (clause_LiteralIsMaximal(lit) && symbol_IsBaseSort(
					term_TopSymbol(clause_LiteralSignedAtom(lit)))) {
				if (prfs_DynamicSortTheory(Search) != (SORTTHEORY) NULL) {
					...
					sort_TheoryInsertClause(prfs_DynamicSortTheory(Search),
							Clause, copy, clause_GetLiteral(copy, i));
				}
				if (prfs_ApproximatedDynamicSortTheory(Search)
						!= (SORTTHEORY) NULL) {
					approx = sort_ApproxMaxDeclClauses(Clause, prfs_Store(
							Search), prfs_Precedence(Search));
					for (; !list_Empty(approx); approx = list_Pop(approx)) {
						copy = (CLAUSE) list_Car(approx);
						sort_TheoryInsertClause(
								prfs_ApproximatedDynamicSortTheory(Search),
								Clause, copy, clause_GetLiteral(copy,
 ...
 ***************************************************************/
{
	if (clause_IsDeclarationClause(Clause)) {
		...
		if (prfs_ApproximatedDynamicSortTheory(Search) != (SORTTHEORY) NULL)
			sort_TheoryDeleteClause(prfs_ApproximatedDynamicSortTheory(Search),
					Clause);
	}
}
 ...
 ***************************************************************/
{
	...
	clause_DeleteSharedClauseList(prfs_UsableClauses(Search),
			prfs_UsableSharingIndex(Search), prfs_Store(Search),
			prfs_Precedence(Search));
	clause_DeleteSharedClauseList(prfs_DocProofClauses(Search),
			prfs_DocProofSharingIndex(Search), prfs_Store(Search),
			prfs_Precedence(Search));
...
/**************************************************************
 * Doi cho Usable clauses va Worked-off clauses
 *
 *
 INPUT:   A proof search object.
 RETURNS: Nothing.
 ...
 ***************************************************************/
{
	...
	/* If a DocProof index is required but not yet allocated in SearchCopy,
	 do it now */
	if (prfs_DocProofSharingIndex(Search) != NULL && prfs_DocProofSharingIndex(
			SearchCopy) == NULL)
	...
	for (Scan = prfs_UsableClauses(Search); !list_Empty(Scan); Scan = list_Cdr(
			Scan))
		prfs_InsertUsableClause(SearchCopy,
				clause_Copy((CLAUSE) list_Car(Scan)));

	for (Scan = prfs_WorkedOffClauses(Search); !list_Empty(Scan); Scan
			= list_Cdr(Scan))
		prfs_InsertWorkedOffClause(SearchCopy, clause_Copy((CLAUSE) list_Car(
				Scan)));

	for (Scan = prfs_DocProofClauses(Search); !list_Empty(Scan); Scan
			= list_Cdr(Scan))
		prfs_InsertDocProofClause(SearchCopy, clause_Copy((CLAUSE) list_Car(
				Scan)));
}

void prfs_InsertWorkedOffClause(PROOFSEARCH Search, CLAUSE Clause)
/**************************************************************
 * insert clause vao worked-off clauses list trong PROOFSEARCH (ca list va index)
 *
 *
 INPUT:   A proof search object and a clause.
 RETURNS: Nothing.
 ...
 ***************************************************************/
{
	...
	prfs_SetWorkedOffClauses(Search, list_Cons(Clause, prfs_WorkedOffClauses(
			Search)));
 ...
 ***************************************************************/
{
	...
	if (prfs_DocProofSharingIndex(Search) == (SHARED_INDEX) NULL)
		...
		prfs_SetDocProofClauses(Search, list_Cons(Clause, prfs_DocProofClauses(
				Search)));
...
/**************************************************************
 * Delete a clause from usable clauses set, then inseart that claues to workoff set
 *
 INPUT:   A proof search object and a clause.
 RETURNS: Nothing.
 EFFECT:  The clause is inserted into the worked off sharing index
 ...
 ***************************************************************/
{
	...
	prfs_SetUsableClauses(Search, list_PointerDeleteElement(prfs_UsableClauses(
			Search), Clause));
	clause_SetFlag(Clause, WORKEDOFF);
 ...
 ***************************************************************/
{
	...
	prfs_SetWorkedOffClauses(Search, list_PointerDeleteElement(
			prfs_WorkedOffClauses(Search), Clause));
...
/**************************************************************
 * Remove 1 clause ra khoi danh sach Usable clause
 *
 *
 INPUT:   A proof search object and a clause.
 RETURNS: Nothing.
 ...
 ***************************************************************/
{
	...
	prfs_SetDocProofClauses(Search, list_PointerDeleteElement(
			prfs_DocProofClauses(Search), Clause));
 ...
 ***************************************************************/
{
	...
	for (Scan = prfs_SplitBlockedClauses(Split); !list_Empty(Scan); Scan
			= list_Cdr(Scan)) {
		putchar('\n');
		putchar(' ');
	...
	for (Scan = prfs_SplitDeletedClauses(Split); !list_Empty(Scan); Scan
			= list_Cdr(Scan)) {
		putchar('\n');
		putchar(' ');
		clause_Print(list_Car(Scan));
 ...
 ***************************************************************/
{
	...
	printf(
			...
			prfs_ValidLevel(Search), prfs_LastBacktrackLevel(Search),
			prfs_SplitCounter(Search), prfs_Loops(Search),
			prfs_BacktrackedClauses(Search));
	...
	for (Scan = prfs_EmptyClauses(Search); !list_Empty(Scan); Scan = list_Cdr(
			Scan)) {
		fputs("\n ", stdout);
		clause_Print(list_Car(Scan));
	}
	fputs("\n Definitions:", stdout);
	for (Scan = prfs_Definitions(Search); !list_Empty(Scan); Scan = list_Cdr(
			Scan)) {
		putchar('\n');
		putchar(' ');
	...
	for (Scan = prfs_GetFinMonPreds(Search); !list_Empty(Scan); Scan
			= list_Cdr(Scan)) {
		fputs("\n  ", stdout);
 ...
 ***************************************************************/
{
	...
	clause_SetNumOfConsLits(BlockedClause, (clause_NumOfConsLits(SplitClause)
			- nc));
	clause_SetNumOfAnteLits(NewClause, na);
	clause_SetNumOfAnteLits(BlockedClause, (clause_NumOfAnteLits(SplitClause)
			- na));
	clause_SetNumOfSuccLits(NewClause, ns);
	clause_SetNumOfSuccLits(BlockedClause, (clause_NumOfSuccLits(SplitClause)
			- ns));
	...
	clause_SetParentLiterals(BlockedClause, list_NReverse(
			clause_ParentLiterals(BlockedClause)));
	...
	clause_SetParentLiterals(NewClause, list_NReverse(clause_ParentLiterals(
			NewClause)));
	...
	if (prfs_SplitCounter(PS) != 0) {
		...
		if (clause_HasSuccLits(Clause)) {
			if (clause_HasGroundSuccLit(Clause)) {
				...
				for (i = clause_LastAntecedentLitIndex(Clause); i
			...
			NextLit = clause_GetLiteral(Clause, clause_LastSuccedentLitIndex(
					Clause));
			...
			while (Change) {
				...
				for (i = clause_LastSuccedentLitIndex(Clause) - 1; i
						>= clause_FirstLitIndex(); i--) {

					NextLit = clause_GetLiteral(Clause, i);

					if (!list_PointerMember(LitList, NextLit)) {
						NextOcc = term_VariableSymbols(clause_LiteralAtom(
								NextLit));
			...
			for (i = clause_FirstSuccedentLitIndex(Clause); i < length
					&& Change; i++)
 ...
 ***************************************************************/
{
	...
	for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		Clause = (CLAUSE) list_Car(Scan);
		if (clause_Length(Clause) == 1 && clause_NumOfSuccLits(Clause) == 1) {
			Atom = clause_GetLiteralAtom(Clause, clause_FirstSuccedentLitIndex(
					Clause));
			if (list_PointerMember(Predicates, (POINTER) term_TopSymbol(Atom))) {
				Pair = list_AssocListPair(Result,
						(POINTER) term_TopSymbol(Atom));
				if (Pair != list_PairNull())
					list_PairRplacSecond(Pair, list_Cons(term_Copy(
							term_FirstArgument(Atom)), list_PairSecond(Pair)));
				else
					Result = list_AssocCons(Result, (POINTER) term_TopSymbol(
							Atom), list_List(
 ...
 ***************************************************************/
{
	...
	if (fol_IsEquality(Atom)) {
		...
		if (clause_LiteralIsOrientedEquality(Literal)) {
			NrOfInstances += sharing_GetNumberOfInstances(term_FirstArgument(
					Atom), WOIndex);
			if (Usables)
				NrOfInstances += sharing_GetNumberOfInstances(
						term_FirstArgument(Atom), UsIndex);
		}
	}
