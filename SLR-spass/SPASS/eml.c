...
{ 
  ...
  eml_NARYSYMBOLS = (SYMBOL **)memory_Malloc(sizeof(SYMBOL) * symbol__MAXSIGNATURE *
		  ...
		  */
  if ( (flag_GetFlagValue(Flags, flag_EMLTR) == flag_EMLTRSEMIFUNC) || 
       !flag_GetFlagValue(Flags, flag_EMLFNARY)) {
...
{
  SYMBOL foSymbol;
  char   newName[symbol__SYMBOLMAXLEN];
  ...
  if (eml_NaryType(SymbolType)) {
    ...
    sprintf(newName,"%s%d%s", eml_PREDNAMEPREFIX[SymbolType], Arity,
...
***************************************************************/
{
  ...
  if (flag_GetFlagValue(Flags, flag_EMLELIMCOMP)) {
...
SYMBOL eml_CreateNarySymbol(SYMBOL PropSymbol, eml_SYMBOLTYPE SymbolType,
		char* NameSuffix, int Arity, int Status, PRECEDENCE Precedence)
/*******************************************************
  INPUT:   A symbol (which MUST be a propositonal symbol/nullary
	   predicate symbol), a symbol type (of the fo symbol to be
...
********************************************************/
{
  ...
  sprintf(newName,"%s%s_%d%s", eml_PREDNAMEPREFIX[SymbolType],
		  symbol_Name(PropSymbol), Arity, NameSuffix);
...
***************************************************************/
{
  ...
  if (flag_GetFlagValue(Flags, flag_EMLFFSORTS))  /* with explicit sorts */
...
***************************************************************/
{
  ...
  if (flag_GetFlagValue(Flags, flag_EML2REL)) {
    TERM newVarY;

    Term = eml_EmbedInRelCalculusProp(Term);
    if (flag_GetFlagValue(Flags, flag_EMLPTRANS)) {
  ...
  else if (flag_GetFlagValue(Flags, flag_EMLFNARY)) {
    char nameSuffix[symbol__SYMBOLMAXLEN] = "";

    flag_SetFlagValue(Flags, flag_EMLFFSORTS, flag_EMLFFSORTSOFF);  
...
***************************************************************/
{
  ...
  /* Term = dia(r,phi)  or  Term = box(r,phi) */
    ...
    LIST newVarList;
    char newSortEncoding[symbol__SYMBOLMAXLEN];
    ...
    if (flag_GetFlagValue(Flags, flag_EMLFUNCNDEQ)) {
      helpTerm = eml_BuildSortQuantifierFor(Term, Top, term_TopSymbol(relSubTerm),
		term_TopSymbol(newVar), flag_GetFlagValue(Flags, flag_EMLFFSORTS), 
      ...
      anotherHelpTerm = eml_BuildSortQuantifierFor(Term, Top, 
		  ...
		  flag_GetFlagValue(Flags, flag_EMLFFSORTS),
...
***************************************************************/
{
  ...
  /* Term = r */
  if (symbol_IsPredicate(Top)) {
    switch (flag_GetFlagValue(Flags, flag_EMLTHEORY)) {
      ...
      default:
        fprintf(stderr, "\nThe option EMLTheory = %d is not implemented in eml_FunctTranslItoNaryPredRel()\n",
             flag_GetFlagValue(Flags, flag_EMLTHEORY));
        misc_Error();
        return NULL;
        break;
    }
...
***************************************************************/
{
  ...
  if (ItoNaryPreds) {
    ...
    /* memory_Free(nameSuffix, symbol__SYMBOLMAXLEN); */
    return  Term;
...
***************************************************************/
{
  ...
  if (eml_IsPropositional(Term)) {
    switch (flag_GetFlagValue(Flags, flag_EMLTR)) {
      case flag_EMLTRREL:
        Term = eml_RelationalTranslation(Term, Flags, Precedence);
        break;
      case flag_EMLTRFUNC:
      case flag_EMLTROPTFUNC:
        Term = eml_FunctionalTranslation(Term, flag_GetFlagValue(Flags, flag_EMLFNARY), Flags, Precedence);
        break;
      ...
      default:
        fprintf(stderr, "\nFlag = %d is not implemented in eml_TranslateToFol()\n",
             flag_GetFlagValue(Flags, flag_EMLTR));
        misc_Error();
        break;
    }
  }
...
{   
  ...
  if ( (flag_GetFlagValue(Flags, flag_EMLTHEORY) != flag_EMLTHEORYK) && 
       (flag_GetFlagValue(Flags, flag_EMLTR) == flag_EMLTRREL) ) {
    ...
    for (i = 1; i < symbol_ActIndex(); i++) {
      if (!symbol_IsFreed(i)) {
        ...
        if ( symbol_IsPredicate(symbol) &&
          ...
          flag = flag_GetFlagValue(Flags, flag_EMLTHEORY);
          switch (flag) {
...
***************************************************************/
{
  ...
  flag_emltr = flag_GetFlagValue(Flags, flag_EMLTR);
  if (flag_GetFlagValue(Flags, flag_DOCPROOF) || flag_GetFlagValue(Flags, flag_EMLPTRANS)) {
	  flag_CheckEmlTranslationFlags(Flags, flag_emltr);
  }

  /* Set additional flags */
  if (flag_emltr == flag_EMLTROPTFUNC) {
    flag_SetFlagValue(Flags, flag_QUANTEXCH, flag_QUANTEXCHON);
  }
   ...
   * if (flag_GetFlagValue(Flags, flag_QUANTEXCH)) {      
    ...
    flag_SetFlagValue(Flags, flag_CNFSTRSKOLEM, flag_CNFSTRSKOLEMOFF);
  } */
  /*
  ...
  if ( !list_Empty(Scan) && !list_Empty(list_Car(Scan)) ) {
    if ( flag_emltr == flag_EMLTROPTFUNC ) {
      ...
      if ( flag_GetFlagValue(Flags, flag_EMLFNARY) ) {
        fprintf(stderr, "ERROR: With the current implementation of the translation routines\n");
