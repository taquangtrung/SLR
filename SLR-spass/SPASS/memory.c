...
static MEMORY_RESOURCE memory_PAGES[memory__DYNMAXSIZE / memory__SHAREDPAGES
		+ 1];
 ...
 *************************************************************/
{
	 ...
	 */
	while (memory_PAGESIZE < (2 * (memory__DYNMAXSIZE + memory_MARKSIZE)
			+ extra)) {
	...
	for (i = 1; i < memory__DYNMAXSIZE; i++) {
		...
		/* last block�s offset */
		CurrentResource->offset = ((memory_PAGESIZE - extra) / TotalSize)
				* TotalSize + sizeof(POINTER) + memory_OFFSET;
	}
}
 ...
 *********************************************************/
{
	...
	if (Bytes >= memory__DYNMAXSIZE) {
		...
		RealBigBlockSize = sizeof(MEMORY_BIGBLOCKHEADERNODE) + memory_MARKSIZE
				+ memory_CalculateRealBlockSize(Bytes);
		...
		if (memory_MAXMEM >= 0) {
			...
			if ((unsigned int) memory_MAXMEM < RealBigBlockSize) {
				...
				misc_UserErrorReport("\n Memory Error.");
				misc_UserErrorReport(
						" Terminated by user given memory restriction,\n");
				misc_UserErrorReport(
		...
		if (*((int *) Resource->free) != EOF) {
			...
			if (Resource->next != Resource->end_of_page) {
				...
				if (NewMemory == NULL) {
					...
					misc_UserErrorReport("\n Memory Error.");
					misc_UserErrorReport(
							" Terminated, ran out of system memory.\n");
#ifdef CHECK
					misc_UserErrorReport("\n Error occurred in memory_Malloc");
					misc_UserErrorReport(" called from file %s at line %d.\n",
							File, Line);
 ...
 **********************************************************/
{
	...
	if (Size >= memory__DYNMAXSIZE) {
		...
		BigBlockHeader = (MEMORY_BIGBLOCKHEADER) ((char *) Freepointer
				- memory_OFFSET - sizeof(MEMORY_BIGBLOCKHEADERNODE));
 ...
 ***************************************************************/
{
	...
	if (memory_BIGBLOCKS != NULL) {
		...
		for (thisblock = memory_BIGBLOCKS; thisblock != NULL; thisblock
				= nextblock) {
 ...
 ***************************************************************/
{
	...
	if (Size >= memory__DYNMAXSIZE) {
		...
		misc_UserErrorReport("\n Parameter size is too big: %d.", Size);
 ...
 ***************************************************************/
{
	...
	for (i = 1; i < memory__DYNMAXSIZE; i++) {
		...
		printf("\n\n Entry: %d aligned size: %d total size: %d\n", i,
				Resource->aligned_size, BlockSize);
