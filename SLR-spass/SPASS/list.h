...
typedef struct LIST_HELP {
	struct LIST_HELP *cdr;				//	phan con lai cua list 		(Contents of Decrement Register - theo ngon ngu LISP)
	POINTER car;						// 	phan tu dau tien cua list  	(Contents of Address Register - theo ngon ngu LISP)
} LIST_NODE;
...
static __inline__ BOOL list_Empty(LIST L) {
	/*
	 * Kiem tra list co phan tu nao ko???
	 *
	 * Tra ve TRUE neu list rong
	 */
...
LIST list_DeleteElementFree(LIST, POINTER, BOOL(*)(POINTER, POINTER), void(*)(
		POINTER));
...
LIST list_DeleteDuplicatesFree(LIST, BOOL(*)(POINTER, POINTER),
		void(*)(POINTER));
...
static __inline__ BOOL list_Member(LIST List, POINTER Element, BOOL(*Test)(
		POINTER, POINTER))
/**************************************************************
