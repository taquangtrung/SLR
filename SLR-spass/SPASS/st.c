 ...
 ***************************************************************/
{
	for (; list_Exist(NodeList); NodeList = list_Cdr(NodeList)) {
		...
		for (Subst = ((st_INDEX) list_Car(NodeList))->subst; subst_Exist(Subst); Subst
				= subst_Next(Subst))
		...
		if (!st_IsLeaf((st_INDEX) list_Car(NodeList)))
			st_CloseUsedVariables(Context,
					((st_INDEX) list_Car(NodeList))->subnodes);
	}
}
...
static st_INDEX st_FirstVariant(const CONTEXT Context, LIST Subnodes,
 ...
 ***************************************************************/
{
	...
	for (EmptyVariant = NULL, *BestNonVariant = NULL; list_Exist(Subnodes); Subnodes
			= list_Cdr(Subnodes)) {
...
void st_EntryCreate(st_INDEX StIndex, POINTER Pointer, TERM Term,
 ...
 ***************************************************************/
{
	...
	/* FIND "LAST" VARIATION */
	while (!st_IsLeaf(Current) && (Current = st_FirstVariant(Context,
			Current->subnodes, &BestNonVariant))) {
		if (st_Max(Current) < MinMax)
			st_SetMax(Current, MinMax);
		else if (st_Min(Current) > MinMax)
			st_SetMin(Current, MinMax);
	...
	if (cont_BindingsSinceLastStart() == 0 && Current && st_IsLeaf(Current)) {
		...
		ComGen = subst_ComGen(Context, BestNonVariant->subst, &SubstOld,
				&SubstNew);

		st_NodeAddInner(BestNonVariant, SubstOld, subst_CloseOpenVariables(
				SubstNew), ComGen, MinMax, Pointer);

	} else

		/* ADD A SINGLE LEAF NODE TO FATHER */
		StIndex->subnodes = list_Cons(st_NodeAddLeaf(subst_CloseOpenVariables(
				subst_Nil()), MinMax, Pointer), StIndex->subnodes);
 ...
 ***************************************************************/
{
	if (st_IsLeaf(StIndex)) {
		...
		*Found = list_DeleteFromList(&(StIndex->entries), Pointer);
		...
		for (Subnodes = StIndex->subnodes; list_Exist(Subnodes); Subnodes
				= list_Cdr(Subnodes)) {
			st_INDEX CurrentNode;
			...
			if (subst_Variation(Context, CurrentNode->subst)) {
				list_Rplaca(Subnodes, st_EntryDeleteHelp(Context, CurrentNode,
						Pointer, Found));
				if (*Found) {
					...
					for (Subnodes = list_Cdr(StIndex->subnodes); list_Exist(
							Subnodes); Subnodes = list_Cdr(Subnodes)) {
						CurrentNode = (st_INDEX) list_Car(Subnodes);
...
BOOL st_EntryDelete(st_INDEX StIndex, POINTER Pointer, TERM Term,
		const CONTEXT Context)
/**************************************************************
 INPUT:
 RETURNS:
 ...
 ***************************************************************/
{
	...
	for (Found = FALSE, Subnodes = StIndex->subnodes; list_Exist(Subnodes); Subnodes
			= list_Cdr(Subnodes)) {
		st_INDEX CurrentNode;
		...
		if (subst_Variation(Context, CurrentNode->subst)) {
			...
			if (Found) {
				StIndex->subnodes = list_PointerDeleteElement(
						StIndex->subnodes, NULL);
 ...
 ***************************************************************/
{
	...
	for (;;) {
		...
		/* DESCENDING */
		for (CurrentNode = (st_INDEX) list_Car(CurrentList); subst_Match(
 ...
 ***************************************************************/
{
	...
	for (;;) {
		...
		/* DESCENDING */
		for (CurrentNode = (st_INDEX) list_Car(CurrentList); subst_MatchReverse(
				IndexContext, CurrentNode->subst); CurrentList
				= CurrentNode->subnodes, CurrentNode = (st_INDEX) list_Car(
				CurrentList))
			if (st_IsLeaf(CurrentNode)) {
...
static LIST st_TraverseTreeGenPreTest(CONTEXT IndexContext, st_INDEX StIndex,
		st_MINMAX MinMax)
/**************************************************************
 INPUT:
 RETURNS:
 ...
 ***************************************************************/
{
	...
	for (;;) {
		...
		/* DESCENDING */
		for (CurrentNode = (st_INDEX) list_Car(CurrentList); (MinMax >= st_Min(
				CurrentNode)) && subst_Match(IndexContext, CurrentNode->subst); CurrentList
				= CurrentNode->subnodes, CurrentNode = (st_INDEX) list_Car(
				CurrentList))
			if (st_IsLeaf(CurrentNode)) {
				Result = list_Append(CurrentNode->entries, Result);
...
static LIST st_TraverseTreeInstancePreTest(CONTEXT IndexContext,
		st_INDEX StIndex, st_MINMAX MinMax)
/**************************************************************
 INPUT:
 RETURNS:
 EFFECTS:
 ***************************************************************/
{
	...
	for (;;) {
		...
		/* DESCENDING */
		for (CurrentNode = (st_INDEX) list_Car(CurrentList); (MinMax <= st_Max(
				CurrentNode)) && subst_MatchReverse(IndexContext,
				CurrentNode->subst); CurrentList = CurrentNode->subnodes, CurrentNode
				= (st_INDEX) list_Car(CurrentList))
			if (st_IsLeaf(CurrentNode)) {
				Result = list_Append(CurrentNode->entries, Result);
				break;
			} else if (list_Cdr(CurrentList)) {
...
LIST st_GetUnifier(CONTEXT IndexContext, st_INDEX StIndex, CONTEXT TermContext,
		TERM Term)
/**************************************************************
 INPUT:
 RETURNS:
 ...
 ***************************************************************/
{
	...
	Result = st_TraverseTreeGenPreTest(IndexContext, StIndex, term_ComputeSize(
			Term));
 ...
 ***************************************************************/
{
	...
	Result = st_TraverseTreeInstancePreTest(IndexContext, StIndex,
			term_ComputeSize(Term));
 ...
 ***************************************************************/
{
	...
	for (;;) {
		...
		/* DESCENDING */
		for (CurrentNode = (st_INDEX) list_Car(CurrentList); (st_EXIST_MINMAX
				>= st_Min(CurrentNode)) && subst_Match(IndexContext,
				CurrentNode->subst); CurrentList = CurrentNode->subnodes, CurrentNode
				= (st_INDEX) list_Car(CurrentList)) {
			if (list_Exist(list_Cdr(CurrentList))) {
				st_StackPush(list_Cdr(CurrentList));
 ...
 ***************************************************************/
{
	...
	for (;;) {
		...
		/* DESCENDING */
		for (CurrentNode = (st_INDEX) list_Car(CurrentList); (st_EXIST_MINMAX
				<= st_Max(CurrentNode)) && subst_MatchReverse(IndexContext,
				CurrentNode->subst); CurrentList = CurrentNode->subnodes, CurrentNode
				= (st_INDEX) list_Car(CurrentList)) {
			if (list_Exist(list_Cdr(CurrentList))) {
				st_StackPush(list_Cdr(CurrentList));
...
POINTER st_ExistUnifier(CONTEXT IndexContext, st_INDEX StIndex,
 ...
 ***************************************************************/
{
	...
	if (st_Exist(StIndex)) {
		...
		FirstDomain = symbol_FirstIndexVariable();
		cont_CreateBinding(IndexContext, FirstDomain, cont_InstanceContext(),
				Term);
...
POINTER st_ExistInstancePreTest(CONTEXT IndexContext, st_INDEX StIndex,
		TERM Term)
/**************************************************************
 INPUT:
 RETURNS:
	...
	if (list_Exist(Result)) {
		...
		if (st_WHICH_CONTEXTS == st_STANDARD)
			...
			case st_INSTANCEPRETEST:
				NewResult
						= st_TraverseForExistInstancePreTest(st_INDEX_CONTEXT);
