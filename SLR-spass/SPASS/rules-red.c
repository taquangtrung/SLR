 ...
 **********************************************************/
{
	...
	for (Scan = Blocked; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (prfs_SplitLevelCondition(clause_SplitLevel(RedClause),
				clause_SplitLevel(Clause), prfs_LastBacktrackLevel(Search)))
			split_DeleteClauseAtLevel(Search, Clause, clause_SplitLevel(
					RedClause));
		else {
			...
			} else if (flag_GetFlagValue(Flags, flag_DOCPROOF))
 ...
 **********************************************************/
{
	...
	for (Scan = Blocked; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (prfs_SplitLevelCondition(clause_SplitLevel(RedClause),
				clause_SplitLevel(Clause), prfs_LastBacktrackLevel(Search))) {
			split_KeepClauseAtLevel(Search, Clause,
 ...
 ********************************************************/
{
	...
	for (i = clause_FirstConstraintLitIndex(Clause); i <= end; i++) {
		...
		if (fol_IsEquality(Atom) && !clause_LiteralIsOrientedEquality(
				clause_GetLiteral(Clause, i)) && term_Equal(term_FirstArgument(
			...
			for (j = i + 1; j <= end; j++) {
				...
				if (term_Equal(PartnerAtom, Atom) || (fol_IsEquality(Atom)
						...
						term_FirstArgument(Atom), term_SecondArgument(
								PartnerAtom)) && term_Equal(term_FirstArgument(
						PartnerAtom), term_SecondArgument(Atom)))) {
					Indexes = list_Cons((POINTER) i, Indexes);
					j = end;
				}
			}
	...
	if (clause_Length(Clause) == 1 && clause_NumOfAnteLits(Clause) == 1
			&& !list_PointerMember(Indexes,
					(POINTER) clause_FirstAntecedentLitIndex(Clause))
			&& fol_IsEquality(clause_GetLiteralAtom(Clause,
		...
		if (unify_UnifyCom(cont_LeftContext(), term_FirstArgument(
				clause_LiteralAtom(clause_GetLiteral(Clause,
						clause_FirstAntecedentLitIndex(Clause)))),
				term_SecondArgument(clause_LiteralAtom(clause_GetLiteral(
						Clause, clause_FirstAntecedentLitIndex(Clause))))))
			Indexes = list_Cons(
					(POINTER) clause_FirstAntecedentLitIndex(Clause), Indexes);
		cont_BackTrack();
	}
...
static BOOL red_AssignmentEquationDeletion(CLAUSE Clause, FLAGSTORE Flags,
		PRECEDENCE Precedence, CLAUSE *Changed, NAT NonTrivClauseNumber,
		BOOL NonTrivDomain)
/**********************************************************
 ...
 ***********************************************************/
{
	...
	if (clause_ContainsNegativeEquations(Clause)) {
		for (i = clause_FirstAntecedentLitIndex(Clause); i
				<= clause_LastAntecedentLitIndex(Clause); i++) {
			if (clause_LiteralIsEquality(clause_GetLiteral(Clause, i))) {
				LeftArg = term_FirstArgument(clause_GetLiteralAtom(Clause, i));
				RightArg
						= term_SecondArgument(clause_GetLiteralAtom(Clause, i));
				if ((term_IsVariable(LeftArg)
						&& clause_NumberOfSymbolOccurrences(Clause,
								term_TopSymbol(LeftArg)) == 1)
						|| (term_IsVariable(RightArg)
								&& clause_NumberOfSymbolOccurrences(Clause,
										term_TopSymbol(RightArg)) == 1))
					Indexes = list_Cons((POINTER) i, Indexes);
			}
		}
	} else if (NonTrivDomain && clause_ContainsPositiveEquations(Clause)) {
		for (i = clause_FirstSuccedentLitIndex(Clause); i
				<= clause_LastSuccedentLitIndex(Clause); i++) {
			if (clause_LiteralIsEquality(clause_GetLiteral(Clause, i))) {
				LeftArg = term_FirstArgument(clause_GetLiteralAtom(Clause, i));
				RightArg
						= term_SecondArgument(clause_GetLiteralAtom(Clause, i));
	...
	if (!list_Empty(Indexes)) {
		...
		if (flag_GetFlagValue(Flags, flag_DOCPROOF)) {
			...
			Copy = clause_Copy(Clause);
			clause_DeleteLiterals(Copy, Indexes, Flags, Precedence);
			red_DocumentAssignmentEquationDeletion(Copy, Indexes,
					NonTrivClauseNumber);
			if (flag_GetFlagValue(Flags, flag_PAED))
				clause_Print(Copy);
			*Changed = Copy;
 ...
 ***********************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(Clause); i < n && !Result; i++)
		if (symbol_Equal(term_TopSymbol(clause_LiteralAtom(clause_GetLiteral(
				Clause, i))), fol_True()))
			Result = TRUE;
 ...
 ***********************************************************/
{
	...
	for (j = clause_FirstSuccedentLitIndex(Clause); j < n && !Result; j++) {
		...
		if (fol_IsEquality(Atom) && !clause_LiteralIsOrientedEquality(
				clause_GetLiteral(Clause, j)) && term_Equal(term_FirstArgument(
				Atom), term_SecondArgument(Atom)))
			Result = TRUE;

		for (i = clause_FirstLitIndex(); i <= la && !Result; i++)
			if (term_Equal(Atom, clause_LiteralAtom(
					clause_GetLiteral(Clause, i))))
				Result = TRUE;
	}

	if (!Result && flag_GetFlagValue(Flags, flag_RTAUT) == flag_RTAUTSEMANTIC
 ...
 ***************************************************************/
{
	...
	CandTerm = st_ExistGen(cont_LeftContext(), sharing_Index(ShIndex),
			...
			while (!list_Empty(LitScan)) {
				...
				if (clause_LiteralsAreComplementary(ActLit, NextLit))
					if (clause_Length(clause_LiteralOwningClause(NextLit)) == 1
							|| subs_SubsumesBasic(clause_LiteralOwningClause(
									NextLit), ActClause,
...
static void red_DocumentMatchingReplacementResolution(CLAUSE Clause,
		LIST LitInds, LIST ClauseNums, LIST PLitInds)
/*********************************************************
...
static BOOL red_MatchingReplacementResolution(CLAUSE Clause,
		SHARED_INDEX ShIndex, FLAGSTORE Flags, PRECEDENCE Precedence,
 ...
 ***************************************************************/
{
	...
	while (i < length) {
		...
		clause_LiteralIsPositive(ActLit)) {
			...
			if (clause_LiteralExists(PLit)) {
				...
				ReducedBy = list_Cons((POINTER) clause_Number(PClause),
						ReducedBy);
				PLits
						= list_Cons((POINTER) clause_LiteralGetIndex(PLit),
								PLits);
				ReducedLits = list_Cons((POINTER) (i + j), ReducedLits);
				if (Copy == Clause && (Document || prfs_SplitLevelCondition(
						clause_SplitLevel(PClause), clause_SplitLevel(Copy),
						Level)))
					Copy = clause_Copy(Clause);
	...
	if (!list_Empty(ReducedBy)) {
		if (Document) {
			...
			red_DocumentMatchingReplacementResolution(Copy, ReducedLits,
					ReducedBy, PLits); /* Lists are consumed */
			if (flag_GetFlagValue(Flags, flag_PMRR)) {
				fputs(" ==> [ ", stdout);
				for (Scan1 = ReducedBy, Scan2 = ReducedLits; !list_Empty(Scan1); Scan1
						= list_Cdr(Scan1), Scan2 = list_Cdr(Scan2))
					printf("%d.%d ", (NAT) list_Car(Scan1), (NAT) list_Car(
							Scan2));
				fputs("] ", stdout);
				clause_Print(Copy);
			}
 ...
 **********************************************************/
{
	list_Delete(clause_ParentClauses(Clause));
	list_Delete(clause_ParentLiterals(Clause));
	clause_SetParentClauses(Clause, list_Nconc(list_List(
			(POINTER) clause_Number(Clause)), ClauseNums));
	clause_SetParentLiterals(Clause, list_Nconc(LitInds, PLitInds));
...
static BOOL red_UnitConflict(CLAUSE Clause, SHARED_INDEX ShIndex,
		FLAGSTORE Flags, PRECEDENCE Precedence, CLAUSE *Changed, int Level)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (clause_Length(Clause) == 1) {
		...
		PLit = (LITERAL) NULL;
		CandTerm = st_ExistUnifier(cont_LeftContext(), sharing_Index(ShIndex),
				cont_RightContext(), clause_LiteralAtom(ActLit));
			...
			if (!term_IsVariable(CandTerm)) {
				...
				while (!list_Empty(Scan)) {
					...
					if (clause_LiteralsAreComplementary(ActLit, PLit)
							&& clause_Length(clause_LiteralOwningClause(PLit))
									== 1) {
						st_CancelExistRetrieval();
						Scan = list_Nil();
					} else {
		...
		if (PLit == (LITERAL) NULL
				&& fol_IsEquality(clause_LiteralAtom(ActLit))) {
			TERM Atom;
			Atom = term_Create(fol_Equality(), list_Reverse(term_ArgumentList(
					clause_LiteralAtom(ActLit))));
			CandTerm = st_ExistUnifier(cont_LeftContext(), sharing_Index(
					ShIndex), cont_RightContext(), Atom);
			while (PLit == (LITERAL) NULL && CandTerm) {
				if (!term_IsVariable(CandTerm)) {
					Scan = sharing_NAtomDataList(CandTerm); /* CAUTION !!! the List is not a copy */
					while (!list_Empty(Scan)) {
		...
		if (clause_LiteralExists(PLit)) {
			...
			PClause = clause_LiteralOwningClause(PLit);
			if (Copy == Clause
					&& (Document || prfs_SplitLevelCondition(clause_SplitLevel(
							PClause), clause_SplitLevel(Copy), Level)))
				Copy = clause_Copy(Clause);
			clause_UpdateSplitDataFromPartner(Copy, PClause);
			clause_DeleteLiteral(Copy, clause_FirstLitIndex(), Flags,
					Precedence);
			if (Document)
				red_DocumentUnitConflict(Copy, list_List(
						(POINTER) clause_FirstLitIndex()), list_List(
						(POINTER) clause_Number(PClause)), list_List(
						(POINTER) clause_FirstLitIndex()));
			if (flag_GetFlagValue(Flags, flag_PUNC)) {
				printf(" ==> [ %d.%d ]", clause_Number(PClause),
			...
			if (!term_IsVariable(AtomGen)) {
				for (LitScan = sharing_NAtomDataList(AtomGen); !list_Empty(
					...
					((i <= lc && clause_LiteralIsFromConstraint(CandLit)) || (i
							...
							CandLit)) || (i >= fs
							&& clause_LiteralIsFromSuccedent(CandLit)))
				...
				if (!term_IsVariable(AtomGen)) {
					for (LitScan = sharing_NAtomDataList(AtomGen); !list_Empty(
						...
						((i <= lc && clause_LiteralIsFromConstraint(CandLit))
								|| (i >= fa && i <= la
										&& clause_LiteralIsFromAntecedent(
												CandLit)) || (i >= fs
								&& clause_LiteralIsFromSuccedent(CandLit)))
								&& subs_SubsumesBasic(CandCl, RedCl,
										clause_FirstLitIndex(), i)) {
 ...
 ***********************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_PSUB) && Subsumer) {
		...
		printf(" by %d %d ", clause_Number(Subsumer), clause_SplitLevel(
				Subsumer));
	}
 ...
 **********************************************************/
{
	list_Delete(clause_ParentClauses(Clause));
	list_Delete(clause_ParentLiterals(Clause));
	clause_SetParentClauses(Clause, list_List((POINTER) clause_Number(Clause)));
...
static void red_DocumentFurtherRewriting(CLAUSE Clause, int i, CLAUSE Rule,
		int ri)
/*********************************************************
 INPUT:   Two clauses and the literal indices involved in the rewrite step.
 RETURNS: Nothing.
 ...
 **********************************************************/
{
	clause_AddParentClause(Clause, (int) list_Car(list_Cdr(
 ...
 ***************************************************************/
{
	...
	if (term_StampOverflow(red_STAMPID))
		term_ResetTermStamp(clause_LiteralSignedAtom(clause_GetLiteral(
				RedClause, clause_FirstLitIndex())));
	term_StartStamp();
	...
	while (Rewritten) {
		...
		while (!stack_Empty(B_Stack)) {
			...
			TermS = st_ExistGen(cont_LeftContext(), sharing_Index(ShIndex),
					RedTermS);
			while (TermS && !Rewritten) {
				EqList = term_SupertermList(TermS);
				for (EqScan = EqList; !list_Empty(EqScan) && !Rewritten; EqScan
						= list_Cdr(EqScan)) {
					...
					if (fol_IsEquality(PartnerEq)) {
						...
						for (LitScan = sharing_NAtomDataList(PartnerEq); !list_Empty(
							...
							RewriteClause = clause_LiteralOwningClause(
									RewriteLit);
							if (clause_LiteralIsPositive(RewriteLit)
									&& clause_Length(RewriteClause) == 1) {
								Oriented = (clause_LiteralIsOrientedEquality(
										RewriteLit) && TermS
								...
								if (Oriented) {
									...
									if (RedClause == Copy
										...
										RedAtom = clause_GetLiteralAtom(Copy,
												clause_FirstLitIndex());
									}
									...
									if (Document) {
										if (!Result)
											red_DocumentRewriting(Copy,
													clause_FirstLitIndex(),
													RewriteClause,
													clause_FirstLitIndex());
										else
											red_DocumentFurtherRewriting(Copy,
													clause_FirstLitIndex(),
													RewriteClause,
													clause_FirstLitIndex());
									}
									Result = TRUE;
									TermT = cont_ApplyBindingsModuloMatching(
											cont_LeftContext(),
											term_Copy(Right), TRUE);
									if (cont_BindingsAreRenamingModuloMatching(
											cont_RightContext()))
										term_SetTermSubtermStamp(TermT);
									term_ReplaceSubtermBy(RedAtom, RedTermS,
											TermT);
									Rewritten = TRUE;
									clause_UpdateSplitDataFromPartner(Copy,
											RewriteClause);
									term_Delete(TermT);
	...
	for (ci = clause_FirstAntecedentLitIndex(RedClause); ci < length; ci++) {
		...
		if (!list_Empty(term_ArgumentList(clause_GetLiteralAtom(Copy, ci)))) {
			while (Rewritten) {
				...
				while (!stack_Empty(B_Stack)) {
					RedTermS = (TERM) stack_PopResult();
					TermS = st_ExistGen(cont_LeftContext(), sharing_Index(
							ShIndex), RedTermS);
...
static BOOL red_LeftTermOfEquationIsStrictlyMaximalTerm(CLAUSE Clause,
		LITERAL Equation, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
	...
	for (i = clause_FirstLitIndex(); i <= last; i++) {
		if (i != except) {
			...
			if (fol_IsEquality(Atom)) {
				/* Atom is an equation */
				if (ord_Compare(LeftTerm, term_FirstArgument(Atom), Flags,
						Precedence) != ord_GREATER_THAN
						|| (!clause_LiteralIsOrientedEquality(ActLit)
								&& ord_Compare(LeftTerm, term_SecondArgument(
										Atom), Flags, Precedence)
										!= ord_GREATER_THAN))
				...
				if (ord_Compare(LeftTerm, Atom, Flags, Precedence)
						!= ord_GREATER_THAN)
					return FALSE;
			}
		}
...
static void red_CRwCalculateAdditionalParents(CLAUSE Reduced,
		LIST RedundantClauses, CLAUSE Subsumer, int OriginalClauseNumber)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	Parents
			= list_PointerDeleteElement(Parents, (POINTER) OriginalClauseNumber);
	for (Scan = RedundantClauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
 ...
 ***************************************************************/
{
	...
	if (fol_IsEquality(Atom) && !clause_LiteralIsOrientedEquality(Literal)
			&& (term_IsVariable(term_FirstArgument(Atom)) || term_IsVariable(
					term_SecondArgument(Atom))) && !term_VariableEqual(
			term_FirstArgument(Atom), term_SecondArgument(Atom)))
		return TRUE;
	else
		return FALSE;
}
	...
	for (i = clause_FirstAntecedentLitIndex(Clause); i <= last; i++) {
		...
		if (red_LiteralIsDefinition(Lit)) {
			...
			if (term_IsVariable(Term)) {
				...
				Var = term_TopSymbol(term_SecondArgument(
						clause_LiteralAtom(Lit)));
			}
			...
			for (j = clause_FirstLitIndex(); j <= lj && success; j++) {
				if (j != i) {
					...
					if (fol_IsEquality(Atom)) {
						/* Atom is an equation */
						if (ord_ContGreater(cont_InstanceContext(),
								LeadingTerm, cont_LeftContext(),
								term_FirstArgument(Atom), Flags, Precedence)
						...
						/* Atom is not an equation */
						if (ord_ContGreater(cont_InstanceContext(),
								LeadingTerm, cont_LeftContext(), Atom, Flags,
								Precedence))
							success = TRUE;
					}
				}
 ...
 ***************************************************************/
{
	...
	if (i <= clause_LastConstraintLitIndex(RuleClause)) {
		...
		if (red_SortSimplification(prfs_DynamicSortTheory(Search), aux,
				NAT_MAX, DocProof, Flags, Precedence, &NewClause)) {
...
static BOOL red_CRwTautologyCheck(PROOFSEARCH Search, CLAUSE RedClause, int i,
		TERM TermSInstance, CLAUSE RuleClause, int j, NAT Mode, CLAUSE *Result)
/**************************************************************
 INPUT:   A proof search object, two clauses, two literal indices
 ...
 ***************************************************************/
{
	...
	*Result = NULL;
	...
	/* are bound. */
	cont_ApplyBindingsModuloMatching(cont_LeftContext(), clause_GetLiteralAtom(
			RuleCopy, j), TRUE);
	...
	if (i >= clause_FirstSuccedentLitIndex(RedClause)
			...
			&& ord_LiteralCompare(clause_GetLiteralTerm(RedClause, i),
					clause_LiteralIsOrientedEquality(clause_GetLiteral(
							RedClause, i)), clause_GetLiteralTerm(RuleCopy, j),
					TRUE, FALSE, Flags, Precedence) != ord_GREATER_THAN) {
		cont_BackTrack();
	 ...
	 } else  */
	{
		...
		for (h = clause_FirstLitIndex(); Rewrite && h <= last; h++) {
			if (h != j) {
				aux = red_CRwLitTautologyCheck(Search, RedClause, i, RuleCopy,
						h, TermSInstance, Mode);
				if (aux == NULL)
					Rewrite = FALSE;
				else {
					...
					if (flag_GetFlagValue(Flags, flag_DOCPROOF)) {
						clause_SetParentClauses(RuleCopy, list_Nconc(
								clause_ParentClauses(aux),
								clause_ParentClauses(RuleCopy)));
						clause_SetParentLiterals(RuleCopy, list_Nconc(
								clause_ParentLiterals(aux),
...
static void red_DocumentContextualRewriting(CLAUSE Clause, int i,
		CLAUSE RuleClause, int ri, LIST AdditionalPClauses,
...
static void red_DocumentFurtherCRw(CLAUSE Clause, int i, CLAUSE RuleClause,
		int ri, LIST AdditionalPClauses, LIST AdditionalPLits)
/**************************************************************
							...
							for (; !list_Empty(EqScan) && !Rewritten; EqScan
								...
								if (fol_IsEquality(PartnerEq)
									...
									for (LitScan = sharing_NAtomDataList(
										...
										if (clause_LiteralIsPositive(RuleLit)
												&& clause_LiteralGetFlag(
														RuleLit, STRICTMAXIMAL)
												&& clause_LiteralIsOrientedEquality(
														RuleLit)
												&& red_LeftTermOfEquationIsStrictlyMaximalTerm(
														RuleClause, RuleLit,
														Flags, Precedence)
												&& red_CRwTautologyCheck(
														Search, Copy, ri,
														RedTermS, RuleClause,
														i, Mode, &HelpClause)) {
											TERM TermT;
 ...
 ***********************************************************/
{
	...
	Atom = clause_GetLiteralAtom(RedCl, i);
	CandTerm = st_ExistInstance(cont_LeftContext(), sharing_Index(ShIndex),
			Atom);
	...
	while (CandTerm) {
		...
		for (Scan = CandLits; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
			...
			if (RedCl != SubsumedCl &&
			/* Literals must be from same part of the clause */
			((i <= lc && clause_LiteralIsFromConstraint(CandLit)) || (i >= fa
					&& i <= la && clause_LiteralIsFromAntecedent(CandLit))
		...
		while (CandTerm) {
			...
			for (Scan = CandLits; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
				...
				((i <= lc && clause_LiteralIsFromConstraint(CandLit)) || (i
						...
						CandLit))) && !list_PointerMember(SubsumedList,
						SubsumedCl) && subs_SubsumesBasic(RedCl, SubsumedCl, i,
						j))
					SubsumedList = list_Cons(SubsumedCl, SubsumedList);
				/* } */
			}
...
static LIST red_GetBackMRResLits(CLAUSE Clause, LITERAL ActLit,
		SHARED_INDEX ShIndex)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	while (CandTerm) {
		...
		for (; !list_Empty(LitScan); LitScan = list_Cdr(LitScan)) {
			...
			if (PClause != Clause && clause_LiteralsAreComplementary(ActLit,
					PLit) && subs_SubsumesBasic(Clause, PClause, i,
					clause_LiteralGetIndex(PLit)))
...
static LIST red_BackMatchingReplacementResolution(CLAUSE RedClause,
		SHARED_INDEX ShIndex, FLAGSTORE Flags, PRECEDENCE Precedence,
		LIST* Result)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (clause_Length(RedClause) == 1) {
		...
		clause_LiteralIsNegative(ActLit)) {
			...
			CandTerm = st_ExistInstance(cont_LeftContext(), sharing_Index(
					ShIndex), clause_LiteralAtom(ActLit));
			RedClNum = clause_Number(RedClause);
			LitList = list_Nil();

			while (CandTerm) {
				for (Iter = sharing_NAtomDataList(CandTerm); !list_Empty(Iter); Iter
						= list_Cdr(Iter))
			...
			while (!list_Empty(LitList)) {
				...
				if (flag_GetFlagValue(Flags, flag_PMRR)) {
					...
					printf(" ==>[ %d.%d ] ", clause_Number(RedClause),
							clause_FirstLitIndex());
				}

				Iter = LitList;
				for (Scan = list_Cdr(LitList); !list_Empty(Scan); Scan
						= list_Cdr(Scan)) /* Get brothers of PLit */
				...
				for (Scan = PIndL; !list_Empty(Scan); Scan = list_Cdr(Scan)) /* Change lits to indexes */
					list_Rplaca(Scan, (POINTER) clause_LiteralGetIndex(
							list_Car(Scan)));
				clause_DeleteLiterals(Copy, PIndL, Flags, Precedence);

				if (Document)
					/* Lists are consumed */
					red_DocumentMatchingReplacementResolution(Copy, PIndL,
							list_List((POINTER) RedClNum), list_List(
									(POINTER) clause_FirstLitIndex()));
		...
		for (i = clause_FirstLitIndex(); i < length; i++) {
			...
			if (!fol_IsEquality(clause_LiteralAtom(ActLit))) {
				...
				for (LitScan = LitList; !list_Empty(LitScan); LitScan
						= list_Cdr(LitScan)) {
					PLit = list_Car(LitScan);
					PClause = clause_LiteralOwningClause(PLit);
					...
					if (Document)
						red_DocumentMatchingReplacementResolution(Copy,
								list_List((POINTER) PInd), list_List(
										(POINTER) RedClNum), list_List(
										(POINTER) i));
...
static void red_ApplyRewriting(CLAUSE RuleCl, int ri, CLAUSE PartnerClause,
		int pli, TERM PartnerTermS, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	ReplaceTermT = cont_ApplyBindingsModuloMatchingReverse(cont_LeftContext(),
 ...
 ***************************************************************/
{
	...
	while (CandTerm) {
		...
		if (!term_IsVariable(CandTerm) && !symbol_IsPredicate(term_TopSymbol(
				CandTerm))) {
			LIST LitList;
...
static LIST red_BackRewriting(CLAUSE RedClause, SHARED_INDEX ShIndex,
		FLAGSTORE Flags, PRECEDENCE Precedence, LIST* Result)
/**************************************************************
	...
	for (; !list_Empty(Inst); Inst = list_Pop(Inst)) {
		...
		if (!term_IsVariable(CandTerm) && !symbol_IsPredicate(term_TopSymbol(
			...
			for (; !list_Empty(LitList); LitList = list_Pop(LitList)) {
				...
				/* Partner literal must be from antecedent or succedent */
				if (clause_Number(RuleClause) != clause_Number(RedClause) && ri
						>= clause_FirstAntecedentLitIndex(RedClause) &&
				...
				!list_PointerMember(Blocked, RedClause)
						&& red_CRwTautologyCheck(Search, RedClause, ri,
								CandTerm, RuleClause, i, Mode, &HelpClause)) {
					CLAUSE Copy;
					...
					ReplaceTermT = cont_ApplyBindingsModuloMatching(
							cont_LeftContext(), term_Copy(term_SecondArgument(
									clause_GetLiteralTerm(RuleClause, i))),
							TRUE);
					cont_BackTrack();
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(RuleClause); i <= ls && !found; i++) {
		Lit = clause_GetLiteral(RuleClause, i);
		if (clause_LiteralIsOrientedEquality(Lit) && clause_LiteralGetFlag(Lit,
				STRICTMAXIMAL) && red_LeftTermOfEquationIsStrictlyMaximalTerm(
				RuleClause, Lit, Flags, Precedence)) {
			Blocked = list_Nconc(red_BackCRwOnLiteral(Search, RuleClause, Lit,
					i, Mode, Result), Blocked);
 ...
 **********************************************************/
{
	...
	for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		Declarations = list_Cons((POINTER) clause_FirstSuccedentLitIndex(
				list_Car(Scan)), Declarations);
...
static BOOL red_SortSimplification(SORTTHEORY Theory, CLAUSE Clause, NAT Level,
		BOOL Document, FLAGSTORE Flags, PRECEDENCE Precedence, CLAUSE *Changed)
/**********************************************************
 ...
 ***********************************************************/
{
	if (Theory != (SORTTHEORY) NULL) {
		...
		while (i <= lc) {
			...
			SortPair = sort_ComputeSortNoResidues(Theory, Term, Copy, i, Flags,
					Precedence);
			...
			if ((Cond = sort_TheoryIsSubsortOfNoResidues(Theory, TermSort,
					LitSort)) != (CONDITION) NULL) {
				...
				NewClauses
						= list_Nconc(NewClauses, sort_ConditionClauses(Cond));
				sort_ConditionPutClauses(Cond, list_Nil());
				sort_ConditionDelete(Cond);

				for (Scan = NewClauses; !list_Empty(Scan); Scan
						= list_Cdr(Scan)) {
					if (Clause == Copy && (Document
							|| prfs_SplitLevelCondition(clause_SplitLevel(
 ...
 ***********************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_RTAUT) != flag_RTAUTOFF && red_Tautology(
			*RedClause, Flags, Precedence))
		return TRUE;
 ...
 ***********************************************************/
{
	...
	Index = (red_OnlyWorkedOffMode(Mode) ? prfs_WorkedOffSharingIndex(Search)
	...
	if (flag_GetFlagValue(Flags, flag_RAED) != flag_RAEDOFF
			&& red_AssignmentEquationDeletion(
					*Clause,
					Flags,
					Precedence,
					&Copy,
	...
	if ((flag_GetFlagValue(Flags, flag_RFREW) && red_RewriteRedClause(*Clause,
			Index, Flags, Precedence, &Copy, prfs_LastBacktrackLevel(Search)))
			|| (flag_GetFlagValue(Flags, flag_RFCRW)
					&& red_ContextualRewriting(Search, *Clause, Mode,
							prfs_LastBacktrackLevel(Search), &Copy))) {
		red_ExchangeClauses(Clause, &Copy, Result);
		Redundant = red_SimpleStaticReductions(Clause, Flags, Precedence,
				Result);
		if (Redundant)
			return Redundant;
		if (clause_IsEmptyClause(*Clause))
			return FALSE;
		if (flag_GetFlagValue(Flags, flag_RFSUB)) {
			*Subsumer = red_ForwardSubsumption(*Clause, Index, Flags,
					Precedence);
			if ((Redundant = (*Subsumer != (CLAUSE) NULL)))
				return Redundant;
		}
	...
	if (red_OnlyWorkedOffMode(Mode) && flag_GetFlagValue(Flags, flag_RSSI)) {
		red_SortSimplification(prfs_DynamicSortTheory(Search), *Clause,
				prfs_LastBacktrackLevel(Search), flag_GetFlagValue(Flags,
						flag_DOCPROOF), Flags, Precedence, &Copy);
		red_ExchangeClauses(Clause, &Copy, Result);
		if (clause_IsEmptyClause(*Clause))
			return FALSE;
	}
	...
	if (flag_GetFlagValue(Flags, flag_RUNC)) {
		red_UnitConflict(*Clause, Index, Flags, Precedence, &Copy,
 ...
 ***********************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_RFSUB)) {
		*Subsumer = (CLAUSE) NULL;
		if (WoIndex != NULL) {
			*Subsumer = red_ForwardSubsumption(*Clause, WoIndex, Flags,
					Precedence);
			if (*Subsumer != (CLAUSE) NULL)
				return TRUE;
		}
		if (UsIndex != NULL) {
			*Subsumer = red_ForwardSubsumption(*Clause, UsIndex, Flags,
					Precedence);
			if (*Subsumer != (CLAUSE) NULL)
				return TRUE;
		}
	...
	while (Rewritten) {
		...
		if (WoIndex != NULL && ((StandardRew && red_RewriteRedClause(*Clause,
				WoIndex, Flags, Precedence, &Copy, prfs_LastBacktrackLevel(
						Search))) || (ContextualRew && red_ContextualRewriting(
				Search, *Clause, red_WORKEDOFF,
				prfs_LastBacktrackLevel(Search), &Copy)))) {
			Rewritten = TRUE;
			red_ExchangeClauses(Clause, &Copy, Result);
			Redundant = red_SimpleStaticReductions(Clause, Flags, Precedence,
	...
	if (flag_GetFlagValue(Flags, flag_RFMRR)) {
		if (WoIndex)
			red_MatchingReplacementResolution(*Clause, WoIndex, Flags,
					Precedence, &Copy, prfs_LastBacktrackLevel(Search));
		red_ExchangeClauses(Clause, &Copy, Result);
		if (clause_IsEmptyClause(*Clause))
			return FALSE;
		if (UsIndex)
			red_MatchingReplacementResolution(*Clause, UsIndex, Flags,
					Precedence, &Copy, prfs_LastBacktrackLevel(Search));
		red_ExchangeClauses(Clause, &Copy, Result);
		if (clause_IsEmptyClause(*Clause))
			return FALSE;
	...
	if (flag_GetFlagValue(Flags, flag_RUNC)) {
		if (WoIndex)
			red_UnitConflict(*Clause, WoIndex, Flags, Precedence, &Copy,
					prfs_LastBacktrackLevel(Search));
		red_ExchangeClauses(Clause, &Copy, Result);
		if (clause_IsEmptyClause(*Clause))
			return FALSE;
		if (UsIndex)
			red_UnitConflict(*Clause, UsIndex, Flags, Precedence, &Copy,
					prfs_LastBacktrackLevel(Search));
		red_ExchangeClauses(Clause, &Copy, Result);
		if (clause_IsEmptyClause(*Clause))
			return FALSE;
...
CLAUSE red_ReductionOnDerivedClause(PROOFSEARCH Search, CLAUSE Clause, NAT Mode)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (red_StaticReductions(Search, &Clause, &RedClause, &Redundant, Mode)) {
		...
		if (RedClause && prfs_SplitLevelCondition(clause_SplitLevel(RedClause),
				clause_SplitLevel(Clause), prfs_LastBacktrackLevel(Search))) {
			split_KeepClauseAtLevel(Search, Clause,
					clause_SplitLevel(RedClause));
		} else if (flag_GetFlagValue(prfs_Store(Search), flag_DOCPROOF))
			prfs_InsertDocProofClause(Search, Clause);
		else
			clause_Delete(Clause);
		Clause = (CLAUSE) NULL;
 ...
 ***************************************************************/
{
	...
	if (red_SelectedStaticReductions(Search, &Clause, &RedClause, &Redundant,
			Mode)) {
		/* <Clause> is redundant */
		red_HandleRedundantDerivedClauses(Search, Redundant, Clause);
		list_Delete(Redundant);
 ...
 ***************************************************************/
{
	...
	if (flag_GetFlagValue(Flags, flag_RBSUB)) {
		Redundant = list_Nil();
		if (red_WorkedOffMode(Mode))
			Redundant = red_BackSubsumption(Clause, prfs_WorkedOffSharingIndex(
					Search), Flags, Precedence);
		if (red_UsableMode(Mode))
			Redundant = list_Nconc(Redundant, red_BackSubsumption(Clause,
	...
	if (flag_GetFlagValue(Flags, flag_RBMRR)) {
		...
		if (red_WorkedOffMode(Mode))
			Redundant = red_BackMatchingReplacementResolution(Clause,
					prfs_WorkedOffSharingIndex(Search), Flags, Precedence,
					&Result);
		if (red_UsableMode(Mode))
	...
	if (flag_GetFlagValue(Flags, flag_RBREW)) {
		Redundant = list_Nil();
		if (red_WorkedOffMode(Mode))
			Redundant = red_BackRewriting(Clause, prfs_WorkedOffSharingIndex(
					Search), Flags, Precedence, &Result);
		if (red_UsableMode(Mode))
			Redundant = list_Nconc(Redundant,
					red_BackRewriting(Clause, prfs_UsableSharingIndex(Search),
	...
	if (flag_GetFlagValue(Flags, flag_RBCRW)) {
		Redundant = list_Nil();
		if (red_WorkedOffMode(Mode))
			Redundant = red_BackContextualRewriting(Search, Clause,
					red_WORKEDOFF, &Result);
		if (red_UsableMode(Mode))
			Redundant = list_Nconc(Redundant, red_BackContextualRewriting(
...
LIST red_CompleteReductionOnDerivedClauses(PROOFSEARCH Search,
		LIST DerivedClauses, NAT Mode, int Bound, NAT BoundMode,
		int *BoundApplied)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	while (!list_Empty(DerivedClauses)) {
		...
		if (Clause != NULL && BoundMode != flag_BOUNDMODEUNLIMITED && Bound
				!= flag_BOUNDSTARTUNLIMITED && !clause_IsFromInput(Clause)
...
//
				...
				NewClauses = list_NumberSort(NewClauses,
						(NAT(*)(POINTER)) clause_Weight);
				DerivedClauses = red_MergeClauseListsByWeight(DerivedClauses,
						NewClauses);
			}
		}
	}
 ...
 ***********************************************************/
{
	...
	for (i = 0; i < length; i++) {
		...
		while (AtomGen) {
			for (LitScan = term_SupertermList(AtomGen); !list_Empty(LitScan); LitScan
					= list_Cdr(LitScan)) {
				CandCl = clause_LiteralOwningClause(list_Car(LitScan));

				if (clause_GetLiteral(CandCl, clause_FirstLitIndex())
						== (LITERAL) list_Car(LitScan) && subs_Subsumes(CandCl,
						RedCl, clause_FirstLitIndex(), i)) {
					st_CancelExistRetrieval();
					return CandCl;
				}
			}
 ...
 ***********************************************************/
{
	...
	if (clause_Exists(Subsumer)) {
		...
		if (flag_GetFlagValue(Flags, flag_DOCSST) && flag_GetFlagValue(Flags,
				flag_PSUB)) {
			fputs("\nFSubsumption:", stdout);
			clause_Print(RedClause);
...
static void red_CDBackSubsumption(CLAUSE RedCl, FLAGSTORE Flags,
		PRECEDENCE Precedence, LIST* UsListPt, LIST* WOListPt, st_INDEX Index)
/**********************************************************
 ...
 ***********************************************************/
{
	...
	if (!clause_IsEmptyClause(RedCl)) {
		...
		while (AtomInst) {
			for (Scan = term_SupertermList(AtomInst); !list_Empty(Scan); Scan
					= list_Cdr(Scan)) {
				SubsumedCl = clause_LiteralOwningClause(list_Car(Scan));
				if ((RedCl != SubsumedCl) && subs_Subsumes(RedCl, SubsumedCl,
						clause_FirstLitIndex(), clause_LiteralGetIndex(
								list_Car(Scan))) && !list_PointerMember(
						SubsumedList, SubsumedCl))
					SubsumedList = list_Cons(SubsumedCl, SubsumedList);
			}
		...
		for (Scan = SubsumedList; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
			...
			if (flag_GetFlagValue(Flags, flag_DOCSST) && flag_GetFlagValue(
					Flags, flag_PSUB)) {
				fputs("\nBSubsumption: ", stdout);
				clause_Print(SubsumedCl);
				printf(" by %d ", clause_Number(RedCl));
 ...
 ***************************************************************/
{
	...
	if (clause_HasTermSortConstraintLits(GivenClause))
		ListOfDerivedClauses = inf_ForwardSortResolution(GivenClause,
				sort_TheoryIndex(Theory), Theory, TRUE, Flags, Precedence);
	else
...
static BOOL red_CDReduce(SORTTHEORY Theory, CLAUSE RedClause, FLAGSTORE Flags,
		PRECEDENCE Precedence, LIST *ApproxUsListPt, LIST *ApproxWOListPt,
		st_INDEX Index)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	red_SortSimplification(Theory, RedClause, NAT_MAX, FALSE, Flags,
			Precedence, &Copy);
	...
	if (red_CDForwardSubsumption(RedClause, Index, Flags, Precedence))
		...
		red_CDBackSubsumption(RedClause, Flags, Precedence, ApproxUsListPt,
				ApproxWOListPt, Index);
 ...
 ***************************************************************/
{
	if (Theory != (SORTTHEORY) NULL) {
		...
		LIST ApproxUsableList, ApproxWOList, EmptyClauses, ApproxDerivables,
				Scan;
		...
		if (clause_HasEmptyConstraint(RedClause) || !flag_GetFlagValue(Flags,
				flag_RSST))
			return FALSE;
		...
		for (i = clause_FirstLitIndex(); i < nc; i++)
			Scan = list_Cons(term_Copy(clause_LiteralAtom(clause_GetLiteral(
					RedClause, i))), Scan);
		...
		while (!list_Empty(ApproxUsableList) && list_Empty(EmptyClauses)) {
			...
			ApproxUsableList = list_PointerDeleteOneElement(ApproxUsableList,
					GivenClause);
			ApproxDerivables = red_CDDerivables(Theory, GivenClause, Flags,
					Precedence);
			ApproxDerivables = split_ExtractEmptyClauses(ApproxDerivables,
					&EmptyClauses);
			...
			if (!list_Empty(EmptyClauses)) { /* Exit while loop! */
				...
				CLAUSE DerClause;
				for (Scan = ApproxDerivables; !list_Empty(Scan) && list_Empty(
						EmptyClauses); Scan = list_Cdr(Scan)) {
		...
		if (!list_Empty(EmptyClauses)) {
			if (flag_GetFlagValue(Flags, flag_DOCSST)) {
				...
				puts(
						"************  Static Soft Typing Subproof finished. ************");
			}
		} else if (flag_GetFlagValue(Flags, flag_PSST)) {
...
/*********************************************************
 * reduce 1 clauseList va insert cac clause list thu duoc vao PROOFSEARCH
 * ket qua tra ve la empty clause list
 *
 INPUT:   A proof search object and a list of unshared clauses.
...
//					}
					...
					ClauseList = red_MergeClauseListsByWeight(ClauseList,
							BackReduced);
				}
			} else
				clause_Delete(Given);
...
/*********************************************************
 * Thuc hien saturation cac clauses trong PROOFSEARCH,
 *
 * Ket qua tra ve la list cac empty clauses thu duoc
 *
 *
 ...
 **********************************************************/
{
	...
	// TTQ_NOTE . Derivables - sinh ra tu red_SatInput
	while (!list_Empty(Scan) && n > 0 && (flag_GetFlagValue(Flags,
			flag_TIMELIMIT) == flag_TIMELIMITUNLIMITED || flag_GetFlagValue(
			Flags, flag_TIMELIMIT) > clock_GetSeconds(clock_OVERALL))) {
		Given = (CLAUSE) list_Car(Scan);
		if (clause_GetFlag(Given, CONCLAUSE)) {
			Derivables = inf_BoundedDepthUnitResolution(Given,
					prfs_UsableSharingIndex(Search), FALSE, Flags, Precedence);
 ...
 **********************************************************/
{
	...
	for (Scan1 = prfs_SplitStack(Search); !list_Empty(Scan1); Scan1 = list_Cdr(
			Scan1))
		for (Scan2 = prfs_SplitDeletedClauses(list_Car(Scan1)); !list_Empty(
				Scan2); Scan2 = list_Cdr(Scan2)) {
			Clause = (CLAUSE) list_Car(Scan2);
			if (!red_ForwardSubsumer(Clause,
					prfs_WorkedOffSharingIndex(Search), Flags, Precedence)
					&& !red_ForwardSubsumer(Clause, prfs_UsableSharingIndex(
							Search), Flags, Precedence) && !red_ClauseDeletion(
					prfs_StaticSortTheory(Search), Clause, Flags, Precedence)) {
				misc_StartErrorReport();
				misc_ErrorReport(
