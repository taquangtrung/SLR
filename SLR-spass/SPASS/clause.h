...
typedef enum {
	MAXIMAL = 1, STRICTMAXIMAL = 2, LITSELECT = 4
} MAXFLAG;
...
typedef enum {
	...
	EQUALITY_RESOLUTION,
	EQUALITY_FACTORING,
	MERGING_PARAMODULATION,
	PARAMODULATION,
	ORDERED_PARAMODULATION,
	SUPERPOSITION_RIGHT,
	SUPERPOSITION_LEFT,
	SIMPLE_HYPER,
	ORDERED_HYPER,
	...
	INPUT,
	CONDENSING,
	ASSIGNMENT_EQUATION_DELETION,
	OBVIOUS_REDUCTIONS,
	SORT_SIMPLIFICATION,
	REWRITING,
	CONTEXTUAL_REWRITING,
	MATCHING_REPLACEMENT_RESOLUTION,
	UNIT_CONFLICT,
	DEFAPPLICATION,
	TERMINATOR,
	TEMPORARY
...
/* like the sort-Bitfield existing for term, now used for varoccs.  */
	...
	typedef struct CLAUSE_HELP {
		int clausenumber;	// Thu tu cua clause trong tap cac clause (cai nay chinh la ID)
...
void clause_MoveSharedClause(CLAUSE, SHARED_INDEX, SHARED_INDEX, FLAGSTORE,
		PRECEDENCE);
void clause_DeleteSharedLiteral(CLAUSE, int, SHARED_INDEX, FLAGSTORE,
		PRECEDENCE);
...
void clause_FPrintCnfDFGProblem(FILE*, BOOL, const char*, const char*,
		const char*, const char*, LIST, LIST, FLAGSTORE, PRECEDENCE, HASH,
		BOOL, BOOL);
void clause_FPrintCnfFormulasDFGProblem(FILE*, BOOL, const char*, const char*,
		const char*, const char*, LIST, LIST, FLAGSTORE, PRECEDENCE); /* unused */
...
void
		clause_SetSplitDataFromParents(CLAUSE Result, CLAUSE Mother,
				CLAUSE Father);
...
LIST clause_MoveBestLiteralToFront(LIST, SUBST, SYMBOL, BOOL(*)(LITERAL, NAT,
		LITERAL, NAT));
...
static __inline__ BOOL clause_LiteralsAreComplementary(LITERAL L1, LITERAL L2) {
	return ((clause_LiteralIsNegative(L1) && clause_LiteralIsPositive(L2))
			|| (clause_LiteralIsNegative(L2) && clause_LiteralIsPositive(L1))); /* xor */
}
...
static __inline__ int clause_Length(CLAUSE Clause) {
	return (clause_NumOfConsLits(Clause) + clause_NumOfAnteLits(Clause)
 ...
 ***************************************************************/
{
...
#ifdef _TRUNGTQ_CODE_
	/**************************************************************/
	/* Function tac dong den justified literals                   */
	/**************************************************************/
	...
	static __inline__ TERM clause_GetJustifiedLiteralAtom(CLAUSE C, int Index) {
		return clause_LiteralAtom(clause_GetJustifiedLiteral(C, Index));
	}

	static __inline__ int clause_NumOfJustifiedLits(CLAUSE Clause) {
		return Clause->j;
	}

	static __inline__ void clause_SetNumOfJustifiedLits(CLAUSE Clause, int Number) {
		Clause->j = Number;
	}

	static __inline__ int clause_FirstJustifiedLitIndex(CLAUSE Clause) {
		return 0;
	}
...
static __inline__ int clause_Number(const CLAUSE Clause) {
	// thu tu cua clause trong tap cac clause
...
static __inline__ NAT clause_SplitLevel(CLAUSE Clause) {
	// Level of splitting where clause is valid
...
static __inline__ void clause_SetSplitField(CLAUSE Clause, SPLITFIELD B,
	...
	if (Clause->splitfield_length != Length) {
		...
		if (Length != 0) {
			Clause->splitfield
					= memory_Malloc(sizeof(SPLITFIELDENTRY) * Length);
		} else
...
static __inline__ void clause_ExpandSplitField(CLAUSE C, NAT Length) {
	...
	if (C->splitfield_length < Length) {
		...
		if (C->splitfield != NULL) {
			memory_Free(C->splitfield, sizeof(SPLITFIELDENTRY)
					* C->splitfield_length);
		}
...
static __inline__ void clause_UpdateSplitDataFromNewSplitting(CLAUSE Result,
	...
	if (field >= Result->splitfield_length) {
		...
		Result->splitfield = memory_Malloc((field + 1)
				* sizeof(SPLITFIELDENTRY));
	...
	Result->splitfield[field] = (Result->splitfield[field]
			| ((SPLITFIELDENTRY) 1 << Level));
}
...
static __inline__ void clause_UpdateSplitDataFromPartner(CLAUSE Result,
	...
	clause_SetSplitLevel(Result, clause_SplitLevel(Partner)
			> clause_SplitLevel(Result) ? clause_SplitLevel(Partner)
...
static __inline__ BOOL clause_HasReducedPredecessor(CLAUSE Clause) {
	...
	return (origin == CONDENSING || origin == REWRITING || origin == SPLITTING
			|| origin == ASSIGNMENT_EQUATION_DELETION || origin
			== SORT_SIMPLIFICATION || origin == OBVIOUS_REDUCTIONS);
}
...
static __inline__ void clause_SetFromMatchingReplacementResolution(
		CLAUSE Clause) {
...
static __inline__ BOOL clause_IsEmptyClause(CLAUSE C) {
	return (C != (CLAUSE) NULL && clause_HasEmptyAntecedent(C)
			&& clause_HasEmptySuccedent(C) && clause_HasEmptyConstraint(C));
}
...
static __inline__ BOOL clause_IsSimpleSortClause(CLAUSE Clause) {
	return (clause_HasEmptyAntecedent(Clause) && (clause_NumOfSuccLits(Clause)
...
static __inline__ BOOL clause_IsSubsortClause(CLAUSE Clause) {
	return (clause_IsSimpleSortClause(Clause) && term_IsVariable(
			term_FirstArgument(clause_LiteralSignedAtom(clause_GetLiteral(
					Clause, clause_NumOfConsLits(Clause))))));
}
...
static __inline__ void clause_ReInit(CLAUSE Clause, FLAGSTORE Flags,
...
static __inline__ void clause_SetDataFromParents(CLAUSE Result, CLAUSE Father,
		int i, CLAUSE Mother, int j, FLAGSTORE Flags, PRECEDENCE Precedence) {
	clause_OrientAndReInit(Result, Flags, Precedence);
