 ...
 **********************************************************/
{
	return ((Literal != NULL) && symbol_IsPredicate(clause_LiteralPredicate(
			Literal)));
}
 ...
 ***************************************************************/
{
	return clause_CopyLitIntervalExcept(Clause, clause_FirstAntecedentLitIndex(
			Clause), clause_LastAntecedentLitIndex(Clause), i);
}
 ...
 ***************************************************************/
{
	return clause_CopyLitIntervalExcept(Clause, clause_FirstSuccedentLitIndex(
			Clause), clause_LastSuccedentLitIndex(Clause), i);
}
 ...
 **********************************************************/
{
	return ((Clause != NULL) && clause_CheckSplitLevel(Clause)
			&& (clause_IsEmptyClause(Clause) ||
			/* Check the first literal as a "random" sample */
			clause_LiteralIsLiteral(clause_GetLiteral(Clause,
					clause_FirstLitIndex()))) && (clause_SplitLevel(Clause)
			== 0 || Clause->splitfield_length > 0)
 ...
 **********************************************************/
{
	...
	for (i = clause_FirstAntecedentLitIndex(Clause); i
		...
		if (fol_IsEquality(clause_LiteralSignedAtom(ActLit))) {
			...
			HelpRes = ord_Compare(term_FirstArgument(clause_LiteralSignedAtom(
					ActLit)), term_SecondArgument(clause_LiteralSignedAtom(
					ActLit)), FlagStore, Precedence);
 ...
 **********************************************************/
{
	...
	for (i = clause_FirstAntecedentLitIndex(Clause); i
			< clause_FirstSuccedentLitIndex(Clause); i++)
		if (clause_LiteralIsEquality(clause_GetLiteral(Clause, i)))
			return TRUE;
 ...
 **********************************************************/
{
	...
	while (Result < 4 && i < clause_Length(Clause) && (!(*Prop) || !(*Monadic)
			|| !(*NonMonadic))) {
		...
		if (!(*Grd) && Arity > 0 && Ground && !fol_IsEquality(
		...
		if (!(*NonMonadic) && Arity > 1 && !Ground && !fol_IsEquality(
				clause_GetLiteralAtom(Clause, i))) {
			Result++;
			*NonMonadic = TRUE;
		}
...
void clause_ContainsSortRestriction(CLAUSE Clause, BOOL *Sortres,
		BOOL *USortres)
/*********************************************************
 INPUT:   A clause.
 ...
 **********************************************************/
{
	...
	for (i = clause_FirstLitIndex(); i <= clause_LastAntecedentLitIndex(Clause)
			&& (!*Sortres || !*USortres); i++) {
		Term = clause_GetLiteralAtom(Clause, i);
 ...
 **********************************************************/
{
	...
	for (i = clause_FirstLitIndex(); i < clause_Length(Clause); i++)
		Result += term_NumberOfSymbolOccurrences(clause_GetLiteralAtom(Clause,
				i), Symbol);
	...
	for (i = clause_FirstLitIndex(); i < clause_Length(Clause); i++) {
		...
		if (!symbol_Equal(term_TopSymbol(Term), fol_Equality())
				|| (!symbol_IsVariable(term_TopSymbol(term_FirstArgument(Term)))
						&& !symbol_IsVariable(term_TopSymbol(
								term_SecondArgument(Term))))
				|| (symbol_IsVariable(term_TopSymbol(term_FirstArgument(Term)))
 ...
 **********************************************************/
{
	if (clause_Length(Clause) == 1 && !clause_HasEmptyAntecedent(Clause)
			&& clause_LiteralIsEquality(clause_FirstAntecedentLit(Clause))
			&& !term_Equal(term_FirstArgument(clause_LiteralAtom(
					clause_FirstAntecedentLit(Clause))), term_SecondArgument(
					clause_LiteralAtom(clause_FirstAntecedentLit(Clause)))))
		return TRUE;
 ...
 **********************************************************/
{
	...
	for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		for (i = clause_FirstSuccedentLitIndex(Clause); i < n; i++) {
			...
			if (symbol_Arity(Pred) == 1 && !list_PointerMember(NonFinite,
					(POINTER) Pred)) {
 ...
 **********************************************************/
{
	...
	if (list_Empty(Scan) || (clause_Weight(list_Car(Scan)) > Weight)) {
		return list_Cons(Clause, Scan);

	} else {
		while (!list_Empty(list_Cdr(Scan)) && (clause_Weight(list_Car(list_Cdr(
				Scan))) <= Weight)) {
			Scan = list_Cdr(Scan);
		}
 ...
 **********************************************************/
{
	...
	for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (weight < clause_MAXWEIGHT)
			clause_SORT[weight]
					= list_Cons(list_Car(Scan), clause_SORT[weight]);
		else
			clause_SORT[clause_MAXWEIGHT] = list_Cons(list_Car(Scan),
					clause_SORT[clause_MAXWEIGHT]);
	}
...
void clause_SetSplitDataFromList(CLAUSE Result, LIST List) {
	...
	while (!list_Empty(Scan)) {
		...
		clause_SetSplitLevel(Result, clause_SplitLevel(TempClause)
				> clause_SplitLevel(Result) ? clause_SplitLevel(TempClause)
	...
	if (l > Result->splitfield_length) {
		if (Result->splitfield != NULL)
			memory_Free(Result->splitfield, sizeof(SPLITFIELDENTRY)
					* Result->splitfield_length);
	...
	while (!list_Empty(List)) {
		...
		for (l = 0; l < TempClause->splitfield_length; l++)
			Result->splitfield[l] = Result->splitfield[l]
					| TempClause->splitfield[l];
	}
}
...
void clause_SetSplitDataFromParents(CLAUSE Result, CLAUSE Mother, CLAUSE Father) {
	...
	if ((clause_SplitLevel(Father) == 0) && (clause_SplitLevel(Mother) == 0))
		return;
	clause_SetSplitLevel(Result, clause_SplitLevel(Mother) > clause_SplitLevel(
			Father) ? clause_SplitLevel(Mother) : clause_SplitLevel(Father));
...
BOOL clause_LiteralIsSort(LITERAL L) {
	...
	return (symbol_IsPredicate(S) && (symbol_Arity(S) == 1));
}
 ...
 **********************************************************/
{
	...
	for (i = 0; i < l; i++) {
		clause_SetLiteral(Result, i, clause_LiteralCopy(clause_GetLiteral(
				Clause, i)));
		...
		if (!term_HasTermStamp(clause_LiteralSignedAtom(ActLit))) {
			...
			for (j = fa; j < n && Result; j++)
				if (i != j) {
					...
					HelpRes = ord_LiteralCompare(clause_LiteralSignedAtom(
							ActLit), clause_LiteralIsOrientedEquality(ActLit),
 ...
 ***********************************************************/
{
	...
	for (i = clause_FirstLitIndex(); i < n; i++)
		clause_LiteralSetAtom(clause_GetLiteral(Clause, i), subst_Apply(Subst,
				clause_GetLiteralAtom(Clause, i)));
}
 ...
 ***********************************************************/
{
	...
	for (i = clause_FirstLitIndex(); i < length; i++) {
		...
		if (clause_LiteralIsEquality(EqLit)) {
			HelpRes = ord_Compare(
					term_FirstArgument(clause_LiteralAtom(EqLit)),
					term_SecondArgument(clause_LiteralAtom(EqLit)), FlagStore,
					Precedence);
...
void clause_ReplaceLiteralSubSet(CLAUSE Clause, int StartIndex, int EndIndex,
		LIST Replacement, FLAGSTORE FlagStore, PRECEDENCE Precedence)
/**************************************************************
 ...
 **************************************************************/
{
	...
	for (i = StartIndex, Scan = Replacement; i <= EndIndex; i++, Scan
			= list_Cdr(Scan)) {
...
static __inline__ void clause_FixLiteralSubsetOrder(CLAUSE Clause,
		int StartIndex, int EndIndex, FLAGSTORE FlagStore,
		PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	/* Get the literals */
	literals = clause_GetLiteralSubSetList(Clause, StartIndex, EndIndex,
			FlagStore, Precedence);

	/* Sort them */
	literals = list_Sort(literals,
			(BOOL(*)(POINTER, POINTER)) clause_LiteralsCompare);

	/* Replace clause literals in subset with sorted literals */
	clause_ReplaceLiteralSubSet(Clause, StartIndex, EndIndex, literals,
			FlagStore, Precedence);

	list_Delete(literals);
}

void clause_FixLiteralOrder(CLAUSE Clause, FLAGSTORE FlagStore,
		PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A clause, a flag store, and a precedence.
 RETURNS: None.
 ...
 ***************************************************************/
{
	...
	clause_FixLiteralSubsetOrder(Clause,
			...
			clause_LastAntecedentLitIndex(Clause), FlagStore, Precedence);

	/* Fix succedent literal order */
	clause_FixLiteralSubsetOrder(Clause, clause_FirstSuccedentLitIndex(Clause),
			clause_LastSuccedentLitIndex(Clause), FlagStore, Precedence);

	/* Fix constraint literal order */
	clause_FixLiteralSubsetOrder(Clause,
			clause_FirstConstraintLitIndex(Clause),
			clause_LastConstraintLitIndex(Clause), FlagStore, Precedence);
 ...
 ***************************************************************/
{
	...
	for (i = lastlitleft, j = lastlitright; i >= firstlitleft && j
			>= firstlitright; --i, --j) {
		TERM lterm, rterm;
 ...
 ***************************************************************/
{
	...
	static const CLAUSE_COMPARE_FUNCTION clause_compare_functions[] = {
			clause_CompareByWeight, clause_CompareByDepth,
			clause_CompareByMaxVar, clause_CompareByClausePartSize,
			clause_CompareByLiterals, clause_CompareBySymbolOccurences,
	...
	result = 0;
	functions = sizeof(clause_compare_functions)
			/ sizeof(CLAUSE_COMPARE_FUNCTION);
 ...
 ****************************************************************/
{
	...
	Result = clause_CreateCrude(Constraint, Antecedent, Succedent, list_Nil(), Conclause);
 ...
 ****************************************************************/
{
	...
	for (k = clause_FirstAntecedentLitIndex(Clause); k
			<= clause_LastAntecedentLitIndex(Clause); k++) {
		ActLit = clause_GetLiteral(Clause, k);
		ActAtom = clause_LiteralAtom(ActLit);
		if (symbol_IsBaseSort(term_TopSymbol(ActAtom)) && (Strong
				|| term_IsVariable(term_FirstArgument(ActAtom)))) {
			if (++i != k) {
 ...
 ***************************************************************/
{
	...
	if (Clause->splitfield != NULL) {
		...
		memory_Free(Clause->splitfield, sizeof(SPLITFIELDENTRY)
				* Clause->splitfield_length);
	}
	clause_Free(Clause);
}
...
void clause_MoveSharedClause(CLAUSE Clause, SHARED_INDEX Source,
		SHARED_INDEX Destination, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
...
void clause_DeleteSharedLiteral(CLAUSE Clause, int Indice,
 ...
 ***************************************************************/
{
	...
	TermList = st_GetInstance(cont_LeftContext(), sharing_Index(ShIndex),
			NewVar);
	...
	while (!list_Empty(TermList)) {
		...
		for (Scan = DelList; !list_Empty(Scan); Scan = list_Cdr(Scan))
 ...
 ***************************************************************/
{
	...
	while (!list_Empty(TermList)) {
		...
		for (Scan = ClList; !list_Empty(Scan); Scan = list_Cdr(Scan))
			list_Rplaca(Scan, clause_LiteralOwningClause(list_Car(Scan)));
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstAntecedentLitIndex(Clause); i < n; i++)
		if (!fol_IsEquality(clause_GetLiteralAtom(Clause, i))
				|| !(term_IsVariable(term_FirstArgument(clause_GetLiteralAtom(
						Clause, i))) || term_IsConstant(term_FirstArgument(
						clause_GetLiteralAtom(Clause, i))))
				|| !(term_IsVariable(term_SecondArgument(clause_GetLiteralAtom(
						Clause, i))) || term_IsConstant(term_SecondArgument(
						clause_GetLiteralAtom(Clause, i)))))
			return FALSE;
 ...
 ***************************************************************/
{
	...
	if (clause_HasTermSortConstraintLits(Clause) || clause_HasOnlySpecDomArgs(
			Clause))
		return FALSE;

	for (i = 0; i < c; i++)
		CVars = list_NPointerUnion(term_VariableSymbols(clause_GetLiteralAtom(
				Clause, i)), CVars);
	...
	if (i == c) {
		...
		for (; i < c; i++)
			LitVars = list_NPointerUnion(LitVars, term_VariableSymbols(
					clause_GetLiteralAtom(Clause, i)));
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(Clause); i < length; i++) {
		Lit = clause_GetLiteral(Clause, i);
		if (clause_LiteralIsMaximal(Lit) && symbol_IsBaseSort(term_TopSymbol(
				clause_LiteralSignedAtom(Lit))))
			return TRUE;
	}
 ...
 ***************************************************************/
{
	...
	if (clause_NumOfAnteLits(Clause) > 0 || clause_NumOfSuccLits(Clause) > 1
...
/**************************************************************
 * TTQ_NOTE . Print 1 list cac clause
 *
 INPUT:   A list of clauses.
 RETURNS: Nothing.
 ...
 ***************************************************************/
{
	...
	if (!list_Empty(clause_ParentClauses(Clause))) {
		...
		for (Scan1 = list_Cdr(Scan1), Scan2 = list_Cdr(Scan2); !list_Empty(
				Scan1); Scan1 = list_Cdr(Scan1), Scan2 = list_Cdr(Scan2))
			printf(",%d.%d", (int) list_Car(Scan1), (int) list_Car(Scan2));
	}
}
 ...
 ***************************************************************/
{
	if (string_Equal(RuleName, "App"))
		return CLAUSE_DELETION;
	else if (string_Equal(RuleName, "EmS"))
		return EMPTY_SORT;
	else if (string_Equal(RuleName, "SoR"))
		return SORT_RESOLUTION;
	else if (string_Equal(RuleName, "EqR"))
		return EQUALITY_RESOLUTION;
	else if (string_Equal(RuleName, "EqF"))
		return EQUALITY_FACTORING;
	else if (string_Equal(RuleName, "MPm"))
		return MERGING_PARAMODULATION;
	else if (string_Equal(RuleName, "SpR"))
		return SUPERPOSITION_RIGHT;
	else if (string_Equal(RuleName, "SPm"))
		return PARAMODULATION;
	else if (string_Equal(RuleName, "OPm"))
		return ORDERED_PARAMODULATION;
	else if (string_Equal(RuleName, "SpL"))
		return SUPERPOSITION_LEFT;
	else if (string_Equal(RuleName, "Res"))
		return GENERAL_RESOLUTION;
	else if (string_Equal(RuleName, "SHy"))
		return SIMPLE_HYPER;
	else if (string_Equal(RuleName, "OHy"))
		return ORDERED_HYPER;
	else if (string_Equal(RuleName, "URR"))
		return UR_RESOLUTION;
	else if (string_Equal(RuleName, "Fac"))
		return GENERAL_FACTORING;
	else if (string_Equal(RuleName, "Spt"))
		return SPLITTING;
	else if (string_Equal(RuleName, "Inp"))
		return INPUT;
	else if (string_Equal(RuleName, "Rew"))
		return REWRITING;
	else if (string_Equal(RuleName, "CRw"))
		return CONTEXTUAL_REWRITING;
	else if (string_Equal(RuleName, "Con"))
		return CONDENSING;
	else if (string_Equal(RuleName, "AED"))
		return ASSIGNMENT_EQUATION_DELETION;
	else if (string_Equal(RuleName, "Obv"))
		return OBVIOUS_REDUCTIONS;
	else if (string_Equal(RuleName, "SSi"))
		return SORT_SIMPLIFICATION;
	else if (string_Equal(RuleName, "MRR"))
		return MATCHING_REPLACEMENT_RESOLUTION;
	else if (string_Equal(RuleName, "UnC"))
		return UNIT_CONFLICT;
	else if (string_Equal(RuleName, "Def"))
		return DEFAPPLICATION;
	else if (string_Equal(RuleName, "Ter"))
		return TERMINATOR;
	else {
		misc_StartErrorReport();
		misc_ErrorReport(
				"\nIn clause_GetOriginFromString: Unknown clause origin '%s'.",
				RuleName);
		misc_FinishErrorReport();
 ...
 ***************************************************************/
{
	...
	case EQUALITY_FACTORING:
		fputs("EqF", File);
		break;
	case MERGING_PARAMODULATION:
		fputs("MPm", File);
		break;
	case SUPERPOSITION_RIGHT:
		fputs("SpR", File);
		break;
	case PARAMODULATION:
		fputs("SPm", File);
		break;
	case ORDERED_PARAMODULATION:
		fputs("OPm", File);
		break;
	case SUPERPOSITION_LEFT:
		fputs("SpL", File);
		break;
	case GENERAL_RESOLUTION:
		fputs("Res", File);
		break;
	case SIMPLE_HYPER:
		fputs("SHy", File);
	...
	case REWRITING:
		fputs("Rew", File);
		break;
	case CONTEXTUAL_REWRITING:
		fputs("CRw", File);
		break;
	case CONDENSING:
		fputs("Con", File);
		break;
	case ASSIGNMENT_EQUATION_DELETION:
		fputs("AED", File);
		break;
	case OBVIOUS_REDUCTIONS:
		fputs("Obv", File);
		break;
	case SORT_SIMPLIFICATION:
		fputs("SSi", File);
		break;
	case MATCHING_REPLACEMENT_RESOLUTION:
		fputs("MRR", File);
		break;
	case UNIT_CONFLICT:
		fputs("UnC", File);
 ...
 ***************************************************************/
{
	...
	printf(" %s %s ", (clause_GetFlag(Clause, WORKEDOFF) ? "WorkedOff"
			: "Usable"), (clause_GetFlag(Clause, CLAUSESELECT) ? "Selected"
			: "NotSelected"));
 ...
 ***************************************************************/
{
	while (!list_Empty(ClList) && clause_Number((CLAUSE) list_Car(ClList))
			!= number)
 ...
 ***************************************************************/
{
	if (clause_HasSolvedConstraint(Clause) && !clause_GetFlag(Clause,
			CLAUSESELECT) && clause_NumOfAnteLits(Clause) > 0
			&& flag_GetFlagValue(Flags, flag_SELECT) != flag_SELECTOFF
			&& (flag_GetFlagValue(Flags, flag_SELECT)
					!= flag_SELECTIFSEVERALMAXIMAL || clause_NumberOfMaxLits(
					Clause) > 1)) {
		NAT i, n;
		LITERAL Lit;
		...
		if (flag_GetFlagValue(Flags, flag_SELECT) == flag_SELECTFROMLIST) {
			for (; i <= n; i++)
				if (symbol_HasProperty(term_TopSymbol(clause_LiteralAtom(
						clause_GetLiteral(Clause, i))), SELECTED)) {
					Lit = clause_GetLiteral(Clause, i);
					i = n + 1;
				}
		} else {
			...
			if (symbol_HasProperty(term_TopSymbol(clause_LiteralAtom(Lit)),
					SELECTED))
				i = n + 1;
			...
			for (; i <= n; i++)
				if (symbol_HasProperty(term_TopSymbol(clause_LiteralAtom(
					...
					Lit = clause_GetLiteral(Clause, i);
					i = n + 1;
				} else if (clause_LiteralWeight(Lit) < clause_LiteralWeight(
...
void clause_SetSpecialFlags(CLAUSE Clause, BOOL SortDecreasing,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	if (SortDecreasing && clause_IsSortTheoryClause(Clause, Flags, Precedence)
			&& symbol_HasProperty(term_TopSymbol(clause_GetLiteralTerm(Clause,
					clause_FirstSuccedentLitIndex(Clause))), DECLSORT))
		...
		if (symbol_IsPredicate(term_TopSymbol(atom))) {
			...
			if (ok) {
				...
				for (j = 0; (j < clause_FirstSuccedentLitIndex(Clause)) && ok; j++) {
					list_Rplaca(pair, list_Cons(
							clause_GetLiteralAtom(Clause, j), list_PairFirst(
									pair)));
				...
				for (j = clause_FirstSuccedentLitIndex(Clause); (j
						< clause_Length(Clause)) && ok; j++) {
					if (j != i) {
						list_Rplacd(pair, list_Cons(clause_GetLiteralAtom(
								Clause, j), list_PairSecond(pair)));
						vars = fol_FreeVariables(clause_GetLiteralAtom(Clause,
								j));
						for (l = vars; !list_Empty(l); l = list_Cdr(l)) {
							if (!term_ListContainsTerm(predvars, list_Car(l))) {
								ok = FALSE;
 ...
 ***************************************************************/
{
	...
	for (predindex = clause_FirstLitIndex(); predindex
			< clause_FirstSuccedentLitIndex(Clause); predindex++)
	...
	*Index = predindex;
	...
	for (i = clause_FirstLitIndex(); (i < clause_FirstSuccedentLitIndex(Clause))
			&& ok; i++) {
		if (i != predindex)
	...
	for (i = clause_FirstSuccedentLitIndex(Clause); (i < clause_Length(Clause))
			&& ok; i++)
	...
	if (!ok)
		...
		for (i = 0; i < clause_FirstSuccedentLitIndex(Clause); i++)
			if (i != predindex)
				list_Rplacd(Pair, list_DeleteElement((LIST) list_PairSecond(
						Pair), clause_GetLiteralAtom(Clause, i), (BOOL(*)(
						POINTER, POINTER)) term_Equal));
		for (i = clause_FirstSuccedentLitIndex(Clause); i < clause_Length(
				Clause); i++)
			list_Rplaca(Pair, list_DeleteElement((LIST) list_PairFirst(Pair),
					clause_GetLiteralAtom(Clause, i),
	...
	for (i = 0; i < n; i++) {
		...
		if (symbol_Equal(term_TopSymbol(Literal), fol_Not())) {
			if (symbol_Arity(term_TopSymbol(fol_Atom(Literal))) == 1
					&& symbol_IsVariable(term_TopSymbol(term_FirstArgument(
							fol_Atom(Literal)))))
...
void clause_FPrintCnfDFG(FILE* File, BOOL OnlyProductive, LIST Axioms,
		LIST Conjectures, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	if (Axioms) {
		...
		for (scan = Axioms; !list_Empty(scan); scan = list_Cdr(scan)) {
			Clause = (CLAUSE) list_Car(scan);
			if (!OnlyProductive || (clause_HasSolvedConstraint(Clause)
...
static void clause_FPrintDescription(FILE* File, const char* Name,
		const char* Author, const char* Status, const char* Description) {
...
static void clause_FPrintClauseFomulaRelationForClauses(FILE* File,
		LIST Clauses, HASH ClauseToTermLabelList)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	clause_FPrintClauseFomulaRelationForClauses(File, Axioms,
			ClauseToTermLabelList);
	clause_FPrintClauseFomulaRelationForClauses(File, Conjectures,
			ClauseToTermLabelList);
	fputs(").\n\n\n", File);
}
...
static void clause_FPrintSettings(FILE* File, LIST Axioms, LIST Conjectures,
		FLAGSTORE Flags, PRECEDENCE Precedence, HASH ClauseToTermLabelList)
/**************************************************************
 INPUT:   A file, a list of axioms, a list of conjectures,
 ...
 ***************************************************************/
{
	...
	if (ClauseToTermLabelList != (HASH) NULL)
		clause_FPrintClauseFormulaRelation(File, Axioms, Conjectures,
				ClauseToTermLabelList);
...
static void clause_FPrintClauses(FILE* File, BOOL OnlyProductive,
		BOOL PrintConjectures, LIST Clauses, FLAGSTORE Flags,
		PRECEDENCE Precedence)
/**************************************************************
...
void clause_FPrintCnfDFGProblem(FILE* File, BOOL OnlyProductive,
		const char* Name, const char* Author, const char* Status,
		const char* Description, LIST Clauses, LIST Conjectures,
		FLAGSTORE Flags, PRECEDENCE Precedence, HASH ClauseToTermLabelList,
 ...
 ***************************************************************/
{
	 ...
	 */
	fputs("list_of_clauses(axioms, cnf).\n", File);
	clause_FPrintClauses(File, OnlyProductive, FALSE, Clauses, Flags,
			Precedence);
	if (IsModel)
		clause_FPrintClauses(File, OnlyProductive, TRUE, Clauses, Flags,
				Precedence);
	fputs("end_of_list.\n\n", File);
	...
	if (!IsModel)
		clause_FPrintClauses(File, OnlyProductive, TRUE, Clauses, Flags,
				Precedence);
	clause_FPrintClauses(File, OnlyProductive, TRUE, Conjectures, Flags,
			Precedence);
	fputs("end_of_list.\n\n", File);

	if (PrintSettings)
		clause_FPrintSettings(File, Clauses, Conjectures, Flags, Precedence,
				ClauseToTermLabelList);
...
void clause_FPrintCnfFormulasDFGProblem(FILE* File, BOOL OnlyProductive,
		const char* Name, const char* Author, const char* Status,
		const char* Description, LIST Axioms, LIST Conjectures,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 INPUT:   A file, a list of axiom clauses and a list of conjecture clauses.
 A flag indicating whether only potentially productive clauses should
 ...
 ***************************************************************/
{
	...
	for (scan = Clauses; !list_Empty(scan) && !Equality; scan = list_Cdr(scan)) {
		Clause = (CLAUSE) list_Car(scan);
		for (i = clause_FirstAntecedentLitIndex(Clause); i < clause_Length(
				Clause); i++)
			if (fol_IsEquality(clause_GetLiteralAtom(Clause, i))) {
				Equality = TRUE;
	...
	fol_FPrintOtterOptions(File, Equality, flag_GetFlagValue(Flags,
 ...
 ***************************************************************/
{
	...
	while (Clauses) {
		...
		if ((Type && !clause_GetFlag(Clause, CONCLAUSE)) || (!Type
 ...
 ***************************************************************/
{
	...
	for (j = 0; j < n; j++)
		Variables = list_NPointerUnion(Variables, term_VariableSymbols(
				clause_GetLiteralAtom(Clause, j)));
	...
	fputs(",[", File);
	for (Iter = clause_ParentClauses(Clause); !list_Empty(Iter); Iter
			= list_Cdr(Iter)) {
...
void clause_PParentsFPrintParentClauses(FILE* File, CLAUSE Clause,
		BOOL ParentPts)
/**************************************************************
