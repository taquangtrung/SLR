...
typedef struct PROOFSEARCH_HELP {
	...
	LIST finmonpreds;
	SHARED_INDEX woindex;		// worked-off clauses index
	LIST wolist;				// worked-off clauses list
	SHARED_INDEX usindex;		// usable clauses index
	LIST uslist;				// usable clauses list
	SORTTHEORY astatic;			// sort theory: static overall approximation, only available in a non-equality setting
	SORTTHEORY adynamic;		// sort theory: dynamic approximation, only considering maximal declarations
	SORTTHEORY dynamic;			// sort theory: (not approximated) dynamic sort, for all maximal declarations
	...
	int keptclauses;
	int derivedclauses;
	int loops;					// bien dieu khien vong lap
	int backtracked;
...
static __inline__ LIST prfs_UsableClauses(PROOFSEARCH Prf) {
	/*
	 * Tach lay Usable Clauses tu PROOFSEARCH Prf
	 */
	return Prf->uslist;
}
...
static __inline__ void prfs_SetStaticSortTheory(PROOFSEARCH Prf,
		SORTTHEORY Theory) {
	Prf->astatic = Theory;
}
...
static __inline__ void prfs_SetDynamicSortTheory(PROOFSEARCH Prf,
		SORTTHEORY Theory) {
	Prf->dynamic = Theory;
}
...
static __inline__ void prfs_SetApproximatedDynamicSortTheory(PROOFSEARCH Prf,
		SORTTHEORY Theory) {
	Prf->adynamic = Theory;
}
...
static __inline__ BOOL prfs_SplitLevelCondition(NAT OriginLevel,
		NAT RedundantLevel, NAT BacktrackLevel) {
...
static __inline__ SPLIT prfs_GetSplitOfLevel(int L, PROOFSEARCH Prf) {
	LIST Scan;
	Scan = Prf->stack;
	while (!list_Empty(Scan) && (((SPLIT) list_Car(Scan))->splitlevel != L))
		Scan = list_Cdr(Scan);
...
static __inline__ LIST prfs_SplitStack(PROOFSEARCH Prf) {
	/*
	 * Tach lay Stack ra khoi 1 PROOFSEACH
	 */
	return Prf->stack;
}
...
static __inline__ BOOL prfs_SplitStackEmpty(PROOFSEARCH Prf) {
	/*
	 * Kiem tra Stack trong PROOFSEARCH co empty hay ko?
	 *
	 * - Tra ve TRUE neu Stack trong PROOFSEARCH day empty
	 */
