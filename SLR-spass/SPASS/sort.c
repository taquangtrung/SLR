 ...
 ***************************************************************/
{
	...
	printf(") Clause: %d Card: %d Fire: %d Var: ", clause_Number(sort_LinkClause(Link)),
			sort_LinkCard(Link), sort_LinkFire(Link));
...
static SLINK sort_TheoryLinkCreate(SORTTHEORY Theory, CLAUSE Origin, CLAUSE Clause, LITERAL Lit)
/**************************************************************
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstConstraintLitIndex(Clause); i <= clause_LastConstraintLitIndex(Clause); i++)
		if (symbol_Equal(Var, term_TopSymbol(term_FirstArgument(clause_GetLiteralAtom(Clause, i)))))
			Input = list_Cons(sort_TheoryNode(Theory, term_TopSymbol(clause_GetLiteralAtom(Clause,
					i))), Input);
		else {
 ...
 ***************************************************************/
{
	...
	if (term_IsVariable(Term)) {
		...
		Theory->suborigcls = list_Cons(list_Cons(Origin, list_Cons(clause_Copy(Clause), list_List(
				Link))), Theory->suborigcls);
	}
 ...
 ***************************************************************/
{
	...
	for (Scan = Conditions; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (sort_LinkNoResidues(Link))
			Result = list_Cons(sort_ConditionCreate(sort_ConditionVar(Cond), Constraint,
					Antecedent, Succedent, list_Cons(sort_LinkClause(Link), list_Copy(
							sort_ConditionClauses(Cond)))), Result);
		else {
			SYMBOL New, Old;
			...
			for (Lits = Succedent; !list_Empty(Lits); Lits = list_Cdr(Lits))
				term_ExchangeVariable(list_Car(Lits), Old, New);
			Result = list_Cons(sort_ConditionNormalize(sort_ConditionCreate(New, list_Nconc(
					Constraint, NewConstraint), list_Nconc(Antecedent, NewAntecedent), list_Nconc(
					Succedent, NewSuccedent), list_Cons(sort_LinkClause(Link), list_Copy(
					sort_ConditionClauses(Cond))))), Result);
		}
	}
	return Result;
 ...
 ***************************************************************/
{
	...
	return sort_ConditionNormalize(sort_ConditionCreate(Act, NewConstraint, NewAntecedent,
			NewSuccedent, NewClauses));
}
 ...
 ***************************************************************/
{
	return sort_ConditionCreate(sort_ConditionVar(C),
			term_CopyTermList(sort_ConditionConstraint(C)), term_CopyTermList(
					sort_ConditionAntecedent(C)), term_CopyTermList(sort_ConditionSuccedent(C)),
				...
				if (!sort_NodeValue(Theory, Head)) {
					...
					for (Help = sort_LinkInput(Link); !list_Empty(Help); Help = list_Cdr(Help))
						if (!list_Empty(sort_NodeConditions(list_Car(Help))))
							Clauses = list_Nconc(list_Copy(sort_ConditionClauses(list_Car(
									sort_NodeConditions(list_Car(Help))))), Clauses);
	...
	while (!list_Empty(Nodes)) {
		...
		for (Scan = sort_NodeLinks(Node); !list_Empty(Scan); Scan = list_Cdr(Scan)) {
			...
			if (sort_LinkDecrementFire(Link) == 0) {
				Head = sort_LinkOutput(Link);
				if (!sort_NodeValue(Theory, Head)) {
					Cond = sort_ConditionCreate(symbol_FirstVariable(), list_Nil(), list_Nil(),
							list_Nil(), list_Nil());
					for (Help = sort_LinkInput(Link); !list_Empty(Help); Help = list_Cdr(Help))
						if (!list_Empty(sort_NodeConditions(list_Car(Help))))
							Cond = sort_ExtendConditionByCondition(Cond, list_Car(
									sort_NodeConditions(list_Car(Help))));
 ...
 *******************************************************/
{
	...
	for (Scan = sort_TheorySuborigcls(Theory); !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (list_Empty(sort_LinkInput(Link)) && sort_LinkNoResidues(Link)) {
			...
			sort_DeleteConditionList(sort_NodeConditions(Node));
			sort_PutNodeConditions(Node, list_List(sort_ConditionCreateNoResidues(list_List(
					sort_LinkClause(Link)))));
			sort_PutNodeTrue(Theory, Node);
		}
	}
	...
	for (Scan = Sort2; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (!sort_NodeValue(Theory, Node)) {
			...
			list_Delete(Clauses);
			return NULL;
		} else if (!list_Empty(sort_NodeConditions(Node)))
			Clauses = list_Nconc(list_Copy(sort_ConditionClauses(
					list_Car(sort_NodeConditions(Node)))), Clauses);
	}
 ...
 *******************************************************/
{
	...
	Result = list_Nil();
	Search = list_List(
			sort_PairCreate(sort_Copy(Sort2), sort_ConditionCreateNoResidues(list_Nil())));
	Visited = list_Nil();
	...
	while (!list_Empty(Search)) {
		...
		if (!sort_TheorySortMember(Theory, Visited, sort_PairSort(Cand))) {
			...
			ValidStart = TRUE;
			Valid = TRUE;
			for (Scan = sort_PairSort(Cand); !list_Empty(Scan) && (ValidStart || Valid); Scan
					= list_Cdr(Scan)) {
				Node = (NODE) list_Car(Scan);
				if (sort_NodeMark(Node) != StartMark) {
					Valid = FALSE;
					ValidStart = FALSE;
				} else if (sort_NodeStart(Node) != StartMark)
					ValidStart = FALSE;
			}
			if (Valid) {
				if (ValidStart)
				...
				for (Scan = sort_PairSort(Cand); !list_Empty(Scan); Scan = list_Cdr(Scan)) {
					Node = (NODE) list_Car(Scan);
					for (Scan2 = sort_TheorySuborigcls(Theory); !list_Empty(Scan2); Scan2
							= list_Cdr(Scan2)) {
						Link = (SLINK) list_Third(list_Car(Scan2));
						if (sort_LinkOutput(Link) == Node && !list_Empty(sort_LinkInput(Link)))
							Search = list_Cons(sort_PairCreate(list_PointerDeleteDuplicates(
									sort_Intersect(sort_DeleteBaseNode(sort_Copy(
											sort_PairSort(Cand)), Node), sort_Copy(sort_LinkInput(
											Link)))), sort_ExtendConditionByLink(
									sort_ConditionCopy(sort_PairCondition(Cand)), Link)), Search);
					}
				}
			}
 ...
 *******************************************************/
{
	...
	for (j = clause_FirstConstraintLitIndex(Clause); j <= clause_LastAntecedentLitIndex(Clause); j++)
		if (j != i) {
			...
			if (symbol_IsBaseSort(term_TopSymbol(Atom)) && term_TopSymbol(term_FirstArgument(Atom))
					== Var)
				Result = sort_Intersect(Result, sort_TheorySortOfSymbol(Theory,
						term_TopSymbol(Atom)));
		}
...
static BOOL sort_MatchNoResidues(SORTTHEORY Theory, TERM Term1, TERM Term2, CLAUSE Clause,
		LIST* Clauses)
/*********************************************************
 ...
 *******************************************************/
{
	...
	for (scan = subst; !subst_Empty(scan); scan = subst_Next(scan)) {
		...
		if ((Cond = sort_TheoryIsSubsortOfNoResidues(Theory, sort_PairSort(Pair), varsort)) == NULL) {
			...
			NewClauses = list_Nconc(NewClauses, list_Copy(sort_ConditionClauses(sort_PairCondition(
					Pair))));
...
static SOJU sort_ComputeSortInternNoResidues(SORTTHEORY Theory, TERM Term, CLAUSE Clause, int i,
...
SOJU sort_ComputeSortNoResidues(SORTTHEORY Theory, TERM Term, CLAUSE Clause, int i,
		FLAGSTORE Flags, PRECEDENCE Precedence)
/*********************************************************
 ...
 *******************************************************/
{
	...
	while (!stack_Empty(SubtermStack)) {
		...
		if (!(SortPair = (SOJU) hash_Get(Term))) {
			SortPair = sort_ComputeSortInternNoResidues(Theory, Term, Clause, i, Flags, Precedence);
 ...
 *******************************************************/
{
	...
	while (!list_Empty(Clauses)) {
		...
		for (i = clause_FirstConstraintLitIndex(Clause); i < n; i++)
			Sorts = list_Cons((POINTER) term_TopSymbol(clause_LiteralAtom(clause_GetLiteral(Clause,
					i))), Sorts);
	...
	for (Scan1 = RenVars; !list_Empty(Scan1); Scan1 = list_Cdr(Scan1)) {
		...
		for (Scan2 = Constraint; !list_Empty(Scan2); Scan2 = list_Cdr(Scan2)) {
			...
			if (symbol_Equal(term_TopSymbol(term_FirstArgument(Atom)), OldVar))
				Lits = list_Cons(term_Create(term_TopSymbol(Atom), list_List(term_Create(NewVar,
						list_Nil()))), Lits);
		}
	}
	list_Delete(RenVars);
 ...
 ***************************************************************/
{
	...
	if (clause_HasOnlyVarsInConstraint(Clause, Flags, Precedence) && clause_HasSortInSuccedent(
			Clause, Flags, Precedence)) {
		...
		for (i = clause_FirstSuccedentLitIndex(Clause); i < length; i++) {
			...
			if (symbol_Arity(term_TopSymbol(clause_LiteralSignedAtom(LitK))) == 1) {
				...
				NewClause = clause_Create(NewConstraint, list_Nil(), NewSuccedent, Flags, Precedence);
 ...
 ***************************************************************/
{
	...
	for (i = clause_FirstSuccedentLitIndex(Clause); i < length; i++) {
		...
		if (clause_LiteralIsMaximal(LitK) && symbol_IsBaseSort(term_TopSymbol(
				clause_LiteralSignedAtom(LitK)))) {
			pli = i;
			NewSuccedent = list_List(term_Copy(clause_LiteralSignedAtom(LitK)));
			NewConstraint = list_Nil();
			lc = clause_LastConstraintLitIndex(Clause);
...
static LIST sort_ApproxClauses(LIST Clauses, FLAGSTORE Flags, PRECEDENCE Precedence)
/**************************************************************
 ...
 *******************************************************/
{
	...
	for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		for (Iter = Clauses; !list_Empty(Iter) && Kept; Iter = list_Cdr(Iter))
			if (list_Car(Iter) != list_Car(Scan) && !list_PointerMember(RedundantClauses, list_Car(
					Iter)) && subs_Subsumes(list_Car(Iter), list_Car(Scan), subs_NoException(),
					subs_NoException())) {
				Kept = FALSE;
...
SORTTHEORY sort_ApproxStaticSortTheory(LIST Clauses, FLAGSTORE Flags, PRECEDENCE Precedence)
/*********************************************************
 ...
 *******************************************************/
{
	...
	for (Scan = ApproxClauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		sort_TheoryInsertClause(Result, Clause, Clause, clause_GetLiteral(Clause,
...
STR sort_AnalyzeSortStructure(LIST Clauses, FLAGSTORE Flags, PRECEDENCE Precedence)
/*********************************************************
 INPUT:   A list of clauses, a flag store and a
 precedence.
	...
	for (Scan = Clauses; !list_Empty(Scan); Scan = list_Cdr(Scan)) {
		...
		if (clause_IsPotentialSortTheoryClause(Clause, Flags, Precedence)) {
			...
			symbol_AddProperty(term_TopSymbol(clause_GetLiteralTerm(Copy,
					clause_FirstSuccedentLitIndex(Copy))), DECLSORT);
			...
			sort_TheoryInsertClause(Theory, Clause, Copy, clause_GetLiteral(Copy,
	...
	for (Scan = Clauses; !list_Empty(Scan) && Decreasing; Scan = list_Cdr(Scan)) {
		...
		for (i = clause_FirstSuccedentLitIndex(Clause); i < l && Decreasing; i++) {
			...
			if (fol_IsEquality(Atom)) {
				...
				SojuLeft = sort_ComputeSortNoResidues(Theory, Left, Clause, i, Flags, Precedence);
				SojuRight = sort_ComputeSortNoResidues(Theory, Right, Clause, i, Flags, Precedence);
				if (sort_IsTopSort(sort_PairSort(SojuRight)) || sort_IsTopSort(sort_PairSort(
						SojuLeft))) {
					...
					if (!sort_Eq(sort_PairSort(SojuRight), sort_PairSort(SojuLeft))) {
						...
						if (Cond && !clause_LiteralIsOrientedEquality(clause_GetLiteral(Clause, i))) {
							sort_ConditionDelete(Cond);
							Cond = sort_TheoryIsSubsortOfNoResidues(Theory,
									sort_PairSort(SojuLeft), sort_PairSort(SojuRight));
						}
						if (Cond)
							sort_ConditionDelete(Cond);
						else {
